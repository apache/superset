...
If new system packages are required, they can be installed before application startup by overriding the container's `command`, e.g. to install Chrome and `chromedriver` in the Celery worker pods:
