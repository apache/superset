...
function to avoid retreiving the
same current_username or current_user_id for different users.

Here's an example:

- Skip cache when retreiving current user_id:  `{{ cache_key_wrapper(current_user_id()) }}`
- Skip cache when retreiving current username: `{{ cache_key_wrapper(current_username()) }}`
