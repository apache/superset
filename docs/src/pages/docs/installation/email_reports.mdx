...
Alerts and reports are not on by default, they are currently behind a feature flag, and also require some additional services and configurations.

#### Requirements:

- `Dockerfile`
-- webdriver to run a headless browser (for screenshots)
- `docker-compose.yaml`
...
- At least one of these is needed to sent alerts and reports:
