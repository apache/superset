...
- Global Async Queries:
    - Feature flag: `GLOBAL_ASYNC_QUERIES: True`
    - Dependencies: Redis 5.0+, celery workers configured and running
    - Documentation: https://github.com/apache/superset/blob/master/CONTRIBUTING.md#async-chart-queries
- Dashboard Native Filters:
    - Feature flag: `DASHBOARD_NATIVE_FILTERS: True`
- Alerts & Reporting:
    - Feature flag: `ALERTS_REPORTS: True`
    - Dependencies: [Celery workers configured & celery beat process](/docs/installation/async-queries-celery)
- Homescreen Thumbnails:
    ...
    ```
    - Dependencies: selenium, pillow 7, celery
- Dynamic Viz Plugin Import:
    - Feaure flag: `DYNAMIC_PLUGINS: True`
