...
When the migration runs, permissions are assigned based on existing `can_csv` access:
...
Gamma users do not receive `can_export_data` or `can_export_image` by default. Admins can grant these permissions to Gamma or custom roles as needed.
