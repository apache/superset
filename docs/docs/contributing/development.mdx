...
#### Quick Setup with MCP Service

For a streamlined setup that includes the MCP (Model Context Protocol) service for AI-powered features:

##### Option 1: Docker with MCP Service (Recommended)

```bash
# Start Superset and MCP service with docker-compose-light
...
Superset frontend will be available at http://localhost:9001.
...
The MCP service provides AI-powered capabilities including natural language chart generation.
See the [MCP Service Documentation](/docs/configuration/mcp-service) for more details.
