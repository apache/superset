...
archive longer than the retention window, without anyone acting. The next
section describes how operators control that window.

## Configuring retention (operators)

A daily background task (`deletion_retention.purge_soft_deleted`, scheduled
through Celery beat) permanently deletes objects that have been archived
longer than the retention window. The window resolves in this order:
...
Setting `SOFT_DELETE_PURGE_DRY_RUN = True` makes the task log what it *would*
purge without deleting anything, which is useful when validating a new window.
...
[version history](./version-history). The retention window for version history
