...
| `get_dashboard_data`              | Get bounded, filter-aware data across a dashboard's charts in one call                                                                                                               |
...
```json
{
  "request": {
    "dashboard_id": 42,
    "add": [
      {"component_type": "header", "text": "Sales overview"},
      {"component_type": "markdown", "code": "**Updated daily**", "width": 12},
...
Markdown tiles are appended in their own rows; headers and dividers are appended
directly to the grid or tab. Each addition can specify `target_tab` by name or
component ID (available from `get_dashboard_layout`). Without it, additions use
the first tab, or the grid on dashboards without tabs. Markdown width is 1–12
columns (default 4); height is in 8-pixel grid units (default 50).

The response includes generated `added_component_ids` and a `components` summary.
Reuse those IDs in `update`, for example
`{"id": "MARKDOWN-…", "code": "Revised notes"}`, or in `remove`, for example
`["MARKDOWN-…"]`. Updates merge only supplied non-null fields and cannot change a
component's type or placement. The reserved dashboard-title `HEADER_ID` is not a
section header and cannot be changed through this tool.

Operations in one request are validated before saving. Existing charts, native
filters, and unrelated dashboard properties are preserved. Removing a tile prunes
empty row/column wrappers but retains tabs. Invalid component IDs, incompatible
fields, invalid layouts, and conflicting update/removal operations return errors
without saving partial changes. Markdown uses the dashboard's existing renderer
and HTML sanitization configuration. Concurrent layout edits are last-write-wins;
avoid simultaneous edits from the UI and MCP clients. This tool does not reorder
or move existing components; use the dashboard editor for those operations.
...
The shared MCP query builder preserves explicit saved `orderby` values. For
`mixed_timeseries`, primary ordering applies only to the primary query; secondary
ordering uses `orderby_b` when provided. Typed Gantt `order_by` continues to map
through native `order_by_cols`, independently of these saved query keys.

The default `update_chart` preview-first flow validates the complete Gantt preview
state, including its saved-chart envelope. The chart name (`slice_name`) is
transport metadata, not a Gantt control; unknown visualization controls still fail
validation.

For same-dataset Gantt updates, omitted `tooltip_columns`, `tooltip_metrics`,
`order_by`, and `row_limit` keep their saved values. Explicit empty lists clear
those controls, and an explicitly supplied row limit replaces the saved limit.
A dataset change does not inherit the old series, tooltips, ordering, or filters.

A single Explore-authored temporal filter is retained when temporal settings are
omitted, without adding a second generated binding. Explicit `temporal_column` or
`time_range` replaces that native time predicate; explicit `filters`, including
`[]`, retains its replacement semantics. Unsupported multiple temporal predicates
remain validation errors. Empty strings in saved `x_axis_time_bounds` are treated
as unset bounds, including a fully cleared range.
