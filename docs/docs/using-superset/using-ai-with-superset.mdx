...
Sunburst ASCII previews reserve space for an omitted-row count and mark clipped
lines with dots. Preview width is bounded in terminal columns, including wide
Unicode labels. Standard pandas timestamps using pytz UTC or named time zones
retain their offsets and nanosecond precision in MCP results.
