...
Dashboard owners can add dynamic chart titles from the **Filters and controls**
modal by creating a **Dynamic title** display control. The title template can
reference dashboard native filters with tokens such as `{{country}}`.

Dynamic titles are scoped to the selected charts on the dashboard and update as
the referenced native filter values change. If a referenced filter has no value,
Superset falls back to the saved chart title when the rendered title is empty.

Dynamic titles are dashboard-specific metadata. They do not rename the saved
chart itself, and they only support dashboard native filters.
