...
Set **"Bound unit"** to **"% of column"** to express the bounds as percentages of either the column's signed maximum value or the magnitude of its sum. This option is unavailable with server pagination because the browser only receives one page of rows and cannot calculate a stable full-column denominator.
