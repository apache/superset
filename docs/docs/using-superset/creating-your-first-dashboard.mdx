...
Set **"Bound unit"** to **"% of column"** to express the bounds as percentages of either the column maximum or the sum of the absolute values in the column. Using value magnitudes prevents positive and negative values from cancelling into an unstable denominator. A non-positive maximum or zero magnitude sum falls back to the automatic data range. This option is unavailable with server pagination because the browser only receives one page of rows and cannot calculate a stable full-column denominator; existing percentage rules also use the automatic range while server pagination is enabled.
