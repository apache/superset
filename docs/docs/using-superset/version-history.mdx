...
editor of a chart created by an older Superset version, an import, or an
external tool may record a one-time settings-migration entry alongside the
user's change, as legacy settings are rewritten to their modern form. It
happens at most once per chart per upgrade, and later saves record only what
the user changed.
