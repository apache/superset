...
Version history applies to **charts and dashboards**. Datasets do not have a
version history panel; they participate in soft deletion and can be recovered
from [Recently Archived](./recently-archived), but their
...
Version rows older than the retention window are pruned by a daily background
task (`version_history.prune_old_versions`, scheduled through Celery beat).
The window is read from the `SUPERSET_VERSION_HISTORY_RETENTION_DAYS`
environment variable and defaults to 30 days.

Pruning never removes an entity's current state: any version that is still
live is preserved along with the rest of its transaction, however old it is.
What ages out is closed history — earlier states you could otherwise have
previewed or restored.

Set the variable to zero (or any non-positive value) to keep history
indefinitely. This window governs version history only; the separate retention
window for archived (soft-deleted) objects is described on
...
<img
  ...
  alt="A chart in preview mode: a banner across the top names the historical version and offers Restore, Open as new chart, and Close preview"
/>
