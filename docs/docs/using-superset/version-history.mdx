...
| Setting | Type | Effect |
| --- | --- | --- |
| `VERSION_HISTORY` | Feature flag | Enables the version history UI and the chart, dashboard, and dataset history, activity, and version-restore APIs |
...
30 days and accepts the same-name environment seed (-1 through 36500; 0 disables
scheduled purge). A host can install `SOFT_DELETE_RETENTION_DAYS_FUNC` to supply
