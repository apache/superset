...
change, as legacy settings are rewritten to their modern form — most
visibly the legacy time settings becoming an ad-hoc filter. On a chart
opened normally in Explore that is all that records; when Explore is
reached from a dashboard, a shared link, or a URL that picks the chart
type, a first save can carry a few more of these automatic rewrites. It
