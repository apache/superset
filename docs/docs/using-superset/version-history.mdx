...
Preview is a close approximation of the version, not a pixel-faithful
replay: charts render with their current definitions rather than the
definitions they had at that version, and properties outside the layout —
the theme and description among them — show their live values. Restore
applies the full captured state, so a restored dashboard can differ from
its preview in these respects.
