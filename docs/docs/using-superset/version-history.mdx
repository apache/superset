...
Empty chart slots left by older dashboard copies are repaired on save,
preserving their size and position.
...
When copying a dashboard **and its charts**, archived chart slots in the copy
become placeholders saying "This archived chart was not copied." Their layout
positions and sizes are preserved, but they do not link to the archived original.
Restoring that original chart later therefore does not attach it to the copy.
Live charts are still cloned. The source dashboard is unchanged, and ordinary
dashboard saves and chart restores keep their existing reattachment behavior.
This rule does not change copying a dashboard without copying its charts.

Copying with charts refuses a layout slot that still references a live chart
removed from the dashboard's membership. The error identifies the slot and the
membership mismatch; no partial copy is saved. Repair the source layout before
retrying. Malformed chart references also return a validation error (HTTP 422).
