...
`ROW_LIMIT`. Queries that use grouping sets, or post-processing that can add
rows (such as the Resample and Forecast options, or a custom
`EXTRA_PANDAS_POSTPROCESSING_OPS` operation), require the background path. Keep
this setting within your web server's request timeout.
...
3. **A running Celery worker.** The background export runs as a Celery task. If no worker
