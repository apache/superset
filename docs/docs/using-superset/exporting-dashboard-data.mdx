...
2. **The backend's SDK dependency.** Not installed by default; install
   ...
   Credentials (GKE workload identity, GCE metadata, Cloud Run), signed
   download URLs are minted through the IAM signBlob API, which requires the
   service account to hold `roles/iam.serviceAccountTokenCreator` on itself;
   key-file credentials sign locally and need no extra role.
...
for MinIO/LocalStack); for GCS, Application Default Credentials. The worker
needs write access to the bucket (e.g. `s3:PutObject`).
