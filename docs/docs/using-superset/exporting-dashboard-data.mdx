...
2. **The backend's SDK dependency.** Not installed by default; install
   ...
   for `GCSExportStorage`. Without it, exports fail. For `GCSExportStorage`
   with token-only Application Default Credentials (GKE workload identity,
   GCE metadata, Cloud Run), signed download URLs are minted through the IAM
   signBlob API, which requires the service account to hold
   `roles/iam.serviceAccountTokenCreator` on itself; key-file credentials
   sign locally and need no extra role.
...
| `EXCEL_EXPORT_LINK_TTL_SECONDS`        | `86400`                | Lifetime of the Superset download *link* (24h) — the redirect URL shared in the email and polling response. Each click on it mints a fresh short-lived storage URL (5 minutes, enough to complete one download); this setting does not govern that storage URL's lifetime. |
...
the **Celery worker** uploads the file (e.g. `s3:PutObject`), while the **web
server** signs the download URL at click time (for S3, credentials authorized
for `s3:GetObject`; for GCS, read access plus the signBlob role described in
the prerequisites when its credentials are token-only).

## Security considerations

- The emailed link resolves to a **pre-signed storage URL**: anyone who holds it
  can download the workbook until it expires. Keep the bucket **private**, enable
...
- **Embedded guest-token sessions get the polling download only.** The export
  ...
  **Export Images to Excel** is not available to guests (the webdriver cannot
  render under a guest identity; the API rejects it).
