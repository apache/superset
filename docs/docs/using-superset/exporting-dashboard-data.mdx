...
| `EXCEL_EXPORT_SYNC_MAX_ROWS`    | `100000`               | Largest export served during the request, as the combined `row_limit` of every query it would run. Over this (or with any query lacking a finite limit) the export is refused and asks for a bucket. Ignored once one is configured. |
