...
Time ranges include the start timestamp and exclude the end timestamp. If both
endpoints resolve to the same timestamp, the picker warns that the range is empty;
you can still apply it. To include a full day, use midnight on that day as the
start and midnight on the following day as the end.
