...
### Semantic Table time grains

For a Table chart backed by a semantic view, Time Grain applies when grouping by
a temporal dimension. Removing the last temporal dimension leaves the saved
choice intact, but the frontend omits a recognized dormant grain from aggregate
requests whose selected columns are known to be non-temporal (including no
grouping columns). Adding the temporal dimension again reuses the saved choice.
Unknown columns or grain values are not silently normalized. Ordinary dataset
charts and raw-record queries are unchanged.

This applies to requests rebuilt by the updated frontend in Explore and
dashboards; it does not rewrite saved charts. Direct API requests and saved
chart-data GET paths that bypass rebuilding remain subject to strict validation,
so old stored query contexts can still fail. Updated frontend assets are required.
...
Charts can be saved from datasets or accessible semantic views. Saving a new
chart requires access to its datasource; overwriting an existing chart also
requires chart edit permission. SQL Lab queries must first be saved as datasets
before they can be used as persistent chart datasources.
...
displayed, not how they're calculated. Switching it requires rerunning the query, though: for
non-additive metrics, a percentage option can pull in an additional rollup level for its
denominator even when the matching Total row/column isn't shown, so select **Run Query** after
changing this control to see the correct percentages. Because the result is always a percentage, a
per-metric custom formatter (currency, decimal precision, etc.) doesn't apply while a fraction
option is selected.
:::
...
**Code folding**: In the built-in Ace editor, use the arrows beside the line numbers to collapse or expand multiline parenthesized queries and block comments. Folding hides those lines without changing the SQL.
