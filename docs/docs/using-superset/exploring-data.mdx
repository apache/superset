...
- **Drill-down hierarchy** (`DRILL_DOWN`, off by default): configure an ordered **Drill-down
  ...
  step back up. When the dashboard has **Cross-filtering** enabled, each level also emits a
  cross-filter for the accumulated path so the rest of the dashboard follows along; with
  cross-filtering off, drilling stays local to the chart. It answers "let me navigate this
  dimension hierarchy by clicking."
