...
If you are running a non-dev docker image, e.g., `apache/superset:2.0.0`, that will not include a headless browser and you will need to install and configure it.  If deploying via `docker-compose`, only the `superset_worker` container needs the headless browser.
