...
For debugging purposes, Superset supports a "dry-run" mode where the worker only writes a message to the logs instead of actually sending email/Slack notifications.  This mode is off by default.  To enable dry-run mode, set `ALERT_REPORTS_NOTIFICATION_DRY_RUN` to `True` in your [superset config](https://github.com/apache/superset/blob/master/docker/pythonpath_dev/superset_config.py).
