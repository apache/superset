...
Export the FLASK_APP environnement variable in order to be able to execute any `superset` command:
...
In order to make the configuration file available to `superset` you need to execute the following command:
