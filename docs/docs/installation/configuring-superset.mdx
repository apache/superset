...
By default Superset is configured to use SQLlite, since it's a simple and fast way to get you started
(no installation needed) but for production environments you should a different database engine on a separate host or
container.
