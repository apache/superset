...
(no installation needed). But for production environments this is highly discouraged for security,
scalability and data integrity reasons. Use only the recommend database engines.
Also use a different database engine on a separate host or container.
