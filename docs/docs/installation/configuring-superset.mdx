...
Alternatively, you can set set the `SUPERSET_SECRET_KEY` by running the following command in your terminal:

```bash
export SUPERSET_SECRET_KEY=$(openssl rand -base64 42)
```

Update the `superset_config.py` by adding these lines:
