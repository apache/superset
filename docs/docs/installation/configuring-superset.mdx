...
`PYTHONPATH` or create an environment variable `SUPERSET_CONFIG_PATH` specifying full path of the `superset_config.py`.

For e.g. assuming your `superset_config.py` is under `/app` directory on a Linux-based system, then you can run:
```bash
...
If your application was installed using docker-compose an alternative configuration is required. See [https://github.com/apache/superset/tree/master/docker#readme](https://github.com/apache/superset/tree/master/docker#readme) for details.
...
For example, the following AUTH_ROLES_MAPPING dictionary would map the OAUTH group "superset_users" to the Superset roles "Gamma" as well as "Alpha", and the OAUTH group "superset_admins" to the Superset role "Admin".
...
Following is an example of mapping from LDAP groups to Superset roles.
...
For more details, Please refer (FAB Security documentation)[https://flask-appbuilder.readthedocs.io/en/latest/security.html].
