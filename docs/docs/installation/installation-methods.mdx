...
You install Superset as a Python package and run it that way, providing your own metadata database.
...
**Summary:** This takes advantage of containerization while remaining simpler than Kubernetes. This is the best way to try out Superset.
...
See [Docker Build Presets](https://superset.apache.org/docs/installation/docker-builds/#build-presets) for more information about the different image versions you can extend. 
...
You will deploy Superset into a K8s cluster, most commonly with the Helm chart. A K8s deployment can scale up and down based on usage and deploy rolling updates with zero downtime - features that big deployments appreciate.
