...
You might need to install pymssql if you're running superset with docker-compose. 

1. Enter docker container `docker exec -it superset_app bash`
2. `pip install pymssql`
3. exit container and run `docker-compose restart`
4. remove `Encrypt=yes` if you have trouble connecting with it
