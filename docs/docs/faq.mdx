...
then becomes the larger of this value and the per-dashboard `stagger_time` metadata. If the backend
does not provide a value at all, for example an older backend that predates this config or a value
explicitly set to `None`, the frontend falls back to a built-in default of `5000` milliseconds.
