...
then becomes the larger of this value and the per-dashboard `stagger_time` metadata. If the config
key is removed entirely (treated as null), the frontend falls back to a built-in default of `5000`
milliseconds.
