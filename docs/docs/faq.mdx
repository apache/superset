...
The manual **Refresh dashboard** button also staggers its chart requests. The window is controlled
by the `SUPERSET_DASHBOARD_MANUAL_REFRESH_STAGGER_MS` server config in `superset_config.py`. The
default is `5000` milliseconds, which matches the fallback used for timed refresh. A per-dashboard
`stagger_time` metadata value still wins when it is larger. Set the server config to `0` to keep
