...
| [QuestDB](/docs/configuration/databases#questdb)                        | `pip install questdb-connect`                                                      | `questdb://<UserName>:<DBPassword>@<Database Host>:<Port>/qdb`                                                                                                                                            |
...
[QuestDB](https://questdb.io/) is a high-performance 
[time-series database](https://questdb.io/glossary/time-series-database/) that 
specializes in hardware efficiency and high-cardinality data. It's ideal for 
demanding IoT or financial use-cases.
...
integrate Apache Superset with QuestDB. Alternatively, there are instructions for 
[Docker](https://questdb.io/docs/third-party-tools/superset/#installing-apache-superset-via-docker-recommended).

To install Superset + QuestDB Connect using `pip`:

```bash
pip install 'questdb-connect'
```

Once both Apache Superset and QuestDB are running, proceed with the following:
...
3. Select `+Database` to add the following parameters:
   ...
   - SQLALCHEMY URI: `questdb://admin:quest@localhost:9000/qdb`
4. Once connected, tables in QuestDB will be visible for creating Datasets in
   Apache Superset.

For more information, see the 
[QuestDB documentation](https://questdb.io/docs/third-party-tools/superset/).
