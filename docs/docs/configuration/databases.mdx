...
When using the ClickHouse Connect engine, Superset mutates column labels by default to avoid collisions. To preserve labels exactly as defined (for example, when a query refers to an alias within the same SELECT), set the following in the database Extra JSON:
...
This setting applies only to the ClickHouse Connect engine and keeps default behavior unchanged if omitted.
