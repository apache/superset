...
Arc is a high-performance time-series database built on DuckDB, Parquet, and MinIO. It provides exceptional write performance (1.89M records/sec) using MessagePack binary protocol and supports multi-database architecture for data isolation.

The recommended connector library for Arc is [arc-superset-dialect](https://pypi.org/project/arc-superset-dialect/).
...
The connection string format for Arc is:
...
**Parameters:**
- `api_key`: Your Arc API token (generate using Arc's authentication system)
- `hostname`: Arc server hostname or IP address
- `port`: Arc API port (default: 8000)
- `database`: Database namespace (e.g., "production", "staging", "default")

##### Example Connection Strings

**Basic connection:**
```
arc://AEzNL9-kBbsoRHYA74wLr_OHYU8H-ABEn1Gd1k3sNLA@localhost:8000/default
```

**Production environment:**
```
arc://your-api-token@arc-production.example.com:8000/production
```

##### Generating an API Key

To generate an API key for Arc, use the Arc CLI or API:

```bash
# Using Arc CLI
python3 cli.py auth create-token --name superset --permissions read,write

# Or via API
curl -X POST http://localhost:8000/auth/tokens \
  -H "Authorization: Bearer ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name": "superset", "description": "Superset integration"}'
```

##### Multi-Database Support

Arc supports multiple databases (schemas) within a single instance. In Superset:
- Each Arc database appears as a **schema** in the SQL Lab
- You can query tables using `database.table` syntax
...
- **High Performance**: 1.89M records/sec write throughput
- **DuckDB Query Engine**: Fast analytical queries with full SQL support
- **Multi-Database Architecture**: Organize data by environment, tenant, or application
- **Parquet Storage**: Columnar format with automatic compaction
- **S3-Compatible Storage**: Works with MinIO, AWS S3, GCS
