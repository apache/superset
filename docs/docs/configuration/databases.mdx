...
| [Atgenomix SeqsLab](/docs/configuration/databases#atgenomix-seqslab)              | `pip install seqslab-connector`                                                    | `seqslab+hive://token:{access_token}@{host}:443/{database}?http_path={job_run_id}`                                                                      |
...
```
seqslab+hive://token:{access_token}@{host}:443/{database}?http_path={job_run_id}
```

The connection string is automatically created once you create an interactive job.
It can be found in "Jobs" -> "the specific job" -> "Activity" -> "Connect to SQL".
