...
In the reporting feature, users can customize the email subject and include (date formatting)[https://strftime.org/] placeholders.
These placeholders will automatically be replaced with the corresponding UTC date when the email is sent.
...
- `"DATE_FORMAT_IN_EMAIL_SUBJECT"` [feature flag](/docs/configuration/configuring-superset#feature-flags) must be enabled to allow date formatting in email subjects (optional for reporting feature).
