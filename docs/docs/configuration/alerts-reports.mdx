...
- Use Playwright with Chrome: this is the recommended approach as of version 4.1.x and later. A working example of a Dockerfile that installs these tools is provided under "Building your own production Docker image" on the [Docker Builds](/docs/installation/docker-builds#building-your-own-production-docker-image) page. Read the code comments there as you'll also need to change a feature flag in your config.
...
In Superset versions 4.0.x and earlier, users installed Firefox or Chrome and that was documented here.
