...
## Coordination Cache Backend

Superset supports an optional coordination cache (`COORDINATION_CACHE_CONFIG`) for
...
The coordination cache uses Flask-Caching style configuration for consistency with other cache
backends. Configure `COORDINATION_CACHE_CONFIG` in `superset_config.py`:

```python
COORDINATION_CACHE_CONFIG = {
    "CACHE_TYPE": "RedisCache",
...
When `COORDINATION_CACHE_CONFIG` is not configured, Superset uses database-backed operations:
