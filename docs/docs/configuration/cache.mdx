...
[Custom cache backends](https://flask-caching.readthedocs.io/en/latest/#custom-cache-backends) are also supported. MinIO (compatible with other S3 services) backend [minio-flask-cache](https://github.com/greggailly/minio-flask-cache) is known to work.
