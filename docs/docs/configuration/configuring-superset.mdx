...
Make sure the pip package [`Flask-OIDC`](https://https://pypi.org/project/flask-oidc/) is installed on the webserver. This was succesfully tested using version 2.2.0.

For this to work, you need to create a new security manager in the same directory as `superset-config.py`, e.g. `keycloak_security_manager.py`:
```python
...
```
Then, append `superset-config.py` accordingly:
```python
...
AUTH_USER_REGISTRATION_ROLE = 'Public'
```
The client-specific OpenID information is contained in `client_secret.json`. Create this file in the same directory as `superset-config.py`:
```json
{  
    "<myOpenIDProvider>": {  
