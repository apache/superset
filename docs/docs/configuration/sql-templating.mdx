...
Here's an example of how to create a guest token with custom attributes:
...
These attributes will be available in SQL queries through the `get_guest_user_attribute()` macro.
