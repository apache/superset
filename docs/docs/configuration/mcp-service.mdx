...
```bash
superset mcp install [OPTIONS]

Options:
  --client [claude-code|claude-desktop]  MCP client to install for  [default: claude-code]
  --name TEXT                           Server name in client config  [default: superset]
...
superset mcp install --client claude-code --name my-superset
```
