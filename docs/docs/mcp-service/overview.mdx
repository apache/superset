...
The Model Context Protocol (MCP) is an open standard for connecting AI assistants to data sources and tools. Superset's MCP service exposes 20 tools, 2 prompts, and 2 resources that allow LLM agents to:
...
- **Chart Tools (8)**: Full CRUD operations, data export, visual previews
- **Dataset Tools (3)**: List, get info, discover filterable columns
- **Explore Tools (1)**: Interactive visualization workflow (PREFERRED)
- **System Tools (1)**: Comprehensive instance statistics
...
:::note
The MCP service runs on HTTP/SSE and requires a proxy script for Claude Desktop integration when not using HTTPS.
:::
...
### Ready to Use MCP?
- **[📚 API Reference](./api-reference)** - Try all 20 tools, 2 prompts, and 2 resources with examples
