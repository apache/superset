...
graph TB
    ...
    subgraph "Tool Layer"
        Tools[17 MCP Tools<br/>Tool Decorated]
...
1. **Define the Domain**
   ...
   - `sql_lab/` - SQL Lab and direct SQL execution
...
The SQL Lab tools follow special patterns for security and parameter handling:

```python
# SQL execution with parameter substitution
@mcp.tool
@mcp_auth_hook(['sql:execute'])
def execute_sql(request: ExecuteSqlRequest) -> ExecuteSqlResponse:
    ...
    # Validate access
    if not validate_database_access(request.database_id):
...
```python
def apply_parameters(sql: str, parameters: Optional[Dict[str, Any]]) -> str:
    """Apply parameters to SQL query safely."""
    if not parameters:
        return sql

    try:
        # Use Python string formatting
        # {table_name} and {year} style parameters
