...
Deleting the last conversation leaves an empty panel with opening suggestions;
use **New Chat** to start another conversation. If the conversation list cannot
be loaded or its initial conversation cannot be created, the panel shows the
error and still lets you create a new conversation.
