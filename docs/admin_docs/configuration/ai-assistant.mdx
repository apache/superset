...
twice. A hard-killed worker can leave a non-terminal message; stale-run recovery
is not automatic.

Retry a failed message submission with the same `request_id`. The assistant
keeps the original run identifier and its accepted context. An unclaimed run
can be queued again after an uncertain broker acknowledgement; only one delivery
can claim it for execution. Retrying a completed turn does not regenerate its
answer. An inline reconnect waits for the original producer's stored result
rather than starting another inference; live deltas still belong to that producer.
...
length is bounded by `AI_AGENT_MAX_TURNS` and `AI_AGENT_TIMEOUT_SECONDS`. A
timeout preserves whatever answer was completed before the deadline. Reaching
the step limit before a final answer ends the run as incomplete; provisional
tool-call narration is not presented as an answer.
...
The default Celery configuration registers `ai.prune_conversations` daily at
03:30. Run Celery beat to enforce it; if you replace `CELERY_CONFIG`, keep both
the `superset.ai.tasks` import and that beat entry. Set the retention to `0` to
disable pruning.
...
| `AI_AGENT_MAX_TURNS` | `20` | Model round trips before an incomplete-run error. |
