...
Cancellation requires the run identifier returned when submitting the message
and the same owned conversation. An unknown run or one belonging to another
conversation returns `404` without recording a cancellation request.
