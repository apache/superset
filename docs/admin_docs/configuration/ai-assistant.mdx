...
Profiles normally select capability *tiers* (`default`, `fast`,
`reasoning`). The provider maps tiers to
...
An explicit per-turn selection takes precedence over a profile's `model` pin;
without either pin, the profile's `model_alias` selects the tier. The same
selection is used on every model round trip and recorded in the run's audit trail.
