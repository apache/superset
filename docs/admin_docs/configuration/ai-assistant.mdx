...
Enable the assistant and its chat host, and configure a provider.
...
Run `superset db upgrade` to create the conversation tables, then `superset init`
to create and assign the assistant's permissions. Without the latter the
...
The optional authoring tools use Superset's FastMCP server in-process. Before
enabling a profile with `create_virtual_dataset`, `generate_chart`, or
`generate_dashboard`, install the `fastmcp` extra alongside your provider,
for example `pip install "apache-superset[ai-openai,fastmcp]"`.
...
Before starting the stack, add the optional provider dependency you need to
...
stack behaves exactly as it did before. If you add dependencies to an already
running stack, restart the application containers so bootstrap installs them.
...
| `AI_ASSISTANT_EVENT_STREAM_PREFIX` | `"ai-events-"` | Prefix for Redis event stream keys. |
...
| `AI_SUGGESTED_PROMPTS_ENABLED` | `False` | Generate opening-question suggestions with the provider. |
| `AI_SUGGESTED_PROMPTS_COUNT` | `3` | Number of suggested questions. |
| `AI_SUGGESTED_PROMPTS_MODEL_ALIAS` | `"fast"` | Model tier for suggestions. |
| `AI_SUGGESTED_PROMPTS_MODEL` | `None` | Concrete model override for suggestions. |
| `AI_SUGGESTED_PROMPTS_MAX_TOKENS` | `300` | Output token budget for suggestions. |
| `AI_SUGGESTED_PROMPTS_PROMPT` | `None` | Replace the suggestion prompt; supports `{count}` and `{max_chars}`. |
