...
| `AI_SUGGESTED_PROMPTS_ENABLED` | `False` | Generate contextual starter prompts when the panel is empty. |
| `AI_SUGGESTED_PROMPTS_COUNT` | `3` | Number of starter prompts to request. |
| `AI_SUGGESTED_PROMPTS_MODEL_ALIAS` | `"fast"` | Model tier used for starter prompts. |
| `AI_SUGGESTED_PROMPTS_MODEL` | `None` | Optional concrete model override for starter prompts. |
| `AI_SUGGESTED_PROMPTS_MAX_TOKENS` | `300` | Output-token cap for starter prompts. |
| `AI_SUGGESTED_PROMPTS_PROMPT` | `None` | Optional replacement prompt for starter-prompt generation. |
...
Suggested prompts are disabled by default. Enabling them makes one additional
model call when an empty panel requests suggestions, so it has the same provider
cost and data-governance implications as any other model request.
