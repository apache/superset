...
Each conversation remembers the profile used by its last accepted turn. Opening
it again or switching tabs restores that profile, independently of other chats.
Choosing a different profile applies when you send the next message; it does not
retarget a run already in progress. For API clients, omitting `agent_key` (or
sending null or an empty string) uses the conversation's stored profile. Send
`"agent_key": "default"` to explicitly switch back. Profile permissions are
checked on acceptance and again on execution. A removed or unavailable profile
falls back to a permitted one; if none is available, no turn is accepted.
