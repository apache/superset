...
Answer and code-block copy buttons use Superset's clipboard compatibility
fallback when the Clipboard API is unavailable or denied. If the browser blocks
both methods, select the text and use Ctrl+C (Cmd+C on macOS).
