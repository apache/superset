...
## Task services and async chart flags

Task infrastructure is registered at boot independently of runtime flags.
`GLOBAL_TASK_FRAMEWORK` gates Tasks UI, admission, task APIs (including polling and
cancellation) and MCP invocation, with existing permissions still enforced.
`GLOBAL_ASYNC_QUERIES` enables async chart eligibility and implies effective GTF in
the stock manager. Custom resolvers with GAQ-on/GTF-off use synchronous fallback.

Disabling effective GTF does not kill admitted workers, but blocks client lifecycle
access. Stop submissions and drain before disabling when polling/cancellation must
remain available. See [async query configuration](./async-queries-celery.mdx) for
the runtime matrix and operational guidance.
