...
`get_guest_user_attribute()` escapes string values using the database dialect's quoting
(the same mechanism as `url_param()`), so the example above is safe to interpolate
directly. If you pass `escape_result=False`, or interpolate non-string values (numbers,
booleans, arrays, objects), you are responsible for validating or allowlisting the values,
since they originate outside Superset.
