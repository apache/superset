...
To use the macro, you can reference the dataset either by its numeric ID or by its name.

- By ID: you need to find the dataset's ID. This can be done by going to the view showing all the datasets, hovering over the dataset you're interested in, and looking at its URL. For example, if the URL for a dataset is https://superset.example.org/explore/?dataset_type=table&dataset_id=42 its ID is 42.
...
If you prefer dataset's name:

```sql
SELECT * FROM {{ dataset("my_dataset") }} LIMIT 10
```
...
You can also force an alias for the returned subquery:

```sql
SELECT a.* FROM {{ dataset(42, alias="a") }} LIMIT 10
```
