...
terminology to make differently named datasets discoverable. To look a dataset
up by UUID, use a `uuid` filter rather than `search`: the column is binary and
the substring search casts to text, so a UUID string never matches. Matching
...
refuses them instead of attempting SQL lineage inference. Unsupported tools are
also hidden from `tools/list`, so clients are not offered a surface that only
refuses. Prompts and resources are unaffected: they expose schema and instance
metadata, not dataset rows. This restriction applies to every caller of the MCP
tools, not just a particular chat client.
...
and long descriptions can increase scan time. Benchmark with your metadata
database and catalog size when tuning discovery latency.
