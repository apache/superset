...
percentage, tie, or expression semantics. This bounds returned results, not the
work performed by the database or the number of rows fetched. In `dry_run`, the
SQL Server restriction remains unchanged because no results are fetched.
