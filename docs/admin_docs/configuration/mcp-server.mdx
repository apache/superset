...
All MCP list tools use **cursor-based pagination** (`cursor` / `next_cursor` fields) rather than offset pagination. To iterate through all results:
...
all_charts = []
while True:
    result = mcp.list_charts(cursor=cursor, page_size=50)
...
The `page` parameter from older integrations is no longer supported. Update any tooling that relied on offset pagination to use `cursor` / `next_cursor` instead.
