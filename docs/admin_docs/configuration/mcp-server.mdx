...
Gauge previews retain data-derived automatic bounds even when configured interval thresholds extend beyond the visible range; Vega previews clip those thresholds into adjacent, non-overlapping bands. Explicit min/max bounds still constrain intervals. On Gauge updates, clearing or changing the dashboard temporal binding preserves user-authored temporal ranges. Native `TEMPORAL_RANGE` adhoc filters require a nonempty, valid comparator (use `"No filter"` for an intentional unrestricted range).
