...
With the `bm25` strategy, a query matching a tool name is promoted ahead of
relevance-ranked results before the `max_results` limit is applied. Matching
ignores case and treats underscores and whitespace as equivalent separators
(including leading, trailing, or repeated whitespace). Only tools visible to
the caller are eligible. Pinned `always_visible` tools remain in `list_tools`
rather than search results; non-exact queries retain BM25 ranking.
