...
Tool-list and search permission checks, success/error audit writes, response-size
audits, and error hooks run in metadata threads with independent Flask contexts
and scoped sessions. These sessions are cleaned up when the operation finishes,
including after failure or client disconnection; inherited request contexts do
not share their ORM user/session with the thread.
