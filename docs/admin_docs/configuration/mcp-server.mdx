...
The `generate_chart` tool dispatches per chart type (`xy`, `table`, `pie`, `gauge`, `treemap_v2`, `pivot_table`, `interactive_pivot`, `mixed_timeseries`, `handlebars`, `big_number`, `histogram`, `box_plot`, `waterfall`, `bullet`) to a registered chart type plugin. Gauge requests use the public MCP discriminator `chart_type: "gauge"`; Superset stores the corresponding native Explore visualization as `viz_type: "gauge_chart"`. Two settings let operators enable or disable individual chart type plugins at runtime, without a code deploy.
...
`dimensions: []` to clear it explicitly.
...
Finite SQL Decimal values are preserved exactly and returned as JSON strings,
including values such as `0.5` that can be represented as floats. This uniform
contract avoids silently rounding `0.10000000000000000001` to `0.1`. Clients
should parse these strings with a decimal-aware numeric type when needed.
Decimal NaN, signaling NaN, and infinities remain JSON `null`.
