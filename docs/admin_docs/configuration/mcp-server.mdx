...
calling principal. Its payload contains only `dashboard_id` and `permalink_key`;
...
Undo restores only the filters whose current state still matches what the
notification applied, so a selection edited in the meantime is kept. Filters
not present in the notification's permalink keep their existing selections in
the open dashboard.

Reconnect deliberately does not fetch or replay filter state: there is no
latest-per-user permalink lookup, and replaying a previously received permalink
could overwrite manual changes or undo. If a notification is missed or its
permalink cannot be resolved, open `dashboard_url` from chat.
...
| `MCP_STRUCTURED_OUTPUT_ENABLED` | `False` | Advertise native tool `outputSchema` metadata and return matching `structuredContent`. The text-only default supports clients and transport bridges that cannot encode structured results. Enable only after validating every MCP client and bridge used by the deployment. |
...
| `max_bytes`          | `50000`   | Maximum serialized response size, in bytes                                                                                                                                                                                                                                                                                                                                                                             |
| `warn_threshold_pct` | `80`      | Warn when response exceeds this percentage of the limit                                                                                                                                                                                                                                                                                                                                                                |
| `max_list_items`     | `100`     | Cap on list-field length (e.g. `charts`, `native_filters`) applied to the `get_*_info` tools before falling back to more aggressive truncation. Raised from a hardcoded 30 in earlier versions; the higher default only keeps more data before the same size-budget fallback kicks in, so it's not a breaking change, but tenants that tuned workflows around the old 30-item cap should lower this value explicitly. |
| `excluded_tools`     | See above | Tools exempt from size checking (e.g., tools that return URLs, not data)                                                                                                                                                                                                                                                                                                                                               |

Write tools that cannot fit after truncation return a minimal confirmation rather
than a size-limit error. This confirmation retains identifying fields, error
details, and short string lists such as `changed_fields` (at most 20 items and
200 total characters). Larger lists are omitted; use `_response_truncated` and
`_truncation_notes` to detect a partial response and re-read the object if needed.
This fallback does not slim `update_chart`'s default payload.
...
MCP_TOOL_SEARCH_CONFIG = {
    ...
    "compact_schemas": True,   # Default description limit when include_schemas=True
...
| `include_schemas`        | `True`    | Include the full `inputSchema` in search results by default. When `False` ("summary mode"), search results omit `inputSchema` entirely and include a lightweight `parameters_hint` listing top-level parameter names. Full schemas are always used when a tool is actually invoked via `call_tool`. |
| `compact_schemas`        | `True`    | Legacy setting selecting the default description limit (300 when `True`, 0 when `False`) if `max_description_length` is omitted. Input schemas preserve `$defs`, `$ref`, nullable unions, and constraints in either mode; titles are omitted. Clients should resolve references within each tool's `inputSchema`.                                                                                                                                      |
...
| `get_dashboard_data`                                                                                     | `can_read` on `Dashboard` **and** `can_read` on `Chart` (it reads chart data across the dashboard) |
...
| ----------------------------------- | -------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| `ToolResultCompatibilityMiddleware` | Gates structured results and sanitizes errors that escape the inner error handler      | Enabled                                               |
...
| `ResponseSizeGuardMiddleware`       | Measures serialized response size, warns at 80% of limit, blocks at limit              | Enabled (configurable via `MCP_RESPONSE_SIZE_CONFIG`) |
...
`StructuredContentStripperMiddleware` is a deprecated compatibility class for
custom startup paths that retains its original stripping behavior. Replace it with
`ToolResultCompatibilityMiddleware(structured_output_enabled=False)`.

### Tool Result Value Contract

When `MCP_STRUCTURED_OUTPUT_ENABLED=True`, native tools advertise a concrete
`outputSchema` during tool discovery and return matching `structuredContent`
alongside their text representation. Clients can use the schema to understand
result fields and validate structured tool-call results. The default is `False`:
the compatibility middleware removes both fields as a pair and preserves the
text representation for clients and bridges that do not support structured output.
...
- If a tool call returns an error about exceeding size limits, the response size guard is blocking an oversized result
- Reduce `page_size` or `limit` parameters, use `select_columns` to exclude large fields, or add filters to narrow results
- To adjust the threshold, change `max_bytes` in `MCP_RESPONSE_SIZE_CONFIG`
...
When the `SOFT_DELETE` feature flag is enabled, deleting a chart or dashboard moves it to trash rather than removing it outright. The `list_charts` and `list_dashboards` tools accept a `deleted_state` parameter to surface those trashed objects:
...
Trashed rows carry a non-null `deleted_at` timestamp in the response (populated only when `deleted_state` is used). Visibility follows the same restore-audience scoping as the equivalent REST API filters (`chart_deleted_state` / `dashboard_deleted_state`): a non-admin caller sees only the trashed objects they can edit (the same audience that can restore them), not merely the ones they own; admins see all trashed objects. That scoping is narrower than the true restore audience in two cases it can't express in SQL: editorship granted via `EXTRA_EDITORS_RESOLVER` and guest users whose editorship derives from roles. Callers in either case may be able to restore a trashed object via `POST /restore` yet not see it listed here.

`deleted_state` is only defined on `list_charts` and `list_dashboards`. Other list tools don't declare the field at all, so passing it is silently ignored rather than rejected — it never returns trashed objects.
...
### Overriding a saved chart's time grain

Pass `extra_form_data={"time_grain_sqla": "P1M"}` to `get_chart_data` or
`get_chart_sql` to aggregate a saved chart by month instead of its saved grain.
For Generic Chart Axes charts, both MCP overrides and dashboard time-grain
filters update the `BASE_AXIS` column's `timeGrain`, regardless of its position
in the query's `columns` list. Other columns are left untouched. Queries without
a `BASE_AXIS` column continue to use `extras.time_grain_sqla`.

### Reading applied dashboard filters

When answering about a filtered dashboard, call `get_dashboard_info` with the
dashboard's `permalink_key` (a saved snapshot) or caller-supplied `filter_state`.
This value projection is provided by `get_dashboard_info`;
`get_dashboard_layout` retains its existing metadata-redacted layout state and
does not return native filter values.
The tool cannot observe browser state without one of these inputs. Its response
includes `filter_state` by default; an explicit `select_columns` must include it.

For users without Dataset drill/write metadata permissions, including embedded
guests, raw `dataMask` and `chartStates` remain hidden. Instead,
`filter_state.native_filter_values` contains configured native filter IDs, display
names, types, and scalar or flat-list selections for select, numeric range, time
range, and time-grain filters, plus display labels and exclusion flags when present.
It does not expose filter targets, dataset IDs, columns from query metadata, or
arbitrary nested filter state. Time-column filters, custom filters, unknown IDs,
and chart cross-filter state are not projected. Historical masks containing
time-column metadata are also omitted, even if the configured filter type has
changed; malformed type-specific value shapes are omitted rather than interpreted.
`native_filter_values_incomplete` flags omitted or malformed context, including SQL or wildcard predicates that
cannot be represented by selected values alone. It also flags retained selections
without query predicates (for example, a cleared or not-yet-applied mask). These
display values are not proof that a predicate is active.

These values describe a snapshot, not automatically enforced query predicates or
a complete account of filter scope. An empty or missing summary must not be
treated as permission to query unfiltered or workspace-wide data. Respect chart
scope and ask for clarification when the context cannot safely constrain an
answer. Dashboard access checks and row-level security remain independent of this
display-context projection. Integrations must ensure the lookup actually happens
and the assistant uses the result; tool guidance alone cannot guarantee this.

## Tool-call errors

For exceptions raised by tools, the shared error handler distinguishes permission
denials, invalid arguments, datasource failures, invalid SQL, and internal errors.
Datasource responses include an enumerated reason rather than raw driver output;
internal errors return an error ID rather than implementation details. Tools such
as `get_chart_data` that return structured error envelopes instead of raising do
not pass through this handler.

Missing tables, columns, and schemas log at `WARNING` without invoking
`MCP_ERROR_HOOK`, even when the exception inherits status 500. Connection failures
with status 500 or higher remain `ERROR` and invoke the hook. An explicitly
sub-500 exception remains a warning even if it carries a connection error reason:
the generic engine reason can also describe a caller's malformed column, and a
connection access denial is still a permission failure.

## Result serialization

Data-bearing tool responses sanitize row values, column samples, and statistics
even when Python producers supply tuples, sets, generators, or read-only mappings.
Missing and non-finite values become JSON `null`; non-UTF-8 binary values become
base64 strings prefixed with `base64:`. Boolean row counts are treated as
unavailable, producing `null` for nullable counts and `0` for non-nullable counts,
rather than being interpreted as one or zero rows.

## Chart data column counts

For saved and unsaved charts, `get_chart_data` column metadata excludes missing
values from `unique_count`. Values serialized as JSON `null`, including NaN,
NaT, and non-finite numbers, contribute to `null_count` instead.

## Geographic chart generation

`generate_chart`, `update_chart`, and `generate_explore_link` accept the same
bounded geographic configs. Call `get_chart_type_schema` with `country_map`,
`world_map`, or `deck_scatter` to discover availability and complete examples.
The `chart_type` discriminator belongs **inside** `config`. Existing chart
contracts and legacy native map behavior are unchanged.
...
```json
{
  ...
  "config": {
    ...
    "filters": [{"column": "country", "op": "IN", "value": ["USA"]}]
  }
}
```

- **Country map:** supported boundary sets are `usa`, `canada`, `australia`,
  `japan`, and `uk`. Select the source `region_format`: `name` (e.g.
  `California`), `abbreviation` (ISO suffix, e.g. `CA`), or `iso_3166_2`
  (e.g. `US-CA`). Identifiers are limited to Superset's bundled boundaries,
  not the entire ISO registry. Japanese abbreviation values are numeric
  **strings**, such as `13`. Names are exact-first, then case/diacritic-insensitive
  only when the result is unique. No fuzzy matching or geocoding is performed.
  Duplicate geometry names (for example, `Halton` in the UK boundary set)
  require an unambiguous abbreviation or ISO code instead of a name.
- **World map:** set `country_format` to `name`, `cca2`, `cca3`, or `cioc`,
  plus `entity` and `metric`. Choropleth color uses `metric`. For bubbles set
  `show_bubbles: true` and supply `secondary_metric` for size (it may be the
  same metric). Superset's bundled world-country dictionary defines coverage.
- **Geographic points:** set `chart_type: "deck_scatter"`,
  `latitude: {"name": "latitude"}`, and `longitude: {"name": "longitude"}`.
  Numeric coordinates must be within ±90 and ±180 respectively. Optional
  `dimension` groups/colors points; `radius_metric` aggregates a nonnegative
  size at each location. Otherwise `radius` is fixed in `point_unit`: `radius_m` (default), `radius_km`,
  `radius_miles`, `square_m`, `square_km`, or `square_miles`. Native minimum
  and maximum on-screen radii are 2 and 250 pixels. Spatial null filters
  exclude rows without coordinates.

Region values that cannot resolve for the selected country/format return an
`InvalidGeographicResult` error (`INVALID_GEOGRAPHIC_RESULT` during compilation).
For example, `BC` does not resolve for USA; choose Canada or explicitly filter
the dataset. No unrecognized returned rows are silently discarded. Multiple
query groups resolving to one boundary also fail: normalize source values
before aggregation rather than incorrectly adding averages or distinct counts.
Source values remain intact for data/export and country-map cross-filtering.
Database `NUMERIC`/Decimal query coordinates and metrics are supported when
finite and representable as JSON numbers. Coordinate bounds are checked before
JSON conversion; numeric strings and booleans are not accepted as coordinates.

Generation validates the configured result (up to `row_limit`, bounded at
10,000), not just the first two rows. This is not an audit of rows excluded by
filters or limits. Saved and cached-preview updates preserve omitted controls;
explicit null optional roles and empty filters clear them. Dataset rebinding
requires a complete typed config and discards old dataset roles and filters.

ASCII/table previews show source data and do not reproduce geography. Vega
previews return `UnsupportedGeographicPreview` rather than an unrelated chart.
Use the returned Explore link for the native map. Point maps use a public
Carto basemap; browser network access and deployment basemap policy still apply.
...
passed as `search` is treated as an exact UUID filter; an explicit `uuid` filter
can also look up one or more datasets by UUID. Matching
datasets are candidates, ordered by the requested sort (last modified by default), not
...
tool refuses in this mode, including SQL Lab, chart/dashboard previews and data,
cached query results, external semantic sources, mutations, and extension tools.
The one exception is `generate_bug_report`, which is deliberately public
(`protect=False`, listed in `ALLOWED_UNPROTECTED`) so diagnostics stay available
...
refuses them instead of attempting SQL lineage inference. Unsupported tools stay
listed and refuse when called, so the assistant reads an explanation it can pass
...
The default `MCP_GUEST_ALLOWED_TOOLS` contains only chart/dashboard tools,
none of which supports dataset-scoped mode. Enabling this allowlist therefore
also makes all of those embedded-guest tool calls refuse.
...
scan, without joins or additional queries, and replaces the `uuid` substring
predicate, whose behavior varied by database. It is not an indexed
