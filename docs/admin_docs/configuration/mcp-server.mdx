...
| `max_description_length` | `300`     | Budget for discovery prose and request-wrapper instructions (0 = no prose truncation). Only complete prose sentences are retained. Applies in both summary and full-schema modes.                                                                                                                                                                                                   |

Calling constraints are stored in the `request` property's schema description, so
schema-first clients can identify the wrapper without remembering previous calls.
Summary mode preserves these instructions in `parameters_hint`. Instructions are
never truncated: their length is deducted from the prose budget first. If a
...
size budgets. This setting does not change result-size limits or schema validation.
