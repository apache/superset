...
| ----------------------------------- | -------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| `ToolResultCompatibilityMiddleware` | Preserves structured results and sanitizes errors that escape the inner error handler  | Enabled                                               |
...
Native tools advertise a concrete `outputSchema` during tool discovery and return
matching `structuredContent` alongside their text representation. Clients can use
the schema to understand result fields and validate structured tool-call results.
