...
moving straight to an `Error` state. A schedule that stays `WORKING` past
its `working_timeout` always goes straight to `Error` without a retry,
regardless of setting. Execution timeouts (the Celery soft time limit) and
the `ALERT_REPORTS_EXECUTION_BUDGET_SECONDS` budget being exceeded are only
excluded from retries while the schedule isn't currently succeeding — i.e.
on its first run, or while it's already `Error`/`Retrying`. Once a schedule
has a prior `Success`, those same failures go through the normal retry path
like any other. This is configured per report in the
