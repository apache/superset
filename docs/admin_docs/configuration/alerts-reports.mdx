...
By default, dashboard PDF reports are produced by stitching raster screenshots. Enabling the `DASHBOARD_REPORTS_BROWSER_PRINT_PDF` [feature flag](/admin-docs/configuration/feature-flags) switches dashboard PDF reports to Playwright's native `page.pdf()`, which produces vector PDFs with selectable text.

### Prerequisites

- The `PLAYWRIGHT_REPORTS_AND_THUMBNAILS` [feature flag](/admin-docs/configuration/feature-flags) must also be enabled (Playwright must be installed and working).
- This path is dashboard-only. Chart PDF reports always use the screenshot path regardless of this flag.
- **Multi-tab reports via `ALERT_REPORT_TABS` are not supported.** When a report targets a dashboard spread across multiple permalink URLs (i.e. `ALERT_REPORT_TABS` is enabled and more than one tab is selected), `_get_browser_print_pdf` returns `None` and the report falls back to the raster screenshot path even when this flag is enabled. Single-URL dashboard reports (including dashboards with tabs navigated via hash fragments) are fully supported.
- If the browser-print render fails for any reason, Superset falls back to the screenshot path automatically. No report delivery is skipped.
...
When `DASHBOARD_REPORTS_BROWSER_PRINT_PDF` is enabled, the following additional controls appear in the Alerts & Reports modal for dashboard PDF reports:

| Control | Stored in | Default |
|---|---|---|
| **PDF font size** | `extra.dashboard.pdf_font_size` | `small` |
| **PDF orientation** | `extra.dashboard.pdf_orientation` | Portrait |
| **Use 2-column adaptive layout** | `extra.dashboard.pdf_layout` | Off (`1col`) |
| **PDF header left / center / right** | `extra.dashboard.pdf_header` | See global defaults below |
| **PDF footer left / center** | `extra.dashboard.pdf_footer` | See global defaults below |

Per-report values override the corresponding global config keys described below.

### Global Configuration Knobs

All of the following keys go in `superset_config.py`. They take effect for every browser-print PDF report that does not supply a per-report override.

#### Font size

```python
# Accepted values: 'small' (default), 'medium', 'large'.
# Controls table cell text (td/th), chart titles, markdown cards, and
# Big Number chart values. ECharts SVG/canvas labels are drawn at render
# time and cannot be resized after the fact.
BROWSER_PRINT_PDF_FONT_SIZE = None  # None → 'small'
```

Font size reference (CSS px at 1600 px viewport → approximate pt on A4 paper):
...
```python
# Accepted values: None/'1col' (default), '2col'.
# '2col' places charts that originally occupied ≤ 50% of a dashboard row
# side-by-side (two per row). Table charts are always kept full-width.
BROWSER_PRINT_PDF_LAYOUT = None  # None → '1col'
```
...
#   portrait  — A4 portrait (210×297 mm) throughout (default).
#   landscape — A4 landscape (297×210 mm) throughout.
#   auto      — Portrait by default; pages containing a table wider than
#               the portrait page width automatically switch to landscape.
BROWSER_PRINT_PDF_ORIENTATION = None  # None → portrait
```

#### Viewport width

```python
# Playwright viewport width used to render the dashboard before scaling
# to A4 paper width. Use the authored dashboard width for best results.
BROWSER_PRINT_PDF_VIEWPORT_WIDTH = 1600
```

#### Header and footer

By default, every PDF page receives a header band (dashboard title + date) and a footer band (confidential notice + page count). **Review and update these defaults before enabling the flag in a deployment that sends reports to external recipients.**

```python
# Set to False to disable the header and footer entirely.
BROWSER_PRINT_PDF_HEADER_FOOTER = True

# Global default header slot content.
# Supported tokens: {title} (dashboard title), {date} (print date).
# Each slot is truncated to 200 px; long strings show an ellipsis.
...
# The right slot ("Page N of M") is always fixed and cannot be overridden.
...
To remove the "Confidential" label and use a custom footer, for example:
...
To disable header and footer stamping completely:
