...
substituting another attachment cannot clear it. Detection is a heuristic, not a
guarantee that every chart contains correct data. Sparse text and solid-colour
charts are not rejected solely for their low whole-page pixel density.

Chart error and empty states are terminal for readiness, but a rendered error is
not successful chart data. Logs distinguish `rendered_holders`, `empty_holders`,
`error_holders` and `semantic_success`; `report_semantic_status` warns when a
completed capture includes terminal chart errors.

This is not a guarantee that every delivered image contains useful data. A solid
dark background, a title-only image, or a legitimate "No data" label may pass the
visual heuristic. Recognized terminal chart errors are delivered with a warning,
not treated as capture rejection. Operators requiring all charts to contain data
must not treat this policy as that guarantee. The strict invariant is that an
artifact **classified as invalid** cannot be delivered; undetected rendering
