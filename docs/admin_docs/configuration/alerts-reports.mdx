...
This is not a guarantee that every delivered image contains useful data. A large
enough title or logo can still exceed the detector's content thresholds while the
rest of the page is incomplete; removing the sparse-content override does not
solve that general limitation. Recognized terminal chart errors are capture-ready
