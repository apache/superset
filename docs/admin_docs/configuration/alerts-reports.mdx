...
- Each alert/report has an "Include a link back to Superset" option (enabled by default) controlling whether the call-to-action link (its email text is configurable via `EMAIL_REPORTS_CTA`) is included in email and Slack notifications. Uncheck it when recipients should not receive a link to your Superset host, e.g. for external audiences.
