...
By default, Alerts and Reports are executed as a user-type editor of the alert/report object (giving
priority to the last modifier, then the creator, then the first user-type editor). Only user-type
subjects are considered from the editors list, since tasks can only be executed as actual users
(not roles or groups). To use a fixed user account, just change the config as follows (`admin` in
this example):
