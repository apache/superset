...
# Scheduled reports share one 15-minute deadline across browser readiness,
# capture/PDF generation, delivery, and terminal-state persistence. Increase
# this only when the complete report pipeline is expected to take longer.
ALERT_REPORTS_EXECUTION_BUDGET_SECONDS = 900

# These reserves are part of (not additions to) the total budget. Readiness
# polling stops in time to leave capacity for the later phases.
