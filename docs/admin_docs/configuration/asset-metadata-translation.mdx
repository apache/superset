...
dashboard loads and the "Recents" list. On the chart and dashboard **list**
pages each row is serialized independently, so those still resolve per row;
back your hook with a cache or an in-process map if that path matters to you.
:::
