...
[SQL templating](/admin-docs/configuration/sql-templating) — virtual dataset
SQL, calculated columns, metric expressions, SQL Lab queries — can opt into
...
:::caution
The macro is bound only in the SQL template processor, so it reaches strings
Superset renders server-side as part of a query. It does **not** reach labels
the frontend renders from stored metadata -- chart axis labels and native
filter names among them -- where `{{ i18n('Sales') }}` would display literally.
:::
...
| SQL-templated content via `{{ i18n(...) }}` | any wrapped string |
