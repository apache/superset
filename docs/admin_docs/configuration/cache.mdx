...
When using `ExecutorType.EDITOR`, thumbnails are rendered as the primary user-type editor of
the dashboard or chart. Only user-type subjects are considered from the editors list, since
tasks can only be executed as actual users (not roles or groups).
