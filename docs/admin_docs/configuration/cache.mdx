...
When a value's serialized size exceeds this threshold it is not written to the data cache. The
request still returns its data, and the value is recomputed from the datasource the next time it is
requested. With `GLOBAL_ASYNC_QUERIES`, that includes the follow-up request a chart sends after its
background query finishes, so an oversized async chart runs its query twice. Each skip emits a
