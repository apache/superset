...
(chart and SQL query results, filter dropdown values, and compatible metrics/dimensions) is bounded by `DATA_CACHE_MAX_VALUE_SIZE` (in bytes), which defaults to `10 * 1024 * 1024` (10 MB). To
change it, set it in `superset_config.py`:
...
chart still renders, but the next load re-queries the datasource instead of getting a cache hit. Each
skip emits a WARNING log naming the key and byte size, and increments the `skip_cache_value_too_large`
statsd metric. Set to `None` to disable the check entirely (no serialization overhead is then
incurred).
