...
To control which user account Selenium authenticates as when rendering thumbnails and warming
up caches, set:

```python
SUPERSET_CACHE_WARMUP_USER = "admin"  # default; must have access to all dashboards/charts
```

Use a dedicated read-only service account here rather than a personal admin account, so that
cache warmup tasks don't fail if a specific user's credentials change.
