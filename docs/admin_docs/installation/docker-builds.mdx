...
- `dev`: For development, with dev-related utilities and root access. This
  ...
  some other used for development/CI. The published `-dev` images do not include a headless
...
Release tags follow this `superset`/`lean` naming starting with 7.0.0. Releases up to and
including 6.1.0 publish the minimal image under the plain tag (e.g. `6.1.0`) and have no
`-lean` tag.
