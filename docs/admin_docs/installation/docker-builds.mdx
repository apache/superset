...
- **Published releases** (`release`): published using
  tags like `7.0.0` and the `latest` tag.
...
# or start from apache/superset:7.0.0 to build on top of the batteries-included
# default (which already has psycopg2-binary, mysqlclient, MCP and Playwright).
# Change the version to the release you want to build from.
