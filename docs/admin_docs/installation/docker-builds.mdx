...
- **Published releases** (`release`): published by the release workflow after
  release manager sign-off, using tags like `7.0.0` and the `latest` tag.
...
- **Merges to release branches** (`push`): built locally for validation only.
  Official release images are published by the release manager after a release
  vote completes and the release is signed off.
...
- `superset`: The default Docker image, including both frontend and backend. Tags
  without a build_preset suffix are `superset` builds (ie: `latest`, `7.0.0`, `master`, ...).
...
- `lean`: A minimal image, including both frontend and backend but **no database
  drivers** — published under `-lean` tags (ie: `latest-lean`, `7.0.0-lean`, `master-lean`).
...
- `dev`: For development, with dev-related utilities and root access. This
  ...
  some others used for development/CI. The published `-dev` images do not include a headless
  browser; to add Chromium, build the `dev` target with `--build-arg INCLUDE_CHROMIUM=true`.
- `py311` and `py312`: Similar to lean but with a different Python version.

Release tags follow this `superset`/`lean` naming starting with 7.0.0. For earlier releases
(6.1.0 and before), the plain tag (e.g. `6.1.0`) is the minimal image; where a `-lean` tag
exists for one of those releases, it is the same minimal image.
...
- `latest-lean`: the minimal variant of the latest official release build, with no database
  drivers.
- `latest-dev`: the `-dev` image of the latest official release build, with dev-related
  utilities and root access (no headless browser).
...
- `master-dev`: The `-dev` image built from the `master` branch, with dev-related utilities
  and root access (no headless browser).
...
your desired Superset version and database drivers.

You would build the image with `docker build -t mysuperset:latest .` or `docker build -t ourcompanysuperset:7.0.0 .`

```Dockerfile
# Start from the minimal lean image (no drivers) and add exactly what you need.
# Replace this with the release tag you deploy.
FROM apache/superset:7.0.0-lean
...
RUN . /app/.venv/bin/activate && \
    ...
    Authlib \
    # Playwright with Chromium takes the screenshots for Alerts & Reports and thumbnails.
    # It is required as of 7.0.0, which removed Selenium support.
    playwright \
...
FROM apache/superset:7.0.0 AS my-custom-image
USER root
...
```bash
docker build --target=my-custom-image --build-arg=BUILD_TRANSLATIONS=true -t mysuperset:7.0.0 .
```
