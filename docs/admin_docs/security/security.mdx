...
default) only omits the "Request access" link. The contact guidance shown alongside it doesn't
depend on the link: chart/dashboard errors name the dataset owners when they're known, while SQL
Lab/table errors carry no owner information and always tell the viewer to contact their Superset
administrator.
