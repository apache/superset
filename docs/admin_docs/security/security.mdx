...
For accounts that use Superset's database authentication (`AUTH_TYPE = AUTH_DB`),
an operator with access to the Superset server can reset a forgotten password
with Flask AppBuilder's CLI. Run the command in the same environment and with the
same configuration as the running Superset application so it connects to the
correct metadata database:

```bash
superset fab reset-password --username alice
```

Replace `alice` with the account's **username**, not its display name or email
address. The command prompts for the new password twice and hides what you type.
Omitting `--password` keeps the new password out of the command and shell history.

For the repository's Docker Compose setup, run it inside the application container:

```bash
docker compose exec superset superset fab reset-password --username alice
```

Use the same Compose files and project options that you used to start your
deployment. The `superset` service name works regardless of the generated container
name. If the command reports that the user was not found, check the username and
metadata database configuration before trying again.

For LDAP, OAuth or other externally authenticated accounts, reset the password
with the identity provider instead. This command updates the password stored in
Superset's metadata database; it does not change credentials in an external
identity provider.
...
Set `SUBJECTS_RELATED_TYPES = None` to expose all subject types globally, including roles.

The RLS picker has the following entity-specific default so role-based RLS rules remain
selectable:
