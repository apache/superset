...
**viewers** (who can view it). For example, adding a role as an editor on a dashboard grants
everyone with that role edit access, without having to add each user individually.
...
For fine-grained control over which dashboards specific users, roles, or groups can access,
...
With this enabled, you can assign viewer subjects to each dashboard in its properties. Users
will see published dashboards where they are assigned directly, or indirectly through a role
or group subject.

**Important considerations:**
- Dashboard viewer access uses normal dataset checks unless `VIEWER_PROMISCUOUS_MODE` is enabled
...
- Dashboards without assigned viewers fall back to dataset-based access
