...
```bash
docker exec -it superset_app superset fab reset-password --username alice
```

Use your deployment's container name if it differs from `superset_app`. If the
command reports that the user was not found, check the username and metadata
database configuration before trying again.
