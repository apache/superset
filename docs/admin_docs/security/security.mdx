...
RLS clauses also support **Jinja templating**, so you can write dynamic filters such as
...
- If you assign an RLS filter directly to a virtual dataset, the clause must reference
  ...
  table's columns.
- In **SQL Lab**, RLS is also enforced: queries run against tables that have associated
  datasets with RLS filters will have the appropriate predicates injected automatically.
