...
default, since subjects enumerate every user, role, and group on the instance. Non-admins
receive a `403`. Only `GET` (list, get, info) is exposed — there is no create, update, or
delete, because subjects are derived automatically from users, roles, and groups and kept in
sync internally.
