...
function parseStoryFile(filePath, sourceConfig) {
  ...
  const defaultImportMatch = content.match(
    new RegExp(
      `import\\s+${componentName}(?:\\s*,\\s*{[^}]*})?\\s+from\\s+['"]([^'"]+)['"]`,
    ),
  );
...
function parseArgsContent(argsContent, args) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const keyOnlyMatch =
      !propMatch && trimmed.match(/^([a-zA-Z_$][a-zA-Z0-9_$]*):$/);
    ...
    if (propMatch) {
      ...
      key = keyOnlyMatch[1];
      const nextLine =
        i + 1 < lines.length ? lines[i + 1].trim().replace(/,\s*$/, '') : '';
...
function extractVariableArrays(content) {
  ...
  const varMatches = content.matchAll(
    /(?:export\s+)?(?:const|let)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)(?::\s*[^=]+)?\s*=\s*\[([^\]]+)\]/g,
  );
  ...
  const objWithOptionsMatches = content.matchAll(
    /(?:const|let)\s+([A-Z][A-Z_0-9]*)\s*=\s*\{[^}]*options:\s*([a-zA-Z_$][a-zA-Z0-9_$]*)/g,
  );
...
function parseArgTypes(argTypesContent, argTypes, fullContent) {
  ...
  while ((propMatch = propPattern.exec(argTypesContent)) !== null) {
    ...
    if (descIndex !== -1) {
      const descValue = extractStringValue(
        propConfig,
        descIndex + 'description:'.length,
      );
      if (descValue) {
    ...
    if (optionsMatch) {
      ...
      const varRefMatch = propConfig.match(
        /options:\s*([a-zA-Z_$][a-zA-Z0-9_$]*(?:\.[a-zA-Z_$][a-zA-Z0-9_$]*)?)/,
      );
      if (varRefMatch) {
        ...
        const shorthandMatch = propConfig.match(
          /(?:^|[,\s])options(?:[,\s]|$)/,
        );
    ...
    const controlShorthandMatch = propConfig.match(
      /control:\s*['"]([^'"]+)['"]/,
    );
    const controlObjectMatch = propConfig.match(
      /control:\s*\{[^}]*type:\s*['"]([^'"]+)['"]/,
    );
...
function extractDocsConfig(content, storyNames) {
  ...
  for (const storyName of storyNames) {
    ...
    const parametersPattern = new RegExp(
      `${storyName}\\.parameters\\s*=\\s*\\{`,
      's',
    );
    const parametersMatch = content.match(parametersPattern);

    if (parametersMatch) {
      const parametersContent = extractBalancedBraces(
        content,
        parametersMatch.index + parametersMatch[0].length - 1,
      );
      if (parametersContent) {
        // Extract sampleChildren - inline array using generic JSON parser
        const sampleChildrenArrayMatch =
          parametersContent.match(/sampleChildren:\s*\[/);
        if (sampleChildrenArrayMatch) {
          const arrayStartIndex =
            sampleChildrenArrayMatch.index +
            sampleChildrenArrayMatch[0].length -
            1;
          const arrayContent = extractBalancedBrackets(
            parametersContent,
            arrayStartIndex,
          );
          if (arrayContent) {
        ...
        const sampleChildrenStyleMatch = parametersContent.match(
          /sampleChildrenStyle:\s*\{/,
        );
        if (sampleChildrenStyleMatch) {
          const styleContent = extractBalancedBraces(
            parametersContent,
            sampleChildrenStyleMatch.index +
              sampleChildrenStyleMatch[0].length -
              1,
          );
          if (styleContent) {
        ...
        if (staticPropsMatch) {
          const staticPropsContent = extractBalancedBraces(
            parametersContent,
            staticPropsMatch.index + staticPropsMatch[0].length - 1,
          );
        ...
        if (galleryMatch) {
          const galleryContent = extractBalancedBraces(
            parametersContent,
            galleryMatch.index + galleryMatch[0].length - 1,
          );
          if (galleryContent) {
            gallery = {};

            // Extract component name
            const compMatch = galleryContent.match(
              /component:\s*['"]([^'"]+)['"]/,
            );
            if (compMatch) gallery.component = compMatch[1];

            // Extract sizes - variable reference
            const sizesVarMatch = galleryContent.match(
              /sizes:\s*([a-zA-Z_$][a-zA-Z0-9_$]*)/,
            );
            ...
            const stylesVarMatch = galleryContent.match(
              /styles:\s*([a-zA-Z_$][a-zA-Z0-9_$]*)/,
            );
            ...
            // Extract sizeProp
            const sizePropMatch = galleryContent.match(
              /sizeProp:\s*['"]([^'"]+)['"]/,
            );
            if (sizePropMatch) gallery.sizeProp = sizePropMatch[1];

            // Extract styleProp
            const stylePropMatch = galleryContent.match(
              /styleProp:\s*['"]([^'"]+)['"]/,
            );
        ...
        if (liveExampleMatch) {
          ...
          const startIndex =
            liveExampleMatch.index + liveExampleMatch[0].length;
          let endIndex = startIndex;
          while (
            endIndex < parametersContent.length &&
            parametersContent[endIndex] !== '`'
          ) {
            // Handle escaped backticks
            if (
              parametersContent[endIndex] === '\\' &&
              parametersContent[endIndex + 1] === '`'
            ) {
              endIndex += 2;
            } else {
              endIndex++;
        ...
        const renderComponentMatch = parametersContent.match(
          /renderComponent:\s*['"]([^'"]+)['"]/,
        );
        ...
        const triggerPropMatch = parametersContent.match(
          /triggerProp:\s*['"]([^'"]+)['"]/,
        );
        if (triggerPropMatch) {
          triggerProp = triggerPropMatch[1];
        }
        const onHidePropMatch = parametersContent.match(
          /onHideProp:\s*['"]([^'"]+)['"]/,
        );
        if (onHidePropMatch) {
        ...
        if (examplesMatch) {
          const examplesStartIndex =
            examplesMatch.index + examplesMatch[0].length - 1;
          const examplesArrayContent = extractBalancedBrackets(
            parametersContent,
            examplesStartIndex,
          );
          if (examplesArrayContent) {
            ...
            const exampleObjPattern =
              /\{\s*title:\s*['"]([^'"]+)['"]\s*,\s*code:\s*`/g;
            let exampleMatch;
            while (
              (exampleMatch = exampleObjPattern.exec(examplesArrayContent)) !==
              null
            ) {
              const title = exampleMatch[1];
              const codeStartIndex =
                exampleMatch.index + exampleMatch[0].length;
              // Find closing backtick for code
              let codeEndIndex = codeStartIndex;
              while (
                codeEndIndex < examplesArrayContent.length &&
                examplesArrayContent[codeEndIndex] !== '`'
              ) {
                if (
                  examplesArrayContent[codeEndIndex] === '\\' &&
                  examplesArrayContent[codeEndIndex + 1] === '`'
                ) {
                  codeEndIndex += 2;
                } else {
                  codeEndIndex++;
              ...
              // Unescape template literal escapes (source text has \` and \$ for literal backticks/dollars)
              const code = examplesArrayContent
                .slice(codeStartIndex, codeEndIndex)
                .replace(/\\`/g, '`')
                .replace(/\\\$/g, '$');
...
function extractArgsAndControls(content, componentName) {
  ...
  if (defaultExportMatch) {
    const metaContent = extractBalancedBraces(
      content,
      defaultExportMatch.index + defaultExportMatch[0].length - 1,
    );
    if (metaContent) {
      const metaArgTypesMatch = metaContent.match(/\bargTypes:\s*\{/);
      if (metaArgTypesMatch) {
        const metaArgTypesContent = extractBalancedBraces(
          metaContent,
          metaArgTypesMatch.index + metaArgTypesMatch[0].length - 1,
        );
  ...
  const {
    ...
    triggerProp,
    onHideProp,
  } = extractDocsConfig(content, storyNames);

  for (const storyName of storyNames) {
    // Try CSF 3.0 format: export const StoryName: StoryObj = { args: {...}, argTypes: {...} }
    const csf3Pattern = new RegExp(
      `export\\s+const\\s+${storyName}[^=]*=[^{]*\\{`,
      's',
    );
    ...
    if (csf3Match) {
      ...
      if (storyContent) {
        ...
        if (argsMatch) {
          const argsContent = extractBalancedBraces(
            storyContent,
        ...
        if (argTypesMatch) {
          const argTypesContent = extractBalancedBraces(
            storyContent,
    ...
    if (csf2ArgsMatch) {
      const argsContent = extractBalancedBraces(
        content,
        csf2ArgsMatch.index + csf2ArgsMatch[0].length - 1,
      );
      if (argsContent) {
        parseArgsContent(argsContent, args);
      }
    ...
    const csf2ArgTypesPattern = new RegExp(
      `${storyName}\\.argTypes\\s*=\\s*\\{`,
      's',
    );
    ...
    if (csf2ArgTypesMatch) {
      const argTypesContent = extractBalancedBraces(
        content,
        csf2ArgTypesMatch.index + csf2ArgTypesMatch[0].length - 1,
      );
      if (argTypesContent) {
        parseArgTypes(argTypesContent, argTypes, content);
      }
...
function generateMDX(component, storyContent) {
  ...
  const propsJson = JSON.stringify(mergedArgs, null, 2).replace(
    /"(\w+)":/g,
    '$1:',
  );
  ...
  const propsTable = Object.entries(mergedArgs)
    .map(([key, value]) => {
      const type =
  ...
  const publicExports = sourceConfig.importPrefix.startsWith('@superset/')
    ...
    : null;
  const isPubliclyExported = !publicExports || publicExports.has(componentName);

  // Determine component description based on source
  const defaultDesc =
    sourceConfig.category === 'ui'
...
<StoryWithControls
  ...
  controls={${controlsJson}}${
    sampleChildrenJson
      ? `
  sampleChildren={${sampleChildrenJson}}`
      : ''
  }${
    sampleChildrenStyleJson
      ? `
...
const CATEGORY_LABELS = {
  ui: {
    ...
    sidebarLabel: 'Core Components',
    description:
      'Buttons, inputs, modals, selects, and other fundamental UI elements.',
  },
  'design-system': {
    title: 'Layout Components',
    sidebarLabel: 'Layout Components',
    description:
      'Grid, Layout, Table, Flex, Space, and container components for page structure.',
  },
  extension: {
    title: 'Extension Components',
    sidebarLabel: 'Extension Components',
    description:
      'Components available to extension developers via @apache-superset/core/components.',
  },
};
...
function buildComponentMetadata(component, storyContent) {
  ...
  const { args, controls, gallery, liveExample } = extractArgsAndControls(
    storyContent,
    componentName,
  );
  const labels = CATEGORY_LABELS[category] || {
...
function generateExtensionTypeDeclarations(extensionComponents) {
  ...
  for (const comp of extensionComponents) {
    ...
    for (const type of extracted.types) {
      if (
        type.definition.includes('AntdAlertProps') ||
      ...
      if (
        type.definition.includes('PropsWithChildren') ||
...
async function main() {
  ...
  console.log(
    `Found ${enabledFiles.length} story files from enabled sources\n`,
  );

  // Find disabled story files (for tracking)
  const disabledFiles = findDisabledStoryFiles();
  console.log(
    `Found ${disabledFiles.length} story files from disabled sources (tracking only)\n`,
  );
  ...
  const jsonData = {
    ...
    statistics: {
      ...
      byCategory,
      extensionCompatible: componentMetadata.filter(c => c.extensionCompatible)
        .length,
  ...
  if (extensionComponents.length > 0) {
    ...
    console.log(
      `  ✓ extension types (${extensionComponents.length} components)`,
    );
  }
  ...
  console.log(
    `Tracked ${disabledFiles.length} components for future implementation.`,
  );
}
