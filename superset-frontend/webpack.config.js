...
// Note: .d.ts files are generated by `npm run plugins:build` before webpack runs,
// so we only need type checking here, not full project builds
if (!isDevMode) {
  plugins.push(
    new ForkTsCheckerWebpackPlugin({
      ...
      typescript: {
        ...
        // Don't use build mode - it triggers full project reference builds
        // which fail on story file imports. The .d.ts files are already
        // generated by `npm run plugins:build` in CI.
        configOverwrite: {
          compilerOptions: {
            skipLibCheck: true,
            incremental: true,
          },
          // Exclude story files from type checking - they import @storybook-shared
          // which is only available in Storybook builds
          exclude: [
            '**/node_modules/**',
            '**/*.stories.*',
            '**/*.test.*',
