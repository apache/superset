...
// `--check` (smoke check): fail instead of writing if the committed registry is
// stale, so CI catches a plugin added/removed without regenerating.
if (process.argv.includes('--check')) {
  const current = existsSync(OUT) ? readFileSync(OUT, 'utf8') : '';
  if (current !== generated) {
    console.error(
      `${path.relative(FE, OUT)} is out of date. ` +
        'Run `npm run gen:backend-querycontext-registry` and commit the result.',
    );
    process.exit(1);
  }
  console.log(`${path.relative(FE, OUT)} is up to date.`);
  process.exit(0);
}
