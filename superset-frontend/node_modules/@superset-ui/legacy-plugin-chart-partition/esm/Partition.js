...
const leafType = _propTypes.default.shape({
  ...
  val: _propTypes.default.number.isRequired
});
...
const propTypes = {
  ...
  equalDateSize: _propTypes.default.bool,
  levels: _propTypes.default.arrayOf(_propTypes.default.string),
  metrics: _propTypes.default.arrayOf(_propTypes.default.oneOfType([_propTypes.default.string, _propTypes.default.object])),
  numberFormat: _propTypes.default.string,
  partitionLimit: _propTypes.default.number,
  partitionThreshold: _propTypes.default.number,
  timeSeriesOption: _propTypes.default.string,
  useLogScale: _propTypes.default.bool,
...
function Icicle(element, props) {
  ...
  function drawVis(i, dat) {
    ...
    const x = _d.default.scale.linear().range([0, w]);

    const y = _d.default.scale.linear().range([0, h]);
    ...
    const root = (0, _d3Hierarchy.hierarchy)(datum); // node.name is the metric/group name
    ...
    function positionAndPopulate(tip, d) {
      ...
      const [tipX, tipY] = _d.default.mouse(element);
    ...
    function click(d) {
      ...
      const t = g.transition().duration(_d.default.event.altKey ? 7500 : 750).attr('transform', nd => "translate(" + x(nd.y) + "," + y(nd.x) + ")");
