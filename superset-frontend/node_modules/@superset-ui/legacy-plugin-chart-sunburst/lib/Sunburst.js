...
function Sunburst(element, props) {
  ...
  const colorFn = _color.CategoricalColorNamespace.getScale(colorScheme); // Helper + path gen functions
  ...
  const formatNum = (0, _numberFormat.getNumberFormatter)(numberFormat || _numberFormat.NumberFormats.SI_3_DIGIT);
  const formatPerc = (0, _numberFormat.getNumberFormatter)(_numberFormat.NumberFormats.PERCENT_3_POINT);
  ...
  function updateBreadcrumbs(sequenceArray, percentageString) {
    ...
    entering.append('svg:polygon').attr('points', breadcrumbPoints).style('fill', d => colorByCategory ? colorFn(d.name) : colorScale(d.m2 / d.m1));
    entering.append('svg:text').attr('x', (breadcrumbDims.width + breadcrumbDims.tipTailWidth) / 2).attr('y', breadcrumbDims.height / 4).attr('dy', '0.85em').style('fill', d => {
      ...
      const col = _d.default.hsl(colorByCategory ? colorFn(d.name) : colorScale(d.m2 / d.m1));
  ...
  function createVisualization(rows) {
    ...
    if (metrics[0] !== metrics[1] && metrics[1]) {
      ...
      ext = _d.default.extent(nodes, d => d.m2 / d.m1);
      colorScale = _d.default.scale.linear().domain([ext[0], ext[0] + (ext[1] - ext[0]) / 2, ext[1]]).range(['#00D1C1', 'white', '#FFB400']);
    }

    arcs.selectAll('path').data(nodes).enter().append('svg:path').attr('display', d => d.depth ? null : 'none').attr('d', arc).attr('fill-rule', 'evenodd').style('fill', d => colorByCategory ? colorFn(d.name) : colorScale(d.m2 / d.m1)).style('opacity', 1).on('mouseenter', mouseenter); // Get total size of the tree = value of root node from partition.
