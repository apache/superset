...
function CountryMap(element, props) {
  ...
  const colorScale = (0, _color.getSequentialSchemeRegistry)().get(linearColorScheme).createLinearScale((0, _d3Array.extent)(data, v => v.metric));
  const colorMap = {};
  data.forEach(d => {
  ...
  if (map) {
    ...
    const url = _countries.default[countryKey];
