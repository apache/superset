 ...
 * under the License.
 */
import { ensureIsInt, ensureIsArray, RollingType, ComparisionType, } from '@superset-ui/core';
...
export const rollingWindowOperator = (formData, queryObject) => {
    ...
    if (isValidTimeCompare(formData, queryObject)) {
        ...
        if (comparisonType === ComparisionType.Values) {
            ...
            columns = Array.from(metricsMap.entries()).map(([offset, metric]) => [comparisonType, metric, offset].join(TIME_COMPARISON_SEPARATOR));
        }
    }
    else {
