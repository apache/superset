...
export default React.memo(function ColumnConfigItem({ column, onChange, configFormLayout, }) {
    const { colors, gridUnit } = useTheme();
    const caretWidth = gridUnit * 6;
    return (<Popover title={column.name} content={() => (<ColumnConfigPopover column={column} onChange={onChange} configFormLayout={configFormLayout}/>)} trigger="click" placement="right">
      <div css={{
            cursor: 'pointer',
        ...
        {column.name}
        <i className="fa fa-caret-right" css={{
            position: 'absolute',
            right: 3 * gridUnit,
            top: 3 * gridUnit,
        }}/>
...
//# sourceMappingURL=ColumnConfigItem.jsx.map