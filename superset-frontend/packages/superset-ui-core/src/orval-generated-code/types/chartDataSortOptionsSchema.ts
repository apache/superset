 ...
 */
import type { ChartDataSortOptionsSchemaAggregates } from './chartDataSortOptionsSchemaAggregates';
import type { ChartDataSortOptionsSchemaColumns } from './chartDataSortOptionsSchemaColumns';
...
In the example, a new column `first_quantile` is created based on values in the column `my_col` using the `percentile` operator with the `q=0.25` parameter. */
  aggregates?: ChartDataSortOptionsSchemaAggregates;
