import { type MappingAlgorithm, theme, ThemeConfig } from 'antd-v5';
import { getBaseConfigWithOverrides } from '.';
import { SupersetTheme } from '..';

const getLightAlgorithm = (supersetTheme: SupersetTheme) => {
  const lightAlgorithm: MappingAlgorithm = seedToken => {
...
export const getLightThemeWithOverrides = (
  supersetTheme: SupersetTheme,
): ThemeConfig => ({
  ...getBaseConfigWithOverrides(supersetTheme),
  algorithm: getLightAlgorithm(supersetTheme),
});
