...
function entrypointFileTemplate(paths, type) {
  ...
  if (type === "css") {
    for (let i = 0; i < paths.length; i++) {
      rows += `<link rel="stylesheet" type="text/css" href="${paths[i]}" />`;
    }
  } else if (type === "js") {
    for (let i = 0; i < paths.length; i++) {
      rows += `<script src="${paths[i]}"></script>\n`;
    }
  }
  return rows
}

function replaceFile(html, manifest, name, type) {
  return html.replace(`<!-- Bundle ${name} ${type} -->`, entrypointFileTemplate(manifest.entrypoints[name][type], type));
}

function toFinalHTML(originalHtml, manifest) {
  let html = originalHtml;
  if (manifest) {
    // replace <!-- Bundle {name} {type} --> with the actual css/js files
    html = replaceFile(html, manifest, "theme", "css");
    html = replaceFile(html, manifest, "spa", "css");
    html = replaceFile(html, manifest, "theme", "js");
    html = replaceFile(html, manifest, "spa", "js");
  }
  return html;
}

function processHTML(manifest) {
  fs.readFile('templates/index.html', 'utf8', function(err, data) {
    let originalHtml = data;
    const finalContent = toFinalHTML(originalHtml, manifest);
    if (!fs.existsSync(BUILD_DIR)){
      fs.mkdirSync(BUILD_DIR, { recursive: true });
    }
    fs.writeFile(`${BUILD_DIR}/index.html`, finalContent, err => {
      if (err) {
        console.error(err);
      } else {
...
class ExfManifestPlugin {
  apply(compiler) {
    const { beforeEmit } = getCompilerHooks(compiler);

    beforeEmit.tap('ManifestPlugin', (manifest) => {
