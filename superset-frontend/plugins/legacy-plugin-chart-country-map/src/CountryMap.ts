...
function CountryMap(element: $TSFixMe, props: $TSFixMe) {
  const {
    data,
    width,
    height,
  ...
  // @ts-expect-error TS(7053): Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
  const colorFn = (d: $TSFixMe) => colorMap[d.properties.ISO] || 'none';
  ...
  let centered: $TSFixMe;

  const clicked = function clicked(d: $TSFixMe) {
  ...
  const updateMetrics = function updateMetrics(region: $TSFixMe) {
    if (region.length > 0) {
      resultText.text(format(region[0].metric));
    }
  };

  const mouseenter = function mouseenter(this: $TSFixMe, d: $TSFixMe) {
    // Darken color
    let c = colorFn(d);
    if (c !== 'none') {
    ...
    const result = data.filter(
      (region: $TSFixMe) => region.country_id === d.properties.ISO,
    );
    updateMetrics(result);
  };

  const mouseout = function mouseout(this: $TSFixMe) {
    d3.select(this).style('fill', colorFn);
    bigText.text('');
    resultText.text('');
  };

  function drawMap(mapData: $TSFixMe) {
  ...
  if (map) {
    ...
    d3.json(url, (error: $TSFixMe, mapData: $TSFixMe) => {
      if (error) {
        const countryName =
