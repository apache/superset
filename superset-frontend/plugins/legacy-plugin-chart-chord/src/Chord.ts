...
function Chord(element: $TSFixMe, props: $TSFixMe) {
  ...
  const group = svg
    ...
    .append('g')
    .attr('class', 'group')
    .on('mouseover', (d: $TSFixMe, i: $TSFixMe) => {
      chord.classed(
        'fade',
        (p: $TSFixMe) => p.source.index !== i && p.target.index !== i,
      );
    });
  ...
  group
    .append('title')
    .text((d: $TSFixMe, i: $TSFixMe) => `${nodes[i]}: ${f(d.value)}`);

  // Add the group arc.
  const groupPath = group
    .append('path')
    .attr('id', (d: $TSFixMe, i: $TSFixMe) => `group${i}`)
    .attr('d', arc)
    .style('fill', (d: $TSFixMe, i: $TSFixMe) => colorFn(nodes[i], sliceId));
  ...
  groupText
    .append('textPath')
    .attr('xlink:href', (d: $TSFixMe, i: $TSFixMe) => `#group${i}`)
    .text((d: $TSFixMe, i: $TSFixMe) => nodes[i]);
  // Remove the labels that don't fit. :(
  groupText
    .filter(function filter(this: $TSFixMe, d: $TSFixMe, i: $TSFixMe) {
      return (
  ...
  chord = svg
    ...
    .on('mouseover', (d: $TSFixMe) => {
      chord.classed('fade', (p: $TSFixMe) => p !== d);
    })
    .style('fill', (d: $TSFixMe) => colorFn(nodes[d.source.index], sliceId))
    .attr('d', path);
