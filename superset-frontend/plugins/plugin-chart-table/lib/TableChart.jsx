...
function SearchInput({ count, value, onChange }) {
    return (<span className="dt-global-filter">
      {t('Search')}{' '}
      <input className="form-control input-sm" placeholder={tn('search.num_records', count)} value={value} onChange={onChange}/>
    </span>);
}
function SelectPageSize({ options, current, onChange, }) {
    return (<span className="dt-select-page-size form-inline">
      {t('page_size.show')}{' '}
      <select className="form-control input-sm" value={current} onBlur={() => { }} onChange={e => {
...
export default function TableChart(props) {
    ...
    const toggleFilter = useCallback(function toggleFilter(key, val) {
        ...
        if (Array.isArray(updatedFilters[target]) &&
    ...
    const getColumnConfigs = useCallback((column, i) => {
        ...
        return {
            ...
            Header: ({ column: col, onClick, style }) => (<th title="Shift + Click to sort by multiple columns" className={[className, col.isSorted ? 'is-sorted' : ''].join(' ')} style={{
                    ...sharedStyle,
                    ...style,
                }} onClick={onClick}>
            ...
            Footer: totals ? (i === 0 ? (<th>{t('Totals')}</th>) : (<td style={sharedStyle}>
    ...
    maxPageItemCount={width > 340 ? 9 : 7} noResults={(filter) => t(filter ? 'No matching records found' : 'No records found')} searchInput={includeSearch && SearchInput} selectPageSize={pageSize !== null && SelectPageSize} 
