...
import { DataRecord, supersetTheme, ThemeProvider } from '@superset-ui/core';
...
const defaultProps = {
  ...
  getValueRange: jest.fn(() => null),
  isActiveFilterValue: jest.fn(() => false),
  isRawRecords: false,
  showCellBars: true,
  sortDesc: false,
  comparisonLabels: ['Main', '#', '△', '%'],
  getSharedStyle: jest.fn(() => ({ textAlign: 'left' })),
  groupHeaderColumns: {},
...
test('calculates value range for metrics when showCellBars is true', () => {
  const mockValueRange = jest.fn(() => [0, 100]);
