...
describe('BigNumberWithTrendline transformProps with Time Comparison', () => {
  ...
  describe('Time Comparison - No Change', () => {
    ...
    it('should handle very small changes as positive', () => {
      const props = generateProps(
      ...
      expect(result.percentageChange).toBeCloseTo(0.000001, 6);
    });
  });
  ...
  describe('Trendline Integration', () => {
    it('should work with trendline and time comparison', () => {
      const props = generateProps(
        [{ value: 1500 }], // current period
        [{ value: 1000 }], // previous period
        {
  ...
  describe('Data Format Handling', () => {
    it('should handle different metric names', () => {
      const customFormData = { ...formData, metric: 'sales' };
      const props = generateProps(
