...
export function transformSeries(series, colorScale, opts) {
    const { name } = series;
    const { area, filterState, seriesContexts = {}, markerEnabled, markerSize, areaOpacity = 1, seriesType, stack, yAxisIndex = 0, showValue, onlyTotal, formatter, totalStackedValues = [], showValueIndexes = [], richTooltip, } = opts;
    ...
    return {
        ...
        label: {
            ...
            position: 'top',
            formatter: (params) => {
                const { value: [, numericValue], dataIndex, seriesIndex, seriesName, } = params;
