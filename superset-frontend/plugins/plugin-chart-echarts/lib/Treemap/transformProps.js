 ...
 * under the License.
 */
import { CategoricalColorNamespace, getColumnLabel, getMetricLabel, getNumberFormatter, getTimeFormatter, NumberFormats, } from '@superset-ui/core';
...
export default function transformProps(chartProps) {
    ...
    const { colorScheme, groupby = [], metric = '', labelType, labelPosition, numberFormat, dateFormat, showLabels, showUpperLabels, dashboardId, emitFilter, } = {
    ...
    const transformer = (data, groupbyLabels, metric, depth, path) => {
        ...
        if (!restGroupby.length) {
            return transform(currGrouping, (result, value, key) => {
                (value ?? []).forEach(datum => {
                    ...
                    const item = {
                        name,
                        value: isNumber(datum[metric]) ? datum[metric] : 0,
                    };
        ...
        const sortedData = transform(currGrouping, (result, value, key) => {
            const name = formatSeriesName(key, {
                numberFormatter,
                timeFormatter: getTimeFormatter(dateFormat),
                ...(coltypeMapping[currGroupby] && {
