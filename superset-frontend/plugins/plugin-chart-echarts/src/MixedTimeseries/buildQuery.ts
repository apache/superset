...
import { flatOperator, pivotOperator } from '@superset-ui/chart-controls';
  ...
  const queryContextA = buildQueryContext(formData1, baseQueryObject => {
    const queryObjectA = {
      ...
      post_processing: [
        ...
        flatOperator(formData1, { ...baseQueryObject, is_timeseries: true }),
      ],
    } as QueryObject;
  ...
  const queryContextB = buildQueryContext(formData2, baseQueryObject => {
    const queryObjectB = {
      ...
      post_processing: [
        ...
        flatOperator(formData2, { ...baseQueryObject, is_timeseries: true }),
      ],
    } as QueryObject;
