...
export const AreaControlPanel: FC<AreaControlPanelProps> = ({
  onChange,
  value,
  datasource,
  form_data,
  ...
  const customizeTabContent = (
    <div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        {formValues.show_value && formValues.stack && (
          <div style={{ marginBottom: 16 }}>
            <CheckboxControl
              label={t('Only Total')}
              description={t('Only show the total value on the stacked chart, and not show on the selected category')}
              value={formValues.only_total ?? true}
              onChange={handleChange('only_total')}
              renderTrigger
              hovered
            />
          </div>
        ...
        {formValues.show_value && formValues.stack && !formValues.only_total && (
          <div style={{ marginBottom: 16 }}>
            <TextControl
              label={t('Percentage threshold')}
              description={t('Minimum threshold in percentage points for showing labels.')}
              value={formValues.percentage_threshold}
              onChange={handleChange('percentage_threshold')}
              isFloat
              controlId="percentage_threshold"
            />
          </div>
        )}
        ...
        <div style={{ marginBottom: 16 }}>
          <CheckboxControl
            ...
            description={t('Whether to show extra controls or not. Extra controls include things like making multiBar charts stacked or side by side.')}
...
(AreaControlPanel as any).isModernPanel = true;
  ...
  controlPanelSections: [
    {
      label: null,
      expanded: true,
      controlSetRows: [[AreaControlPanel as any]],
    },
  ],
  controlOverrides: {
    row_limit: {
