...
function getTotalValuePadding({
  ...
  width: number;
  height: number;
  valueToShow: string; // DODO added 45525377
}) {
  const padding: {
    left?: string;
    top?: string;
  ...
  const LEGEND_WIDTH = valueToShow.length * 7.47; // DODO changed 45525377
  if (chartPadding.top) {
    padding.top = donut
...
export default function transformProps(
  ...
  const totalText = t('Total: %s', numberFormatter(totalValue)); // DODO added 45525377
    ...
    graphic: showTotal
      ? {
          ...
          ...getTotalValuePadding({
            ...
            width,
            height,
            valueToShow: totalText, // DODO added 45525377
          }),
          style: {
            // text: t('Total: %s', numberFormatter(totalValue)),
            text: totalText, // DODO changed 45525377
            fontSize: 16,
            fontWeight: 'bold',
          },
          z: 10,
