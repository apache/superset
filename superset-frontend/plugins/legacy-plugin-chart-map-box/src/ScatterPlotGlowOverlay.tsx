...
const defaultProps = {
  ...
  dotRadius: 4,
  lngLatAccessor: (location: $TSFixMe) => [location[0], location[1]],
  renderWhileDragging: true,
};

const computeClusterLabel = (properties: $TSFixMe, aggregation: $TSFixMe) => {
...
class ScatterPlotGlowOverlay extends PureComponent {
  ...
  drawText(ctx: $TSFixMe, pixel: $TSFixMe, options = {}) {
    const IS_DARK_THRESHOLD = 110;
    const {
      // @ts-expect-error TS(2339): Property 'fontHeight' does not exist on type '{}'.
      fontHeight = 0,
      ...
      radius = 0,
      // @ts-expect-error TS(2339): Property 'rgb' does not exist on type '{}'.
      rgb = [0, 0, 0],
      // @ts-expect-error TS(2339): Property 'shadow' does not exist on type '{}'.
      shadow = false,
    } = options;
    ...
    // @ts-expect-error TS(2339): Property 'compositeOperation' does not exist on ty... Remove this comment to see the full error message
  ...
  redraw({ width, height, ctx, isDragging, project }: $TSFixMe) {
    const {
      // @ts-expect-error TS(2339): Property 'aggregation' does not exist on type 'Rea... Remove this comment to see the full error message
      aggregation,
      // @ts-expect-error TS(2339): Property 'compositeOperation' does not exist on ty... Remove this comment to see the full error message
      compositeOperation,
      // @ts-expect-error TS(2339): Property 'dotRadius' does not exist on type 'Reado... Remove this comment to see the full error message
      dotRadius,
      // @ts-expect-error TS(2339): Property 'lngLatAccessor' does not exist on type '... Remove this comment to see the full error message
      lngLatAccessor,
      // @ts-expect-error TS(2339): Property 'locations' does not exist on type 'Reado... Remove this comment to see the full error message
      locations,
      ...
      rgb,
      // @ts-expect-error TS(2339): Property 'zoom' does not exist on type 'Readonly<{... Remove this comment to see the full error message
      zoom,
    } = this.props;

    const radius = dotRadius;
    const clusterLabelMap: $TSFixMe = [];

    locations.forEach((location: $TSFixMe, i: $TSFixMe) => {
