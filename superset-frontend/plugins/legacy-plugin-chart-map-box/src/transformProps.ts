...
export default function transformProps(chartProps: $TSFixMe) {
  ...
  if (hasCustomMetric) {
    // @ts-expect-error TS(2339): Property 'initial' does not exist on type '{ maxZo... Remove this comment to see the full error message
    opts.initial = () => ({
      sum: 0,
      squaredSum: 0,
      min: Infinity,
    ...
    // @ts-expect-error TS(2339): Property 'map' does not exist on type '{ maxZoom: ... Remove this comment to see the full error message
    opts.map = (prop: $TSFixMe) => ({
      sum: prop.metric,
      squaredSum: prop.metric ** 2,
      min: prop.metric,
      max: prop.metric,
    });
    // @ts-expect-error TS(2339): Property 'reduce' does not exist on type '{ maxZoo... Remove this comment to see the full error message
    opts.reduce = (accu: $TSFixMe, prop: $TSFixMe) => {
  ...
  return {
    ...
    onViewportChange({ latitude, longitude, zoom }: $TSFixMe) {
