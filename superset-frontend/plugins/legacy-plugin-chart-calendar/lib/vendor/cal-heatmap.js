...
var CalHeatMap = function () {
                ...
                x: function (d) {
                    if (self.options.domain === 'month') {
                        if (self.options.colLimit > 0 || self.options.rowLimit > 0) {
                            return Math.floor((d.getHours() + (d.getDate() - 1) * 24) /
                        ...
                        if (self.options.colLimit > 0 || self.options.rowLimit > 0) {
                            return Math.floor((d.getHours() + self.getWeekDay(d) * 24) /
                                self._domainType.hour.row(d));
                        }
                        return (Math.floor(d.getHours() / self._domainType.hour.row(d)) +
                            self.getWeekDay(d) * 4);
    ...
    function _initCalendar() {
        ...
        self.Legend.redraw(self.graphDim.width -
    ...
    function w(d, outer) {
        var width = self.options.cellSize *
    ...
    function h(d, outer) {
        var height = self.options.cellSize * self._domainType[self.options.subDomain].row(d) +
            self.options.cellPadding *
...
CalHeatMap.prototype = {
    ...
    init: function (settings) {
        ...
        if (typeof options.itemNamespace !== 'string' ||
    ...
    maxDomainIsReached: function (datetimestamp) {
        'use strict';
        return (this.options.maxDate !== null &&
    ...
    minDomainIsReached: function (datetimestamp) {
        'use strict';
        return (this.options.minDate !== null &&
    ...
    dateIsLessThan: function (dateA, dateB) {
        ...
        return (normalizedMillis(dateA, this.options.subDomain) <
    ...
    getWeekNumber: function (d) {
        'use strict';
        var f = this.options.weekStartOnMonday === true
            ? d3.time.format('%W')
            : d3.time.format('%U');
        return f(d);
    },
    ...
    getDatas: function (source, startDate, endDate, callback, afterLoad, updateMode) {
        ...
        var _callback = function (error, data) {
            ...
            else if (self.options.dataType === 'csv' ||
    ...
    resize: function () {
        ...
        this.root
            ...
            .duration(options.animationDuration)
            .attr('width', function () {
            if (options.legendVerticalPosition === 'middle' ||
    ...
    setLegend: function () {
        ...
        if ((arguments.length > 0 && !arrayEquals(oldLegend, this.options.legend)) ||
            arguments.length >= 2) {
            this.Legend.buildColors();
            this.fill();
        }
        this.Legend.redraw(this.graphDim.width -
...
Legend.prototype.redraw = function (width) {
    ...
    function getLegendXPosition() {
        switch (options.legendHorizontalPosition) {
            case 'right':
                if (options.legendVerticalPosition === 'center' ||
    ...
    function getLegendYPosition() {
        if (options.legendVerticalPosition === 'bottom') {
            return (calendar.graphDim.height +
                options.legendMargin[0] -
...
Legend.prototype.buildColors = function () {
    ...
    else if (options.legendColors.hasOwnProperty('min') &&
