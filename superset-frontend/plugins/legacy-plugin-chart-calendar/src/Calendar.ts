...
function Calendar(element: $TSFixMe, props: $TSFixMe) {
  const {
    data,
    height,
    cellPadding = 3,
  ...
  const subDomainTextFormat = showValues
    ? (date: $TSFixMe, value: $TSFixMe) => valueFormatter(value)
    : null;
  ...
  Object.keys(metricsData).forEach(metric => {
    ...
    const colorScale = getSequentialSchemeRegistry()
      ...
      // @ts-expect-error TS(2345): Argument of type '[undefined, undefined] | [any, a... Remove this comment to see the full error message
