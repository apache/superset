...
// @ts-expect-error TS(7016): Could not find a declaration file for module 'd3-h... Remove this comment to see the full error message
...
function init(root: $TSFixMe) {
  const flat: $TSFixMe = [];
  const dy = 1 / (root.height + 1);
  let prev: $TSFixMe = null;
  root.each((n: $TSFixMe) => {
    n.y = dy * n.depth;
    n.dy = dy;
    if (n.parent) {
...
function getAncestors(d: $TSFixMe) {
  const ancestors = [d];
  let node = d;
  while (node.parent) {
...
function Icicle(element: $TSFixMe, props: $TSFixMe) {
  const {
    width,
    height,
    data,
  ...
  function hasDateNode(n: $TSFixMe) {
    return metrics.includes(n.data.name) && hasTime;
  }

  function getCategory(depth: $TSFixMe) {
    if (!depth) {
      return 'Metric';
    }
    if (hasTime && depth === 1) {
  ...
  function drawVis(i: $TSFixMe, dat: $TSFixMe) {
    const datum = dat[i];
    const w = width;
    ...
    if (partitionThreshold && partitionThreshold >= 0) {
      ...
      root.each((n: $TSFixMe) => {
        n.sum = n.children
          ? n.children.reduce((a: $TSFixMe, v: $TSFixMe) => a + v.weight, 0) ||
            1
          : 1;
        if (n.children) {
    ...
    root.eachAfter((n: $TSFixMe) => {
      n.sum = n.children
        ? n.children.reduce((a: $TSFixMe, v: $TSFixMe) => a + v.weight, 0) || 1
        : 1;
    });

    function positionAndPopulate(tip: $TSFixMe, d: $TSFixMe) {
      let t = '<table>';
      if (useRichTooltip) {
    ...
    // @ts-expect-error TS(7023): 'click' implicitly has return type 'any' because i... Remove this comment to see the full error message
    function click(d: $TSFixMe) {
      if (!d.children) {
        if (d.parent) {
      ...
      const t = g
        ...
        .attr(
          'transform',
          (nd: $TSFixMe) => `translate(${x(nd.y)},${y(nd.x)})`,
        );

      t.select('rect')
        .attr('width', d.dy * zoomX)
        .attr('height', (nd: $TSFixMe) => nd.dx * zoomY);

      t.select('text')
        .attr('transform', transform)
        .style('opacity', (nd: $TSFixMe) => (nd.dx * zoomY > 12 ? 1 : 0));
    ...
    g.append('svg:rect')
      ...
      .attr('height', (d: $TSFixMe) => d.dx * zoomY);

    g.append('svg:text')
      .attr('transform', transform)
      .attr('dy', '0.35em')
      .style('opacity', (d: $TSFixMe) => (d.dx * zoomY > 12 ? 1 : 0))
      .text((d: $TSFixMe) => {
        if (!d.disp) {
          return d.name;
        }
    ...
    // Apply color scheme
    g.selectAll('rect').style('fill', (d: $TSFixMe) => {
