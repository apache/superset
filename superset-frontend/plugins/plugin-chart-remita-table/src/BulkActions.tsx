...
interface BulkActionProps {
  ...
  onActionClick: (actionKey: string, selectedIds: any[]) => void;
}
export const BulkActions: React.FC<BulkActionProps> = ({
  ...
  const dropdownActions = actions.split.filter(action => {
  ...
  const buttonActions = actions.nonSplit.filter(action => {
    if (action.visibilityCondition === 'all') return true;
    if (action.visibilityCondition === 'selected') return true;
    if (action.visibilityCondition === 'unselected') return !hasSelection;
    return true;
    ...
    <>
      ...
      <span className="bulk-actions-container">
        <div className="btn-group">
          ...
          {/* Show dropdown but disable if needed */}
          {dropdownActions.length > 0 && (
            <Dropdown
              overlay={
                <Menu>
                  {dropdownActions.map(action => (
                    <Menu.Item
                      key={action.key}
                      onClick={() => onActionClick(action.key, Array.from(selectedRows))}
              ...
              trigger={['click']}
              disabled={!hasSelection && dropdownActions.every(a => a.boundToSelection)}
            >
              <button
          ...
          {/* Show all buttons but disable if needed */}
