...
function StickyWrap({
  ...
  useLayoutEffect(() => {
    ...
    // Schedule measurement in rAF to avoid doing it directly inside click handlers
    raf = (window as any).requestAnimationFrame ? requestAnimationFrame(measure) : (setTimeout(measure, 0) as any);
    return () => {
      try {
        if ((window as any).cancelAnimationFrame && typeof raf === 'number') cancelAnimationFrame(raf);
...
function useInstance<D extends object>(instance: TableInstance<D>) {
  ...
  const setStickyState = useCallback(
    ...
    // Keep callback stable across page/rows to avoid re-measuring on every click
