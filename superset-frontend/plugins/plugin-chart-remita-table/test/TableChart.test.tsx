...
describe('plugin-chart-table', () => {
  ...
  describe('TableChart', () => {
    ...
    it('render with bulk actions enabled', () => {
      ...
      // Check for header checkbox rendered (multi-select)
    ...
    it('render with split actions', () => {
      renderTableChart({
        ...
        overrideProps: {
          enable_bulk_actions: true,
          split_actions: new Set([
            { key: 'approve', label: 'Approve', type: 'dropdown', boundToSelection: false, visibilityCondition: 'all' },
            { key: 'reject', label: 'Reject', type: 'dropdown', boundToSelection: false, visibilityCondition: 'all' },
          ] as any),
          non_split_actions: new Set([
            { key: 'view', label: 'View', type: 'button', boundToSelection: false, visibilityCondition: 'all' },
          ] as any),
          show_split_buttons_in_slice_header: true,
        },
      });

      // Check non-split action button and bulk action dropdown trigger are present
    ...
    it('retains selection across navigation when enabled', () => {
      ...
      // Persist as Map entries [[id, row], ...]
      const entries = [
        ['row1', { id: 'row1', name: 'A' }],
        ['row2', { id: 'row2', name: 'B' }],
      ];
      localStorage.setItem('selectedRows_test-slice', JSON.stringify(entries));
      ...
      // Renders without clearing persisted selection
