...
const numberFormat = function (optsIn: $TSFixMe) {
  const defaults = {
...
const rd = /\d/;
const rz = /^0/;
const naturalSort = (as: $TSFixMe, bs: $TSFixMe) => {
  // nulls first
  if (bs !== null && as === null) {
    return -1;
  }
...
const sortAs = function (order: $TSFixMe) {
  const mapping = {};

  // sort lowercased keys similarly
  const lMapping = {};
  order.forEach((element: $TSFixMe, i: $TSFixMe) => {
  ...
  return function (a: $TSFixMe, b: $TSFixMe) {
...
const getSort = function (sorters: $TSFixMe, attr: $TSFixMe) {
  if (sorters) {
...
const fmtNonString = (formatter: $TSFixMe) => (x: $TSFixMe) =>
...
const baseAggregatorTemplates = {
  ...
  uniques(fn: $TSFixMe, formatter = usFmtInt) {
    // @ts-expect-error TS(7031): Binding element 'attr' implicitly has an 'any' typ... Remove this comment to see the full error message
  ...
  extremes(mode: $TSFixMe, formatter = usFmt) {
    // @ts-expect-error TS(7031): Binding element 'attr' implicitly has an 'any' typ... Remove this comment to see the full error message
    return function ([attr]) {
      return function (data: $TSFixMe) {
        return {
          val: null,
          sorter: getSort(
  ...
  quantile(q: $TSFixMe, formatter = usFmt) {
    // @ts-expect-error TS(7031): Binding element 'attr' implicitly has an 'any' typ... Remove this comment to see the full error message
    return function ([attr]) {
      return function () {
        return {
  ...
  sumOverSum(formatter = usFmt) {
    // @ts-expect-error TS(7031): Binding element 'num' implicitly has an 'any' type... Remove this comment to see the full error message
  ...
  fractionOf(wrapped: $TSFixMe, type = 'total', formatter = usFmtPct) {
    return (...x: $TSFixMe[]) =>
      function (data: $TSFixMe, rowKey: $TSFixMe, colKey: $TSFixMe) {
        return {
...
const extendedAggregatorTemplates = {
  countUnique(f: $TSFixMe) {
    return baseAggregatorTemplates.uniques((x: $TSFixMe) => x.length, f);
  },
  listUnique(s: $TSFixMe, f: $TSFixMe) {
  ...
  var(ddof: $TSFixMe, f: $TSFixMe) {
    return baseAggregatorTemplates.runningStat('var', ddof, f);
  },
  stdev(ddof: $TSFixMe, f: $TSFixMe) {
...
const zeroPad = (number: $TSFixMe) => `0${number}`.substr(-2, 2); // eslint-disable-line no-magic-numbers

const derivers = {
  bin(col: $TSFixMe, binWidth: $TSFixMe) {
    return (record: $TSFixMe) => record[col] - (record[col] % binWidth);
  },
  dateFormat(
    col: $TSFixMe,
    formatString: $TSFixMe,
    utcOutput = false,
    mthNames = mthNamesEn,
    dayNames = dayNamesEn,
  ) {
    const utc = utcOutput ? 'UTC' : '';
    return function (record: $TSFixMe) {
      const date = new Date(Date.parse(record[col]));
      if (Number.isNaN(date)) {
        return '';
      }
      return formatString.replace(/%(.)/g, function (m: $TSFixMe, p: $TSFixMe) {
        switch (p) {
          case 'y':
...
const flatKey = (attrVals: $TSFixMe) => attrVals.join(String.fromCharCode(0));
...
class PivotData {
  ...
  constructor(inputProps = {}, subtotals = {}) {
    ...
    // @ts-expect-error TS(2339): Property 'forEachRecord' does not exist on type 't... Remove this comment to see the full error message
    PivotData.forEachRecord(this.props.data, this.processRecord);
  }

  getFormattedAggregator(record: $TSFixMe, totalsKeys: $TSFixMe) {
  ...
  arrSort(attrs: $TSFixMe, partialOnTop: $TSFixMe, reverse = false) {
    const sortersArr = attrs.map((a: $TSFixMe) =>
  ...
  sortKeys() {
    if (!this.sorted) {
      this.sorted = true;
      const v = (r: $TSFixMe, c: $TSFixMe) => this.getAggregator(r, c).value();
      switch (this.props.rowOrder) {
        case 'key_z_to_a':
          ...
          break;
        case 'value_a_to_z':
          this.rowKeys.sort((a: $TSFixMe, b: $TSFixMe) =>
            naturalSort(v(a, []), v(b, [])),
          );
          break;
        case 'value_z_to_a':
          this.rowKeys.sort(
            (a: $TSFixMe, b: $TSFixMe) => -naturalSort(v(a, []), v(b, [])),
          );
          break;
        default:
          this.rowKeys.sort(
      ...
      switch (this.props.colOrder) {
        case 'key_z_to_a':
          ...
          break;
        case 'value_a_to_z':
          this.colKeys.sort((a: $TSFixMe, b: $TSFixMe) =>
            naturalSort(v([], a), v([], b)),
          );
          break;
        case 'value_z_to_a':
  ...
  processRecord(record: $TSFixMe) {
    // this code is called in a tight loop
    const colKey: $TSFixMe = [];
    const rowKey: $TSFixMe = [];
    this.props.cols.forEach((col: $TSFixMe) => {
      colKey.push(col in record ? record[col] : 'null');
    });
    this.props.rows.forEach((row: $TSFixMe) => {
  ...
  getAggregator(rowKey: $TSFixMe, colKey: $TSFixMe) {
    let agg;
...
// @ts-expect-error TS(2339): Property 'forEachRecord' does not exist on type 't... Remove this comment to see the full error message
PivotData.forEachRecord = function (input: $TSFixMe, processRecord: $TSFixMe) {
