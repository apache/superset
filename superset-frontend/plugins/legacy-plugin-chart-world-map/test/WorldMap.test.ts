...
test('restores original fill color on mouseout for country with data', () => {
  ...
  // Verify stroke is restored to border defaults
  expect(mockD3Selection.style).toHaveBeenCalledWith('stroke', '#333');
  expect(mockD3Selection.style).toHaveBeenCalledWith('stroke-width', '1');
});
...
test('disables highlightOnHover to prevent moveToFront DOM detachment', () => {
  WorldMap(container, baseProps);
...
test('applies highlight fill and stroke on mouseover', () => {
  const mockElement = document.createElement('path');
  mockElement.setAttribute('class', 'datamaps-subunit USA');
  container.appendChild(mockElement);
  ...
  // Verify highlight styling applied
  expect(mockD3Selection.style).toHaveBeenCalledWith('fill', '#61B0B7');
  expect(mockD3Selection.style).toHaveBeenCalledWith('stroke', '#000');
  expect(mockD3Selection.style).toHaveBeenCalledWith('stroke-width', '1');
});
