...
function processScatterData(
  ...
  return spatialFeatures.map((feature, index) => {
    ...
    if (jsColumns?.length) {
      jsColumns.forEach(col => {
        if (record[col] !== undefined) {
          scatterPoint.extraProps![col] = record[col];
        }
      });
    }

    // Add radius from metric
    if (radiusMetricLabel && record[radiusMetricLabel] != null) {
      const radiusValue = parseFloat(record[radiusMetricLabel]);
    ...
    // Add category for color mapping
    if (categoryColumn && record[categoryColumn] != null) {
      scatterPoint.cat_color = record[categoryColumn];
    }
...
export default function transformProps(chartProps: ChartProps) {
  ...
  // Process the query data to extract scatter features
