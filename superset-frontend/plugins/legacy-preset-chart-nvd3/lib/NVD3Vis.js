...
import { CategoricalColorNamespace, evalExpression, getNumberFormatter, getTimeFormatter, isDefined, NumberFormats, smartDateVerboseFormatter, t, } from '@superset-ui/core';
...
function nvd3Vis(element, props) {
    ...
    const drawGraph = function drawGraph() {
        ...
        const xLabelRotation = (xTicksLayout === 'auto' && isVizTypes(['column', 'dist_bar'])) ||
            xTicksLayout === '45°'
            ? 45
            : 0;
        ...
        const canShowBrush = isTruthy(showBrush) ||
            (showBrush === 'auto' &&
        ...
        if (chart.xAxis && chart.xAxis.tickFormat) {
            ...
            if (isXAxisString) {
                chart.xAxis.tickFormat(d => d.length > MAX_NO_CHARACTERS_IN_LABEL
        ...
        if (chart.yAxis !== undefined || chart.yAxis2 !== undefined) {
            ...
            if (isTimeSeries && annotationData && activeAnnotationLayers.length > 0) {
                ...
                const timeSeriesAnnotations = activeAnnotationLayers
                    ...
                    .reduce((bushel, a) => bushel.concat((annotationData[a.name] || []).map(series => {
                    if (!series) {
                        return {};
                    }
            ...
            if (isTimeSeries && activeAnnotationLayers.length > 0) {
                ...
                if (annotationData) {
                    ...
                    activeAnnotationLayers
                        .filter(x => x.annotationType === ANNOTATION_TYPES.EVENT &&
                        annotationData &&
                    ...
                    activeAnnotationLayers
                        .filter(x => x.annotationType === ANNOTATION_TYPES.INTERVAL &&
                        annotationData &&
                        annotationData[x.name])
                        .forEach((config, index) => {
                        const e = applyNativeColumns(config);
...
//# sourceMappingURL=NVD3Vis.js.map