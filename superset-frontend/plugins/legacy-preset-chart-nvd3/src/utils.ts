...
export function cleanColorInput(value: $TSFixMe) {
 ...
 * @param {*} format
 */
export function getTimeOrNumberFormatter(format: $TSFixMe) {
  return format === SMART_DATE_ID
    ? getTimeFormatter(SMART_DATE_ID)
    : getNumberFormatter(format);
}

export function drawBarValues(
  svg: $TSFixMe,
  data: $TSFixMe,
  stacked: $TSFixMe,
  axisFormat: $TSFixMe,
  ...
  const countSeriesDisplayed = data.filter((d: $TSFixMe) => !d.disabled).length;
  const totalStackedValues =
    stacked && data.length !== 0
      ? data[0].values.map((bar: $TSFixMe, iBar: $TSFixMe) => {
          const bars = data
            .filter((series: $TSFixMe) => !series.disabled)
            .map((series: $TSFixMe) => series.values[iBar]);

          return d3.sum(bars, (d: $TSFixMe) => d.y);
        })
      : [];
  ...
  setTimeout(() => {
    ...
    svg
      ...
      .filter(
        (d: $TSFixMe, i: $TSFixMe) =>
          !stacked || i === countSeriesDisplayed - 1,
      )
      .selectAll('rect')
      .each(function each(this: $TSFixMe, d: $TSFixMe, index: $TSFixMe) {
...
function getFormattedKey(seriesKey: $TSFixMe, shouldDompurify: $TSFixMe) {
  if (seriesKey === '<NULL>') {
...
export function generateCompareTooltipContent(
  d: $TSFixMe,
  valueFormatter: $TSFixMe,
) {
  let tooltip = '';
  tooltip +=
    ...
    '</td></tr></thead><tbody>';
  d.series.sort((a: $TSFixMe, b: $TSFixMe) => (a.value >= b.value ? -1 : 1));
  d.series.forEach((series: $TSFixMe) => {
...
export function generateMultiLineTooltipContent(
  d: $TSFixMe,
  xFormatter: $TSFixMe,
  yFormatters: $TSFixMe,
) {
  ...
  d.series.forEach((series: $TSFixMe, i: $TSFixMe) => {
...
export function generateTimePivotTooltip(
  d: $TSFixMe,
  xFormatter: $TSFixMe,
  yFormatter: $TSFixMe,
) {
...
function getLabel(stringOrObjectWithLabel: $TSFixMe) {
  return stringOrObjectWithLabel.label || stringOrObjectWithLabel;
}

function createHTMLRow(col1: $TSFixMe, col2: $TSFixMe) {
...
export function hideTooltips(shouldRemove: $TSFixMe) {
...
export function generateTooltipClassName(uuid: $TSFixMe) {
  return `tooltip-${uuid}`;
}

export function removeTooltip(uuid: $TSFixMe) {
...
export function wrapTooltip(chart: $TSFixMe) {
  const tooltipLayer =
  ...
  tooltipLayer.tooltip.contentGenerator((d: $TSFixMe) => {
    let tooltip = `<div>`;
...
export function tipFactory(layer: $TSFixMe) {
  return d3tip()
    ...
    .html((d: $TSFixMe) => {
      ...
      const body = Array.isArray(layer.descriptionColumns)
        ? layer.descriptionColumns.map((c: $TSFixMe) => d[c])
        : Object.values(d);
...
export function getMaxLabelSize(svg: $TSFixMe, axisClass: $TSFixMe) {
  // axis class = .nv-y2  // second y axis on dual line chart
  // axis class = .nv-x  // x axis on time series line chart
  const tickTexts = svg.selectAll(`.${axisClass} g.tick text`);
  if (tickTexts.length > 0) {
    const lengths = tickTexts[0].map((text: $TSFixMe) =>
...
export function formatLabel(input: $TSFixMe, verboseMap = {}) {
  ...
  const verboseLookup = (s: $TSFixMe) => verboseMap[s] || s;
...
export function stringifyTimeRange(extent: $TSFixMe) {
  if (extent.some((d: $TSFixMe) => d.toISOString === undefined)) {
    return null;
  }

  return extent.map((d: $TSFixMe) => d.toISOString().slice(0, -1)).join(' : ');
}

export function setAxisShowMaxMin(axis: $TSFixMe, showminmax: $TSFixMe) {
  if (axis && axis.showMaxMin && showminmax !== undefined) {
    axis.showMaxMin(showminmax);
  }
}

export function computeYDomain(data: $TSFixMe) {
  if (Array.isArray(data) && data.length > 0 && Array.isArray(data[0].values)) {
    const extents = data
      .filter(d => !d.disabled)
      .map(row => d3.extent(row.values, (v: $TSFixMe) => v.y));
    // @ts-expect-error TS(7031): Binding element 'min' implicitly has an 'any' type... Remove this comment to see the full error message
    const minOfMin = d3.min(extents, ([min]) => min);
    // @ts-expect-error TS(7031): Binding element 'max' implicitly has an 'any' type... Remove this comment to see the full error message
