...
nv.utils.noData = function noData(chart: $TSFixMe, container: $TSFixMe) {
  ...
  noDataText
    .enter()
    .append('text')
    .attr('class', (d: $TSFixMe) => `nvd3 nv-noData ${d.class}`)
    .attr('dy', (d: $TSFixMe) => d.dy)
    .style('text-anchor', 'middle');

  noDataText
    .attr('x', x)
    .attr('y', y)
    .text((d: $TSFixMe) => d.text);
};
  ...
  data: PropTypes.oneOfType([
    PropTypes.arrayOf(
      PropTypes.oneOfType([
        ...
        // dist-bar
        PropTypes.shape({
          // @ts-expect-error TS(2345): Argument of type '{ key: PropTypes.Requireable<str... Remove this comment to see the full error message
          key: PropTypes.string,
          values: PropTypes.arrayOf(categoryAndValueXYType),
        }),
        // area, line, compare, bar
        PropTypes.shape({
          // @ts-expect-error TS(2345): Argument of type '{ key: PropTypes.Requireable<(st... Remove this comment to see the full error message
          key: PropTypes.arrayOf(PropTypes.string),
          values: PropTypes.arrayOf(numericXYType),
        }),
        // dual-line
        PropTypes.shape({
          // @ts-expect-error TS(2345): Argument of type '{ classed: PropTypes.Requireable... Remove this comment to see the full error message
        ...
        // box-plot
        PropTypes.shape({
          // @ts-expect-error TS(2345): Argument of type '{ label: PropTypes.Requireable<s... Remove this comment to see the full error message
...
function nvd3Vis(element: $TSFixMe, props: $TSFixMe) {
  const {
    data,
    width: maxWidth,
    height: maxHeight,
  ...
  container.innerHTML = '';
  const activeAnnotationLayers = annotationLayers.filter(
    (layer: $TSFixMe) => layer.show,
  );

  // Search for the chart id in a parent div from the nvd3 chart
  let chartContainer = container;
  let chartId: $TSFixMe = null;
  ...
  function isVizTypes(types: $TSFixMe) {
  ...
  const drawGraph = function drawGraph() {
    ...
    switch (vizType) {
      ...
      case VizType.Pie:
        ...
        if (['key', 'value', 'percent'].includes(pieLabelType)) {
          ...
          chart.labelType(
            (d: $TSFixMe) => `${d.data.x}: ${numberFormatter(d.data.y)}`,
          );
        } else {
          // pieLabelType in ['key_percent', 'key_value_percent']
          const total = d3.sum(data, (d: $TSFixMe) => d.y);
          const percentFormatter = getNumberFormatter(
            NumberFormats.PERCENT_2_POINT,
          );
          if (pieLabelType === 'key_percent') {
            chart.tooltip.valueFormatter((d: $TSFixMe) => percentFormatter(d));
            chart.labelType(
              (d: $TSFixMe) =>
                `${d.data.x}: ${percentFormatter(d.data.y / total)}`,
            );
          } else {
            // pieLabelType === 'key_value_percent'
            chart.tooltip.valueFormatter(
              (d: $TSFixMe) =>
                `${numberFormatter(d)} (${percentFormatter(d / total)})`,
            );
            chart.labelType(
              (d: $TSFixMe) =>
      ...
      case VizType.LegacyBubble:
        ...
        chart.tooltip.contentGenerator((d: $TSFixMe) =>
          generateBubbleTooltipContent({
            point: d.point,
            entity,
            xField,
        ...
        chart.pointDomain([
          0,
          d3.max(data, (d: $TSFixMe) =>
            d3.max(d.values, (v: $TSFixMe) => v.size),
          ),
        ]);
        break;
    ...
    if (canShowBrush && onBrushEnd !== NOOP) {
      if (chart.focus) {
        chart.focus.dispatch.on('brush', (event: $TSFixMe) => {
    ...
    if (chart.xAxis && chart.xAxis.tickFormat) {
      ...
      if (isXAxisString) {
        chart.xAxis.tickFormat((d: $TSFixMe) =>
    ...
    if (vizType === VizType.TimePivot) {
      ...
      chart.useInteractiveGuideline(true);
      chart.interactiveLayer.tooltip.contentGenerator((d: $TSFixMe) =>
      ...
      chart.color(
        (d: $TSFixMe) =>
          d.color || colorFn(cleanColorInput(d[colorKey]), sliceId),
      );
    }
    ...
    if (yIsLogScale) {
      chart.yAxis.tickFormat((d: $TSFixMe) =>
    ...
    if (chart.yAxis !== undefined || chart.yAxis2 !== undefined) {
        ...
        const timeSeriesAnnotations = activeAnnotationLayers
          .filter(
            (layer: $TSFixMe) =>
              ...
              layer.annotationType === ANNOTATION_TYPES.TIME_SERIES,
          )
          .reduce(
            (bushel: $TSFixMe, a: $TSFixMe) =>
              bushel.concat(
                (annotationData[a.name] || []).map((series: $TSFixMe) => {
                  if (!series) {
                    return {};
                  }
      ...
      if (isTimeSeries && activeAnnotationLayers.length > 0) {
        ...
        const formulas = activeAnnotationLayers.filter(
          ...
          (a: $TSFixMe) => a.annotationType === ANNOTATION_TYPES.FORMULA,
        );
        ...
        if (formulas.length > 0) {
          const xValues: $TSFixMe = [];
          let period = Math.min(
            ...data.map((d: $TSFixMe) =>
              Math.min(
                ...d.values
                  .slice(1)
                  .map((v: $TSFixMe, i: $TSFixMe) => v.x - d.values[i].x),
              ),
            ),
          );
          ...
          const formulaData = formulas.map((fo: $TSFixMe) => {
        ...
        if (annotationData) {
          ...
          activeAnnotationLayers
            ...
            .forEach((config: $TSFixMe, index: $TSFixMe) => {
              ...
              if (records.length > 0) {
                annotations
                  ...
                  .append('line')
                  .attr({
                    x1: (d: $TSFixMe) => xScale(new Date(d[e.timeColumn])),
                    y1: 0,
                    x2: (d: $TSFixMe) => xScale(new Date(d[e.timeColumn])),
                    y2: annotationHeight,
                  })
              ...
              if (chart.focus) {
                chart.focus.dispatch.on('onBrush.interval-annotation', () => {
                  annotations
                    ...
                    .data(records)
                    .attr({
                      x: (d: $TSFixMe) => xScale(new Date(d[e.timeColumn])),
                      width: (d: $TSFixMe) => {
