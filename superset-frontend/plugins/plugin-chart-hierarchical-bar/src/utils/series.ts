...
export function getLegendProps(
  ...
  const legend: LegendComponentOption | LegendComponentOption[] = {
    ...
    selectorLabel: {
      fontFamily:
        (theme as any)?.typography?.families?.sansSerif ?? 'sans-serif',
      fontSize: (theme as any)?.typography?.sizes?.s ?? 11,
      color: theme?.colors?.grayscale?.base ?? '#000',
      borderColor: theme?.colors?.grayscale?.base ?? '#000',
    },
  };
...
export function getChartPadding(
  ...
  // Safely resolve a numeric legend margin
  const resolvedMargin = !show
    ? 0
    : typeof margin === 'number'
      ? margin
      : (defaultLegendPadding[orientation] ?? 20); // fallback in case of misalignment
  ...
  if (isHorizontal) {
    return {
      left:
        left + (orientation === LegendOrientation.Bottom ? resolvedMargin : 0),
      right:
        right + (orientation === LegendOrientation.Right ? resolvedMargin : 0),
      top: top + (orientation === LegendOrientation.Top ? resolvedMargin : 0),
      bottom:
        bottom + (orientation === LegendOrientation.Left ? resolvedMargin : 0),
    };
  }

  return {
    left: left + (orientation === LegendOrientation.Left ? resolvedMargin : 0),
    right:
      right + (orientation === LegendOrientation.Right ? resolvedMargin : 0),
    top: top + (orientation === LegendOrientation.Top ? resolvedMargin : 0),
    bottom:
      bottom + (orientation === LegendOrientation.Bottom ? resolvedMargin : 0),
  };
}
...
// Add this function to the bottom of src/utils.ts

export function getContributionLabel(metricLabel: string): string {
  // This is often used for calculating percentage metrics
  return `% of ${metricLabel}`;
}
