...
function computeGraph(links) {
    ...
    const nodes = Array.from(links.reduce((set, { source, target }) => set.add(source).add(target), new Set())).map(id => ({ id, name: id }));
    return {
        nodes,
...
function SankeyLoop(element, props) {
    ...
    const diagram = sankeyDiagram()
        .nodeTitle(d => d.name)
        .linkTitle(({ source: { name: sName, value: sValue }, target: { name: tName }, value, }) => `${sName} → ${tName}: ${countFormat(value)} (${percentFormat(value / sValue)})`)
...
//# sourceMappingURL=SankeyLoop.js.map