...
class HorizonChart extends PureComponent {
  render() {
    const {
      ...
      data,
      // @ts-expect-error TS(2339): Property 'seriesHeight' does not exist on type 'Re... Remove this comment to see the full error message
      seriesHeight,
    ...
    if (colorScale === 'overall') {
      const allValues = data.reduce(
        (acc: $TSFixMe, current: $TSFixMe) => acc.concat(current.values),
        [],
      );
