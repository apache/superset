...
// The deck.gl Accessor type allows a plain color constant in addition to a
// per-datum function, so TS can't infer these are callable from the layer
// props alone; the test data always configures a function accessor.
type ColorAccessor = (datum: unknown) => number[];
