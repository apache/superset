...
export default function transformProps(
      ...
      if (compareIndex < sortedData.length) {
        ...
        if (bigNumber !== null && compareValue !== null) {
          percentChange = compareValue
            ...
            : 0;
          formattedSubheader = `${formatPercentChange(percentChange)}${compareSuffix ? ` ${compareSuffix}` : ''}`;
        }
      }
    } else {
      formattedSubheader = '';
  ...
  return {
    ...
    headerFormatter,
    formatTime,
    formData: formData as BigNumberWithTrendlineFormData,
    headerFontSize,
    subheaderFontSize,
    mainColor,
    showTimestamp,
