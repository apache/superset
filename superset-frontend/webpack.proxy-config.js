...
function toDevHTML(originalHtml) {
  ...
  if (manifest) {
    ...
    html = html.replace(
      /<!-- Bundle (css|js) (.*?) START -->[\s\S]*?<!-- Bundle \1 \2 \3 END -->/gi,
      (match, assetType, assetsPrefix, bundleName) => {
        if (bundleName in manifest.entrypoints) {
          return `<!-- DEV bundle: ${bundleName} ${assetType} START -->\n  ${(
            ...
            .map(chunkFilePath =>
              assetType === 'css'
                ? `<link rel="stylesheet" type="text/css" href="${assetsPrefix}${chunkFilePath}" />`
                : `<script src="${assetsPrefix}${chunkFilePath}"></script>`,
            )
            .join(
              '\n  ',
