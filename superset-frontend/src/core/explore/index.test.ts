...
// requestChartDataResolved already resolves an async 202 job envelope into
// its final result array (see its own coverage in chartActions.test.ts), so
// this module only needs to mock that resolved boundary.
...
test('getQuery builds form data from the current controls, not stale form_data', async () => {
  ...
  mockRequestChartDataResolved.mockResolvedValue([{ query: 'SELECT 1' }]);
...
test('getChartData requests a full result and returns columns/rows', async () => {
  activateExplore();
  mockRequestChartDataResolved.mockResolvedValue([
    { colnames: ['region', 'sales'], data: [{ region: 'US', sales: 100 }] },
  ]);
