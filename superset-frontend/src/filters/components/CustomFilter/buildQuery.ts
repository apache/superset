...
import { DEFAULT_FORM_DATA, PluginFilterCustomFilterQueryFormData } from './types';

const buildQuery: BuildQuery<PluginFilterCustomFilterQueryFormData> = (
  formData: PluginFilterCustomFilterQueryFormData,
  _options,
) => {
  const { search, coltypeMap } = _options?.ownState || {};
    ...
    const query: QueryObject[] = [
      {
        ...
        orderby: sortAscending !== undefined
          ? columns.map(column => [column, !!sortAscending])
          : [],
      },
    ];
    return query;
