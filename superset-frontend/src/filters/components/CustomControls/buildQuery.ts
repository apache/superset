...
import { PluginFilterCustomControlsQueryFormData } from './types';

const buildQuery: BuildQuery<PluginFilterCustomControlsQueryFormData> = (
  formData: PluginFilterCustomControlsQueryFormData,
) =>
  buildQueryContext(formData, baseQueryObject => {
    ...
    const col = Array.isArray(rawCol)
      ...
      : typeof rawCol === 'object' && rawCol !== null
        ? (rawCol as { label?: string; column_name?: string; sqlExpression?: string }).column_name ||
