...
import { getExploreLongUrl } from 'src/explore/exploreUtils';
import ChartContainer from 'src/Superstructure/chart/ChartContainer';
import {
...
import { slicePropShape, chartPropShape } from 'src/dashboard/util/propShapes';

import { isFilterBox } from 'src/dashboard/util/activeDashboardFilters';
import getFilterValuesByFilterId from 'src/dashboard/util/getFilterValuesByFilterId';
...
export default class Chart extends React.Component {
  ...
  async exportCSV(isFullCSV = false) {
    this.props.logEvent(LOG_ACTIONS_EXPORT_CSV_DASHBOARD_CHART, {
      slice_id: this.props.slice.slice_id,
      is_cached: this.props.isCached,
    });
    const csvData = await exportChart({
      formData: isFullCSV
