...
import { useOnSelectChange } from './useOnSelectChange';
...
test('Should call constructos', () => {
  ...
  const { result } = renderHook(() => useOnSelectChange(constructors));

  act(() => {
    result.current.current({ dbId: 12, schema: 'current-schema' });
  });
