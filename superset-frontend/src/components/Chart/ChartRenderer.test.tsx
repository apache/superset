...
test('should render SuperChart', () => {
  const { getByTestId } = render(
    // @ts-expect-error TS(2322): Type '{ chartIsStale: boolean; chartId: number; da... Remove this comment to see the full error message
...
test('should use latestQueryFormData instead of formData when chartIsStale is true', () => {
  const { getByTestId } = render(
    // @ts-expect-error TS(2322): Type '{ chartIsStale: true; chartId: number; datas... Remove this comment to see the full error message
...
test('should not render chart context menu if the context menu is suppressed for given viz plugin', () => {
  ...
  const { queryByTestId } = render(
    // @ts-expect-error TS(2322): Type '{ vizType: string; chartId: number; datasour... Remove this comment to see the full error message
