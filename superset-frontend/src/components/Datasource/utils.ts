...
export function recurseReactClone(
  children: $TSFixMe,
  type: $TSFixMe,
  propExtender: $TSFixMe,
) {
  /**
...
export function updateColumns(
  prevCols: $TSFixMe,
  newCols: $TSFixMe,
  addSuccessToast: $TSFixMe,
) {
  // cols: Array<{column_name: string; is_dttm: boolean; type: string;}>
  const databaseColumnNames = newCols.map((col: $TSFixMe) => col.column_name);
  const currentCols = prevCols.reduce((agg: $TSFixMe, col: $TSFixMe) => {
  ...
  const columnChanges = {
    ...
    removed: prevCols
      ...
      .map((col: $TSFixMe) => col.column_name),
    finalColumns: [],
  };
  newCols.forEach((col: $TSFixMe) => {
  ...
  prevCols
    .filter((col: $TSFixMe) => col.expression)
    .forEach((col: $TSFixMe) => {
      // @ts-expect-error TS(2345): Argument of type 'any' is not assignable to parame... Remove this comment to see the full error message
...
export async function fetchSyncedColumns(datasource: $TSFixMe) {
  const params = {
