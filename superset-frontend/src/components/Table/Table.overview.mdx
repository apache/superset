...
To ensure consitency for column sorting and avoid redundant definitions for common column sorters reusable sort functions are provided
...
When using a provided sorter function the pattern is to wrap the call to the sorter with an inline function, then specify the specici attribute, representing a column,
of the data objct for that row as the first argument of the sorter function.
