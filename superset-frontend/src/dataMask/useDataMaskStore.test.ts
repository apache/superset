...
// Ported from the retired dataMask reducer tests: the required-filter default
// restore and its explicit-clear exception now live in fillNativeFilters here.
const REQUIRED_SELECT_ID = 'NATIVE_FILTER-region';

const requiredSelectMetadata = () => ({
  native_filter_configuration: [
    {
      id: REQUIRED_SELECT_ID,
      name: 'region',
...
test.each<[null | [], boolean]>([
  ...
  'preserves an explicit required select clear (value: %j, UI mask: %j) from a permalink',
  (value, useUiMask) => {
    // The mask apply_dashboard_filters produces for values: [].
    ...
    if (useUiMask) {
      loaded[REQUIRED_SELECT_ID].extraFormData = getSelectExtraFormData(
        'region',
        [],
        true,
      );
    ...
    const result = buildHydratedDataMask(requiredSelectMetadata(), {}, loaded);

    expect(result[REQUIRED_SELECT_ID].filterState).toEqual({ value });
    expect(result[REQUIRED_SELECT_ID].extraFormData).toEqual(
      loaded[REQUIRED_SELECT_ID].extraFormData,
    );
  },
);

test('still restores a required select default for an incomplete permalink', () => {
  const loaded = {
    [REQUIRED_SELECT_ID]: {
      id: REQUIRED_SELECT_ID,
  ...
  expect(result[REQUIRED_SELECT_ID].filterState).toEqual({ value: ['APAC'] });
  expect(result[REQUIRED_SELECT_ID].extraFormData).toEqual({
    filters: [{ col: 'region', op: 'IN', val: ['APAC'] }],
  });
});
...
test.each([null, 'invalid', {}, 1, [null], [undefined], [1], [{}]])(
  'restores a required select default for malformed adhoc filters (%j)',
  adhocFilters => {
    const loaded = {
      [REQUIRED_SELECT_ID]: {
        id: REQUIRED_SELECT_ID,
        ...
        extraFormData: { adhoc_filters: adhocFilters },
      },
    } as unknown as DataMaskStateWithId;
