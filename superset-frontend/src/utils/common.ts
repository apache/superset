...
export function storeQuery(query: $TSFixMe) {
...
export function optionLabel(opt: $TSFixMe) {
  if (opt === null) {
    return NULL_STRING;
  }
  if (opt === '') {
...
export function optionValue(opt: $TSFixMe) {
  if (opt === null) {
    return NULL_STRING;
  }
  return opt;
...
export function optionFromValue(opt: $TSFixMe) {
  // From a list of options, handles special values & labels
  return { value: optionValue(opt), label: optionLabel(opt) };
}

function getColumnName(column: $TSFixMe) {
...
export function applyFormattingToTabularData(
  ...
  return data.map((row: $TSFixMe) => ({
    ...
    ...timeFormattedColumns.reduce((acc: $TSFixMe, colName: $TSFixMe) => {
