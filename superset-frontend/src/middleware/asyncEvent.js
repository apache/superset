...
const initAsyncEvents = (options) => {
  ...
  const middleware = store => next => {
    ...
    const processEvents = async () => {
            ...
            switch(asyncEvent['status']) {
              case JOB_STATUS.DONE:
                try {
                  ...
                  store.dispatch(successAction(componentId, componentData));
                } catch(errorResponse) {
                  getClientErrorObject(errorResponse).then(parsedResponse => {
                    store.dispatch(errorAction(componentId, parsedResponse));
                  });
                }
                break;
              case JOB_STATUS.ERROR:
                const parsedEvent = parseErrorJson(asyncEvent);
                store.dispatch(errorAction(componentId, parsedEvent));
                break;
            }
      ...
      setTimeout(processEvents, POLLING_DELAY);
    };
