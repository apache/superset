...
function getSidebarWidths(key: $TSFixMe) {
  // @ts-expect-error TS(7053): Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
  return getItem(key, defaultSidebarsWidth[key]);
}

function setSidebarWidths(key: $TSFixMe, dimension: $TSFixMe) {
  const newDimension = Number(getSidebarWidths(key)) + dimension.width;
  setItem(key, newDimension);
}

function ExploreViewContainer(props: $TSFixMe) {
  ...
  return (
    <ExploreContainer>
      <ConnectedExploreChartHeader
        ...
        // @ts-expect-error TS(2322): Type 'boolean | Element' is not assignable to type... Remove this comment to see the full error message
      ...
      <ExplorePanelContainer id="explore-container">
        ...
        >
          ...
          <DataSourcePanel
            ...
            actions={props.actions}
            width={width}
            // @ts-expect-error TS(2322): Type '{ formData: any; datasource: any; controls: ... Remove this comment to see the full error message
            user={props.user}
          />
        </Resizable>
        {isCollapsed ? (
...
const retainQueryModeRequirements = (hiddenFormData: $TSFixMe) =>
  Object.keys(hiddenFormData ?? {}).filter(
    key => !QUERY_MODE_REQUISITES.has(key),
  );

function patchBigNumberTotalFormData(form_data: $TSFixMe, slice: $TSFixMe) {
  if (
...
function mapStateToProps(state: $TSFixMe) {
  ...
  // @ts-expect-error TS(2339): Property 'slice_id' does not exist on type 'Omit<S... Remove this comment to see the full error message
  const slice_id = form_data.slice_id ?? slice?.slice_id ?? 0; // 0 - unsaved chart
  // @ts-expect-error TS(2339): Property 'extra_form_data' does not exist on type ... Remove this comment to see the full error message
  ...
  if (
    // @ts-expect-error TS(2339): Property 'viz_type' does not exist on type 'Omit<S... Remove this comment to see the full error message
    form_data.viz_type === 'big_number_total' &&
    slice?.form_data?.subheader &&
