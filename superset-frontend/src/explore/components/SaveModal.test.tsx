...
const setup = (props = defaultProps, store = initialStore) =>
  // @ts-expect-error TS(2741): Property 'dashboardId' is missing in type '{ addDa... Remove this comment to see the full error message
...
test('updates slice name and selected dashboard', async () => {
  ...
  const { getByRole, getByTestId } = setup(
    {
      actions: {
        // @ts-expect-error TS(2322): Type '{ saveDataset: jest.Mock<any, any, any>; cre... Remove this comment to see the full error message
        saveDataset,
        createDashboard,
        saveSliceFailed,
        setFormData,
...
test('make sure slice_id in the URLSearchParams before the redirect', () => {
  ...
  // @ts-expect-error TS(2345): Argument of type '{ slice: { slice_id: number; sli... Remove this comment to see the full error message
