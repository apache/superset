...
describe('Custom Frame Integration Tests', () => {
  describe('Complete User Flow Simulation', () => {
    it('should handle complete user interaction flow', () => {
      ...
      // Step 2: User sees dropdown options (should not include relative)
      ...
      expect(sinceOptions).toEqual(['specific', 'now', 'today']);
      expect(untilOptions).toEqual(['specific', 'now', 'today']);
      
      // Step 3: User changes start date to 'now'
      currentRange = { ...currentRange, sinceMode: 'now' as DateTimeModeType };
      expect(currentRange.sinceMode).toBe('now');
      
      // Step 4: User changes back to 'specific' - should still work
      currentRange = { ...currentRange, sinceMode: 'specific' as DateTimeModeType };
      expect(currentRange.sinceMode).toBe('specific');
      expect(currentRange.sinceDatetime).toBeTruthy();
      
      // Step 5: Encode/decode cycle (simulates form submission/reload)
    ...
    it('should maintain data integrity through multiple operations', () => {
      const testOperations = [
        (range: any) => ({ ...range, sinceMode: 'now' as const }),
        (range: any) => ({ ...range, untilMode: 'today' as const }),
        (range: any) => ({ ...range, sinceMode: 'specific' as const }),
        (range: any) => ({ ...range, untilMode: 'specific' as const }),
      ];
      
      let currentRange = customTimeRangeDecode('').customRange;
      
      testOperations.forEach((operation, index) => {
        currentRange = operation(currentRange);
        
        // After each operation, encoding/decoding should work
        const encoded = customTimeRangeEncode(currentRange);
        const decoded = customTimeRangeDecode(encoded);
  ...
  describe('Backward Compatibility Verification', () => {
    it('should maintain compatibility with existing valid ranges', () => {
      ...
      existingValidRanges.forEach(range => {
        ...
        // Should maintain original semantics where possible
        if (range.includes('now')) {
          expect(result.customRange.sinceMode === 'now' || result.customRange.untilMode === 'now').toBe(true);
        }
        if (range.includes('today')) {
          expect(result.customRange.sinceMode === 'today' || result.customRange.untilMode === 'today').toBe(true);
        }
      });
    });
