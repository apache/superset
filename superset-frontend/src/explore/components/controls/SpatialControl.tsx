/**
 * Licensed to the Apache Software Foundation (ASF) under one
 * or more contributor license agreements.  See the NOTICE file
 * distributed with this work for additional information
 * regarding copyright ownership.  The ASF licenses this file
 * to you under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance
 * with the License.  You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, either express or implied.  See the License for the
 * specific language governing permissions and limitations
 * under the License.
 */
import { Component } from 'react';
import PropTypes from 'prop-types';
import {
  Row,
  Col,
  Checkbox,
  Label,
  Popover,
} from '@superset-ui/core/components';
import { t } from '@superset-ui/core';

import PopoverSection from '@superset-ui/core/components/PopoverSection';
import ControlHeader from '../ControlHeader';
import SelectControl from './SelectControl';

const spatialTypes = {
  latlong: 'latlong',
  delimited: 'delimited',
  geohash: 'geohash',
};

const propTypes = {
  onChange: PropTypes.func,
  value: PropTypes.object,
  animation: PropTypes.bool,
  choices: PropTypes.array,
};

const defaultProps = {
  onChange: () => {},
  animation: true,
  choices: [],
};

export default class SpatialControl extends Component {
  constructor(props: $TSFixMe) {
    super(props);
    const v = props.value || {};
    let defaultCol;
    if (props.choices.length > 0) {
      defaultCol = props.choices[0][0];
    }
    this.state = {
      type: v.type || spatialTypes.latlong,
      delimiter: v.delimiter || ',',
      latCol: v.latCol || defaultCol,
      lonCol: v.lonCol || defaultCol,
      lonlatCol: v.lonlatCol || defaultCol,
      reverseCheckbox: v.reverseCheckbox || false,
      geohashCol: v.geohashCol || defaultCol,
      value: null,
      errors: [],
    };
    this.toggleCheckbox = this.toggleCheckbox.bind(this);
    this.onChange = this.onChange.bind(this);
    this.renderReverseCheckbox = this.renderReverseCheckbox.bind(this);
  }

  componentDidMount() {
    this.onChange();
  }

  onChange() {
    // @ts-expect-error TS(2339): Property 'type' does not exist on type 'Readonly<{... Remove this comment to see the full error message
    const { type } = this.state;
    const value = { type };
    const errors = [];
    const errMsg = t('Invalid lat/long configuration.');
    if (type === spatialTypes.latlong) {
      // @ts-expect-error TS(2339): Property 'latCol' does not exist on type '{ type: ... Remove this comment to see the full error message
      value.latCol = this.state.latCol;
      // @ts-expect-error TS(2339): Property 'lonCol' does not exist on type '{ type: ... Remove this comment to see the full error message
      value.lonCol = this.state.lonCol;
      // @ts-expect-error TS(2339): Property 'lonCol' does not exist on type '{ type: ... Remove this comment to see the full error message
      if (!value.lonCol || !value.latCol) {
        errors.push(errMsg);
      }
    } else if (type === spatialTypes.delimited) {
      // @ts-expect-error TS(2339): Property 'lonlatCol' does not exist on type '{ typ... Remove this comment to see the full error message
      value.lonlatCol = this.state.lonlatCol;
      // @ts-expect-error TS(2339): Property 'delimiter' does not exist on type '{ typ... Remove this comment to see the full error message
      value.delimiter = this.state.delimiter;
      // @ts-expect-error TS(2339): Property 'reverseCheckbox' does not exist on type ... Remove this comment to see the full error message
      value.reverseCheckbox = this.state.reverseCheckbox;
      // @ts-expect-error TS(2339): Property 'lonlatCol' does not exist on type '{ typ... Remove this comment to see the full error message
      if (!value.lonlatCol || !value.delimiter) {
        errors.push(errMsg);
      }
    } else if (type === spatialTypes.geohash) {
      // @ts-expect-error TS(2339): Property 'geohashCol' does not exist on type '{ ty... Remove this comment to see the full error message
      value.geohashCol = this.state.geohashCol;
      // @ts-expect-error TS(2339): Property 'reverseCheckbox' does not exist on type ... Remove this comment to see the full error message
      value.reverseCheckbox = this.state.reverseCheckbox;
      // @ts-expect-error TS(2339): Property 'geohashCol' does not exist on type '{ ty... Remove this comment to see the full error message
      if (!value.geohashCol) {
        errors.push(errMsg);
      }
    }
    this.setState({ value, errors });
    // @ts-expect-error TS(2339): Property 'onChange' does not exist on type 'Readon... Remove this comment to see the full error message
    this.props.onChange(value, errors);
  }

  setType(type: $TSFixMe) {
    this.setState({ type }, this.onChange);
  }

  toggleCheckbox() {
    this.setState(
      // @ts-expect-error TS(2339): Property 'reverseCheckbox' does not exist on type ... Remove this comment to see the full error message
      prevState => ({ reverseCheckbox: !prevState.reverseCheckbox }),
      this.onChange,
    );
  }

  renderLabelContent() {
    // @ts-expect-error TS(2339): Property 'errors' does not exist on type 'Readonly... Remove this comment to see the full error message
    if (this.state.errors.length > 0) {
      return 'N/A';
    }
    // @ts-expect-error TS(2339): Property 'type' does not exist on type 'Readonly<{... Remove this comment to see the full error message
    if (this.state.type === spatialTypes.latlong) {
      // @ts-expect-error TS(2339): Property 'lonCol' does not exist on type 'Readonly... Remove this comment to see the full error message
      return `${this.state.lonCol} | ${this.state.latCol}`;
    }
    // @ts-expect-error TS(2339): Property 'type' does not exist on type 'Readonly<{... Remove this comment to see the full error message
    if (this.state.type === spatialTypes.delimited) {
      // @ts-expect-error TS(2339): Property 'lonlatCol' does not exist on type 'Reado... Remove this comment to see the full error message
      return `${this.state.lonlatCol}`;
    }
    // @ts-expect-error TS(2339): Property 'type' does not exist on type 'Readonly<{... Remove this comment to see the full error message
    if (this.state.type === spatialTypes.geohash) {
      // @ts-expect-error TS(2339): Property 'geohashCol' does not exist on type 'Read... Remove this comment to see the full error message
      return `${this.state.geohashCol}`;
    }
    return null;
  }

  renderSelect(name: $TSFixMe, type: $TSFixMe) {
    return (
      <SelectControl
        // @ts-expect-error TS(2322): Type '{ ariaLabel: any; name: any; choices: any; v... Remove this comment to see the full error message
        ariaLabel={name}
        name={name}
        // @ts-expect-error TS(2339): Property 'choices' does not exist on type 'Readonl... Remove this comment to see the full error message
        choices={this.props.choices}
        // @ts-expect-error TS(7053): Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        value={this.state[name]}
        clearable={false}
        onFocus={() => {
          this.setType(type);
        }}
        onChange={(value: $TSFixMe) => {
          this.setState({ [name]: value }, this.onChange);
        }}
      />
    );
  }

  renderReverseCheckbox() {
    return (
      <span>
        {t('Reverse lat/long ')}
        <Checkbox
          // @ts-expect-error TS(2339): Property 'reverseCheckbox' does not exist on type ... Remove this comment to see the full error message
          checked={this.state.reverseCheckbox}
          onChange={this.toggleCheckbox}
        />
      </span>
    );
  }

  renderPopoverContent() {
    return (
      <div style={{ width: '300px' }}>
        <PopoverSection
          title={t('Longitude & Latitude columns')}
          // @ts-expect-error TS(2339): Property 'type' does not exist on type 'Readonly<{... Remove this comment to see the full error message
          isSelected={this.state.type === spatialTypes.latlong}
          onSelect={this.setType.bind(this, spatialTypes.latlong)}
        >
          <Row gutter={16}>
            <Col xs={24} md={12}>
              {t('Longitude')}
              {this.renderSelect('lonCol', spatialTypes.latlong)}
            </Col>
            <Col xs={24} md={12}>
              {t('Latitude')}
              {this.renderSelect('latCol', spatialTypes.latlong)}
            </Col>
          </Row>
        </PopoverSection>
        <PopoverSection
          title={t('Delimited long & lat single column')}
          info={t(
            'Multiple formats accepted, look the geopy.points ' +
              'Python library for more details',
          )}
          // @ts-expect-error TS(2339): Property 'type' does not exist on type 'Readonly<{... Remove this comment to see the full error message
          isSelected={this.state.type === spatialTypes.delimited}
          onSelect={this.setType.bind(this, spatialTypes.delimited)}
        >
          <Row gutter={16}>
            <Col xs={24} md={12}>
              {t('Column')}
              {this.renderSelect('lonlatCol', spatialTypes.delimited)}
            </Col>
            <Col xs={24} md={12}>
              {this.renderReverseCheckbox()}
            </Col>
          </Row>
        </PopoverSection>
        <PopoverSection
          title={t('Geohash')}
          // @ts-expect-error TS(2339): Property 'type' does not exist on type 'Readonly<{... Remove this comment to see the full error message
          isSelected={this.state.type === spatialTypes.geohash}
          onSelect={this.setType.bind(this, spatialTypes.geohash)}
        >
          <Row gutter={16}>
            <Col xs={24} md={12}>
              {t('Column')}
              {this.renderSelect('geohashCol', spatialTypes.geohash)}
            </Col>
            <Col xs={24} md={12}>
              {this.renderReverseCheckbox()}
            </Col>
          </Row>
        </PopoverSection>
      </div>
    );
  }

  render() {
    return (
      <div>
        <ControlHeader {...this.props} />
        <Popover
          content={this.renderPopoverContent()}
          placement="topLeft" // so that popover doesn't move when label changes
          trigger="click"
        >
          <Label className="pointer">{this.renderLabelContent()}</Label>
        </Popover>
      </div>
    );
  }
}

// @ts-expect-error TS(2339): Property 'propTypes' does not exist on type 'typeo... Remove this comment to see the full error message
SpatialControl.propTypes = propTypes;
// @ts-expect-error TS(2339): Property 'defaultProps' does not exist on type 'ty... Remove this comment to see the full error message
SpatialControl.defaultProps = defaultProps;
