...
function inferSqlExpressionColumn(adhocMetric: $TSFixMe) {
  if (
...
function inferSqlExpressionAggregate(adhocMetric: $TSFixMe) {
  if (
    adhocMetric.sqlExpression &&
    sqlaAutoGeneratedMetricRegex.test(adhocMetric.sqlExpression)
  ) {
...
export default class AdhocMetric {
  ...
  constructor(adhocMetric: $TSFixMe) {
  ...
  getDefaultLabel() {
    // @ts-expect-error TS(2345): Argument of type '{ useVerboseName: true; }' is no... Remove this comment to see the full error message
