...
function inferSqlExpressionAggregate(adhocMetric: AdhocMetricInput): string | null {
  if (
    adhocMetric.sqlExpression &&
    sqlaAutoGeneratedMetricRegex.test(adhocMetric.sqlExpression)
  ) {
...
export default class AdhocMetric {
  ...
  translateToSql(
    params: { useVerboseName?: boolean; transformCountDistinct?: boolean } = { useVerboseName: false, transformCountDistinct: false },
  ): string {
    if (this.expressionType === EXPRESSION_TYPES.SIMPLE) {
