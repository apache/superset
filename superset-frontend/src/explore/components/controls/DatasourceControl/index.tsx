...
export const getDatasourceTitle = (datasource: $TSFixMe) => {
  if (datasource?.type === 'query') return datasource?.sql;
  return datasource?.name || '';
};

const preventRouterLinkWhileMetaClicked = (evt: $TSFixMe) => {
  if (evt.metaKey) {
    evt.preventDefault();
  } else {
    evt.stopPropagation();
...
class DatasourceControl extends PureComponent {
  ...
  onDatasourceSave = (datasource: $TSFixMe) => {
    ...
    // @ts-expect-error TS(2339): Property 'form_data' does not exist on type 'Reado... Remove this comment to see the full error message
    const timeCol = this.props.form_data?.granularity_sqla;
    const isGranularitySqlaTemporal = columns.find(
      ({ column_name }: $TSFixMe) => column_name === timeCol,
    )?.is_dttm;
    // the current main_dttm_col might not be a temporal column anymore
    const isDefaultTemporal = columns.find(
      ({ column_name }: $TSFixMe) => column_name === defaultTemporalColumn,
    )?.is_dttm;
    ...
    // @ts-expect-error TS(2339): Property 'onDatasourceSave' does not exist on type... Remove this comment to see the full error message
  ...
  toggleShowDatasource = () => {
    // @ts-expect-error TS(2339): Property 'showDatasource' does not exist on type '... Remove this comment to see the full error message
  ...
  toggleChangeDatasourceModal = () => {
    // @ts-expect-error TS(2339): Property 'showChangeDatasourceModal' does not exis... Remove this comment to see the full error message
  ...
  toggleEditDatasourceModal = () => {
    // @ts-expect-error TS(2339): Property 'showEditDatasourceModal' does not exist ... Remove this comment to see the full error message
  ...
  toggleSaveDatasetModal = () => {
    // @ts-expect-error TS(2339): Property 'showSaveDatasetModal' does not exist on ... Remove this comment to see the full error message
    this.setState(({ showSaveDatasetModal }) => ({
      showSaveDatasetModal: !showSaveDatasetModal,
    }));
  };

  handleMenuItemClick = ({ key }: $TSFixMe) => {
    switch (key) {
      case CHANGE_DATASET:
  ...
  render() {
    ...
    const defaultDatasourceMenu = (
      <Menu onClick={this.handleMenuItemClick}>
        // @ts-expect-error TS(2339): Property 'isEditable' does not exist on type 'Read... Remove this comment to see the full error message
    ...
    return (
      <Styles data-test="datasource-control" className="DatasourceControl">
        <div className="data-container">
          ...
          >
            <Icons.MoreOutlined
              // @ts-expect-error TS(2322): Type '{ IconSize: string; iconColor: any; classNam... Remove this comment to see the full error message
              IconSize="xl"
        ...
        {isMissingDatasource && isMissingParams && (
          <div className="error-alert">
            <ErrorAlert
              // @ts-expect-error TS(2322): Type '{ level: string; errorType: string; descript... Remove this comment to see the full error message
              level="warning"
        ...
        {isMissingDatasource && !isMissingParams && (
          <div className="error-alert">
            // @ts-expect-error TS(2741): Property 'message' is missing in type '{ type: "wa... Remove this comment to see the full error message
            <ErrorAlert
              type="warning"
              ...
              descriptionDetails={
                <>
                  ...
                  <p>
                    <Button
                      // @ts-expect-error TS(2322): Type '"warning"' is not assignable to type 'Button... Remove this comment to see the full error message
                      buttonStyle="warning"
                      onClick={() =>
