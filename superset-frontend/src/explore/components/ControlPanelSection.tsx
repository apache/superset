...
export function ControlPanelSection({
  ...
  const { label, description, resetOnHide = true } = section;
  const wasVisible = usePrevious(isVisible);

  useEffect(() => {
    if (wasVisible === true && isVisible === false && resetOnHide) {
  ...
  }, [wasVisible, isVisible, section, setControlValue, resetOnHide]);
  ...
  return (
    ...
    >
      {section.controlSetRows.map((controlSets, i) => {
        ...
        return (
          <ControlRow key={`controlsetrow-${i}`} controls={renderedControls} />
        );
      })}
    </Collapse.Panel>
  );
