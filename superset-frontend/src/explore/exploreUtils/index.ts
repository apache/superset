...
export function getChartKey(explore: $TSFixMe) {
...
export function getAnnotationJsonUrl(slice_id: $TSFixMe, force: $TSFixMe) {
...
export function getExploreUrl({
  ...
  if (slice_id) {
    ...
    if (method === 'GET') {
      // @ts-expect-error TS(2339): Property 'viz_type' does not exist on type '{ slic... Remove this comment to see the full error message
      ...
      if (adhoc_filters && adhoc_filters.length) {
        // @ts-expect-error TS(2339): Property 'adhoc_filters' does not exist on type '{... Remove this comment to see the full error message
...
export const getQuerySettings = (formData: $TSFixMe) => {
...
export const getLegacyEndpointType = ({
  resultType,
  resultFormat,
}: $TSFixMe) => (resultFormat === 'csv' ? resultFormat : resultType);
...
export const exploreChart = (formData: $TSFixMe, requestParams: $TSFixMe) => {
...
export function formatSelectOptions(options: $TSFixMe) {
  return options.map((opt: $TSFixMe) => [opt, opt.toString()]);
}
