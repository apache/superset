...
const groupByControl = {
  ...
  optionRenderer: (c: $TSFixMe) => <StyledColumnOption column={c} showType />,
  valueKey: 'column_name',
  // @ts-expect-error TS(7031): Binding element 'opt' implicitly has an 'any' type... Remove this comment to see the full error message
  filterOption: ({ data: opt }, text: $TSFixMe) =>
    (opt.column_name &&
      opt.column_name.toLowerCase().indexOf(text.toLowerCase()) >= 0) ||
    (opt.verbose_name &&
      opt.verbose_name.toLowerCase().indexOf(text.toLowerCase()) >= 0),
  mapStateToProps: (state: $TSFixMe, control: $TSFixMe) => {
    const newState = {};
    if (state.datasource) {
...
export function columnChoices(datasource: $TSFixMe) {
  if (datasource && datasource.columns) {
    return datasource.columns
      ...
      .sort((opt1: $TSFixMe, opt2: $TSFixMe) =>
...
export const controls = {
  ...
  datasource: {
    ...
    default: null,
    description: null,
    mapStateToProps: ({ datasource }: $TSFixMe) => ({
      datasource,
  ...
  granularity_sqla: {
    ...
    mapStateToProps: (state: $TSFixMe) => {
      const props = {};
      if (state.datasource) {
        // @ts-expect-error TS(2339): Property 'choices' does not exist on type '{}'.
  ...
  adhoc_filters: {
    ...
    mapStateToProps: (state: $TSFixMe) => ({
      columns: state.datasource
        ? state.datasource.columns.filter((c: $TSFixMe) => c.filterable)
        : [],
