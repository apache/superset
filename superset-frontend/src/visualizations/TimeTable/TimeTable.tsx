...
// @ts-expect-error TS(7016): Could not find a declaration file for module 'must... Remove this comment to see the full error message
...
const TimeTableStyles = styled.div`
  // @ts-expect-error TS(2339): Property 'height' does not exist on type '{ theme?... Remove this comment to see the full error message
...
const TimeTable = ({
  ...
  const memoizedColumns = useMemo(
    () => [
      ...
      // @ts-expect-error TS(7006): Parameter 'columnConfig' implicitly has an 'any' t... Remove this comment to see the full error message
  ...
  const memoizedRows = useMemo(() => {
    const renderSparklineCell = (
      ...
      if (column.timeRatio) {
        ...
        sparkData = entries.map((d: $TSFixMe) => d[valueField]);
      }

      return (
        // @ts-expect-error TS(2741): Property 'renderTooltip' is missing in type '{ ari... Remove this comment to see the full error message
        <SparklineCell
    ...
    const renderValueCell = (
      ...
      if (column.colType === 'time') {
        ...
        if (reversedEntries.length > 0) {
          const stats = reversedEntries.slice(undefined, column.timeLag).reduce(
            function ({ count, sum }: $TSFixMe, entry: $TSFixMe) {
      ...
      return (
        ...
        >
          {errorMsg || (
            // @ts-expect-error TS(2322): Type 'string | number | null | undefined' is not a... Remove this comment to see the full error message
            <span style={{ color }}>
              // @ts-expect-error TS(2322): Type 'number | null | undefined' is
              not assignable... Remove this comment to see the full error
              message
    ...
    const renderLeftCell = (row: $TSFixMe) => {
  ...
  return (
    <TimeTableStyles
      ...
      className={className}
      // @ts-expect-error TS(2322): Type '{ children: Element; "data-test": string; cl... Remove this comment to see the full error message
      height={height}
    >
      <TableView
        ...
        data={memoizedRows}
        // @ts-expect-error TS(2322): Type '{ id: any; desc: string; }[]' is not assigna... Remove this comment to see the full error message
