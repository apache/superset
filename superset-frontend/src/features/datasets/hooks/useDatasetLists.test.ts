...
describe('useDatasetsList', () => {
  ...
  test('fetches first page of datasets successfully', async () => {
  ...
  test('fetches multiple pages (pagination) until count reached', async () => {
    const page1Data = [
      { id: 1, table_name: 'table1', schema: 'public' },
      { id: 2, table_name: 'table2', schema: 'public' },
    ];
  ...
  test('extracts dataset names correctly', async () => {
    const getSpy = jest.spyOn(SupersetClient, 'get').mockResolvedValue({
      json: {
        count: 3,
        result: [
  ...
  test('handles empty dataset response', async () => {
    const getSpy = jest.spyOn(SupersetClient, 'get').mockResolvedValue({
      json: {
        count: 0,
        result: [],
  ...
  test('stops pagination when results reach count', async () => {
  ...
  test('resets datasets when schema changes', async () => {
    const getSpy = jest
      .spyOn(SupersetClient, 'get')
      .mockResolvedValueOnce({
        json: {
  ...
  test('skips fetching when schema is null or undefined', () => {
  ...
  test('encodes schemas with spaces and special characters in endpoint URL', async () => {
    const getSpy = jest.spyOn(SupersetClient, 'get').mockResolvedValue({
      json: { count: 0, result: [] },
    } as any);
