...
describe('CSVUploadModal', () => {
  ...
  describe('test basic rendering', () => {
    ...
    test('renders the columns elements correctly', () => {
      ...
      const switchOverwriteDuplicates = screen.getByTestId(
        'overwriteDuplicates',
      );
  ...
  describe('test form post and errors', () => {
    ...
    test('form post', async () => {
      ...
      userEvent.click(uploadButton);
      await waitFor(() =>
        fetchMock.called('glob:*api/v1/database/1/csv_upload/'),
      );

      // Get the matching fetch calls made
      const matchingCalls = fetchMock.calls(
        'glob:*api/v1/database/1/csv_upload/',
      );
