  ...
  toggleExpandSlice: sliceId =>
    set(
      state => {
        ...
        // Toggle the effective expanded state (per-slice override falling back
        // to the global expandAllSlices flag), so the first toggle on a slice
        // shown only because expandAllSlices is true correctly collapses it.
        const currentlyExpanded =
          expandedSlices[sliceId] ?? state.expandAllSlices;
        expandedSlices[sliceId] = !currentlyExpanded;
  ...
  setRefreshFrequency: (refreshFrequency, isPersistent) =>
    set(
      state => ({
        ...
        // Dirty on a persistent change; never clear existing unsaved edits.
        // The reset path omits isPersistent and leaves the flag untouched.
        ...(typeof isPersistent === 'boolean' && {
          hasUnsavedChanges: isPersistent || state.hasUnsavedChanges,
        }),
      }),
      false,
