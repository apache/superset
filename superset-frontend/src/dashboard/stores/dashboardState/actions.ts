...
export const setIsStarred = (isStarred: boolean): void =>
  useDashboardStateStore.getState().setIsStarred(isStarred);

export const markSaved = (lastModifiedTime: number): void =>
...
export const setAutoRefreshStatus = (status: AutoRefreshStatus): void =>
  useDashboardStateStore.getState().setAutoRefreshStatus(status);

export const setAutoRefreshPaused = (isPaused: boolean): void =>
  useDashboardStateStore.getState().setAutoRefreshPaused(isPaused);

export const setAutoRefreshPausedByTab = (isPausedByTab: boolean): void =>
  useDashboardStateStore.getState().setAutoRefreshPausedByTab(isPausedByTab);

export const recordAutoRefreshSuccess = (timestamp?: number): void =>
  useDashboardStateStore.getState().recordAutoRefreshSuccess(timestamp);

export const recordAutoRefreshError = (
  error: string | undefined,
  timestamp?: number,
): void =>
  useDashboardStateStore.getState().recordAutoRefreshError(error, timestamp);

export const setAutoRefreshFetchStartTime = (timestamp: number | null): void =>
  useDashboardStateStore.getState().setAutoRefreshFetchStartTime(timestamp);

export const setAutoRefreshPauseOnInactiveTab = (
