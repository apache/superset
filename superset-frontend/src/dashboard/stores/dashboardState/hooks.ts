...
import type { AutoRefreshStatus } from 'src/dashboard/types/autoRefresh';
...
export const useIsSliceExpanded = (sliceId: number): boolean =>
  useDashboardStateStore(
    s => !!(s.expandedSlices?.[sliceId] ?? s.expandAllSlices),
  );
...
export const useAutoRefreshStatus = (): AutoRefreshStatus =>
  useDashboardStateStore(s => s.autoRefreshStatus);

export const useAutoRefreshPaused = (): boolean =>
  useDashboardStateStore(s => s.autoRefreshPaused);

export const useAutoRefreshPausedByTab = (): boolean =>
  useDashboardStateStore(s => s.autoRefreshPausedByTab);

export const useLastSuccessfulRefresh = (): number | null =>
  useDashboardStateStore(s => s.lastSuccessfulRefresh);

export const useLastAutoRefreshTime = (): number | null =>
  useDashboardStateStore(s => s.lastAutoRefreshTime);

export const useLastRefreshError = (): string | null =>
  useDashboardStateStore(s => s.lastRefreshError);

export const useRefreshErrorCount = (): number =>
  useDashboardStateStore(s => s.refreshErrorCount);

export const useAutoRefreshFetchStartTime = (): number | null =>
  useDashboardStateStore(s => s.autoRefreshFetchStartTime);

export const useAutoRefreshPauseOnInactiveTab = (): boolean =>
  useDashboardStateStore(s => s.autoRefreshPauseOnInactiveTab);
