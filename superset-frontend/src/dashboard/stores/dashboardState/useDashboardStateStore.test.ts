...
test('toggleExpandSlice flips the per-slice expanded flag', () => {
  store.getState().toggleExpandSlice(7);
  expect(store.getState().expandedSlices).toEqual({ 7: true });
  // Collapsing records an explicit false (not a delete), so the expandAllSlices
  // fallback can't silently re-expand it.
  store.getState().toggleExpandSlice(7);
  expect(store.getState().expandedSlices).toEqual({ 7: false });
});

test('toggleExpandSlice collapses a slice shown via expandAllSlices', () => {
  store.setState({ expandAllSlices: true });
  // No per-slice override yet: the effective state comes from expandAllSlices,
  // so the first toggle must collapse it to an explicit false.
  store.getState().toggleExpandSlice(7);
  expect(store.getState().expandedSlices).toEqual({ 7: false });
});
...
test('setRefreshFrequency dirties when persistent and never clears unsaved changes', () => {
  ...
  expect(store.getState().hasUnsavedChanges).toBe(true);
  // Non-persistent change preserves prior unsaved edits rather than clearing.
  store.getState().setRefreshFrequency(60, false);
  expect(store.getState().shouldPersistRefreshFrequency).toBe(false);
  expect(store.getState().hasUnsavedChanges).toBe(true);
  // With nothing pending, a non-persistent change stays clean.
  store.setState({ hasUnsavedChanges: false });
  store.getState().setRefreshFrequency(0, false);
  expect(store.getState().hasUnsavedChanges).toBe(false);
});
