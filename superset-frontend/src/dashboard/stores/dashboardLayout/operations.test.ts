...
// Ported from the retired dashboardLayout reducer tests (#44303): moving a
// component into itself or a descendant must be refused, and the check has to
// survive a stale `parents` array.
describe('moveComponentInLayout self-nesting guard', () => {
  const dropResult = {
  ...
  test('refuses a move into its own descendant', () => {
    const layout = {
    ...
    } as unknown as Parameters<typeof moveComponentInLayout>[0];

    expect(moveComponentInLayout(layout, dropResult)).toBeNull();
  });

  test('refuses a move into a descendant whose parents are stale', () => {
    const layout = {
      parent: { id: 'parent', type: ROW_TYPE, children: ['column'] },
