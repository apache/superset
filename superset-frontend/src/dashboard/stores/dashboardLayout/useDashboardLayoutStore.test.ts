...
import { undoLayout } from './actions';
...
test('undoLayout refuses to restore a rootless layout snapshot (W5)', () => {
  // A rootless layout enters history...
  store.getState().setLayout({ V1: makeLayoutItem({ id: 'V1' }) });
  // ...then a valid rooted layout becomes current.
  store.getState().setLayout({
    [DASHBOARD_ROOT_ID]: makeLayoutItem({
      id: DASHBOARD_ROOT_ID,
      type: 'ROOT',
  ...
  // The guard blocked the undo; the rooted layout is preserved.
  expect(store.getState().layout[DASHBOARD_ROOT_ID]).toBeDefined();
  expect(store.getState().layout.V1).toBeUndefined();
});
