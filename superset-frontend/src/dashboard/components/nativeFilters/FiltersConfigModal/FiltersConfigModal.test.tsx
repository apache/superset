...
test('switches the order between two filters', async () => {
  ...
  const filterContainer = screen.getByTestId('filter-title-container');
  const dragContainers = filterContainer.querySelectorAll('[draggable="true"]');

  fireEvent.dragStart(dragContainers[0]);
  fireEvent.dragEnter(dragContainers[1]);
  fireEvent.dragOver(dragContainers[1]);
  fireEvent.drop(dragContainers[1]);
...
test('rearranges three filters', async () => {
  const nativeFilterState = [
    buildNativeFilter('NATIVE_FILTER-1', 'state', []),
    buildNativeFilter('NATIVE_FILTER-2', 'country', []),
    buildNativeFilter('NATIVE_FILTER-3', 'product', []),
  ...
  const state = {
    ...
    dashboardInfo: {
      metadata: {
        native_filter_configuration: nativeFilterState,
      },
    },
    dashboardLayout,
  };
  ...
  fireEvent.dragStart(dragContainers[0]);
  fireEvent.dragEnter(dragContainers[2]);
  fireEvent.dragOver(dragContainers[2]);
  fireEvent.drop(dragContainers[2]);
  fireEvent.dragEnd(dragContainers[0]);
