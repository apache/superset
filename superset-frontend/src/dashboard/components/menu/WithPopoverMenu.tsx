...
const defaultShouldFocus = (event: any, container: ShouldFocusContainer) =>
  container?.contains(event.target) ||
  event.target.id === 'menu-item' ||
  event.target.parentNode?.id === 'menu-item';

const WithPopoverMenu = ({
  children,
  disableClick = false,
  menuItems = [],
  onChangeFocus = null,
  ...
  const handleClick = useCallback(
    (event: any) => {
      ...
      const shouldFocusResult = shouldFocus(
        event,
        containerRef.current as ShouldFocusContainer,
      );
    ...
    [editMode, shouldFocus, disableClick, isFocused, onChangeFocus],
  );

  useEffect(() => {
    if (editMode && propIsFocused && !isFocused) {
      document.addEventListener('click', handleClick);
      document.addEventListener('drag', handleClick);
      setIsFocused(true);
  ...
  }, [editMode, propIsFocused, isFocused, handleClick]);
