...
const DropIndicator = styled.div`
  ...
  position: absolute;
  top: 0;
  // @ts-expect-error TS(2339): Property 'pos' does not exist on type '{ theme?: T... Remove this comment to see the full error message
  ${({ pos }) => (pos === 'left' ? 'left: -4px' : 'right: -4px')};
  border-radius: 2px;
`;

const CloseIconWithDropIndicator = (props: $TSFixMe) => (
  <>
    <Icons.CloseOutlined iconSize="s" />
    {props.showDropIndicators.right && (
      // @ts-expect-error TS(2322): Type '{ className: string; pos: string; }' is not ... Remove this comment to see the full error message
      <DropIndicator className="drop-indicator-right" pos="right" />
    )}
  </>
);

const Tabs = (props: $TSFixMe) => {
  ...
  const { tabIndex: initTabIndex, activeKey: initActiveKey } = useMemo(() => {
    ...
    if (tabIndex === 0 && activeTabs?.length) {
      props.component.children.forEach((tabId: $TSFixMe, index: $TSFixMe) => {
  ...
  const renderChild = useCallback(
    ({ dragSourceRef: tabsDragSourceRef }) => {
      // @ts-expect-error TS(7006): Parameter 'tabId' implicitly has an 'any' type.
      const tabItems = tabIds.map((tabId, tabIndex) => ({
        ...
        label: removeDraggedTab(tabId) ? (
          ...
          <>
            // @ts-expect-error TS(2339): Property 'left' does not exist on type 'false | { ... Remove this comment to see the full error message
  ...
  return (
    <Draggable
      // @ts-expect-error TS(2322): Type '{ children: ({ dragSourceRef: tabsDragSource... Remove this comment to see the full error message
      component={tabsComponent}
