...
jest.mock('src/dashboard/components/dnd/DragDroppable', () => ({
  Draggable: ({ children }: $TSFixMe) => (
    <div data-test="mock-draggable">{children({})}</div>
  ),
  Droppable: ({ children, depth }: $TSFixMe) => (
    // @ts-expect-error TS(2322): Type '{ children: any; "data-test": string; depth:... Remove this comment to see the full error message
...
jest.mock(
  ...
  () =>
    ({ availableColumnCount, depth }: $TSFixMe) => (
      // @ts-expect-error TS(2322): Type '{ children: any; "data-test": string; depth:... Remove this comment to see the full error message
...
function setup(overrideProps: $TSFixMe) {
...
test.skip('should render a BackgroundStyleDropdown when focused', () => {
  ...
  // @ts-expect-error TS(2339): Property 'find' does not exist on type 'RenderResu... Remove this comment to see the full error message
