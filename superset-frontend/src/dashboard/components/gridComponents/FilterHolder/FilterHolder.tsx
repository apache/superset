...
const FilterHolder = ({
  ...
  const sanitizeDataMask = useCallback((mask: DataMask): DataMask => {
    const { filterState, ...restDataMask } = mask;
    return filterState
      ? {
          ...restDataMask,
          filterState: {
  ...
  const handleFilterSelectionChange = useCallback(
    ...
    [applyMode, dispatch, sanitizeDataMask],
  );
  ...
  }, [applyMode, dataMask, dispatch, filter]);
  ...
  const popoverMenuItems = useMemo(
    () => [
      <PopoverDropdown
        ...
        overlayStyle={{ zIndex: theme.zIndexPopupBase || 3100 }}
