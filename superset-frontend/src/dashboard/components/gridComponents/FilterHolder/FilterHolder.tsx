...
import { FilterBarOrientation, LayoutItem, RootState } from 'src/dashboard/types';
...
const FilterHolder = ({
  ...
  const filter = filterId ? (nativeFilters[filterId] as Filter | undefined) : undefined;

  const titlePosition = (component.meta?.titlePosition as 'top' | 'left') || 'top';
  const applyMode = (component.meta?.applyMode as 'instant' | 'manual') || 'instant';
    ...
    () => [
      <PopoverDropdown
        ...
        renderButton={(opt: { label: ReactNode }) => (
          <span css={css`display: inline-flex; align-items: center; gap: 4px; font-weight: 500; font-size: 12px;`}>
      ...
      ...(availableFilters.length > 0
        ? [
            <PopoverDropdown
              ...
              renderButton={(opt: { label: ReactNode }) => (
                <span css={css`display: inline-flex; align-items: center; gap: 4px; font-weight: 500; font-size: 12px; max-width: 140px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;`}>
        ...
        >
          <div
            ...
            className={cx(
              ...
              titlePosition === 'left' && 'dashboard-component-filter-holder--label-left',
            )}
            css={css`
            ...
            {filterWithDataMask ? (
              <div
                css={css`
                  ...
                  height: 100%;
                  display: flex;
                  flex-direction: ${buttonPlacement === 'bottom'
                    ? 'column'
                    : 'row'};
                  align-items: ${buttonPlacement === 'bottom'
                    ? 'stretch'
                    : 'center'};
                  justifyContent: center;
                  gap: ${theme.sizeUnit}px;
              ...
              >
                ...
                {applyMode === 'manual' && (
                  <div
                    css={css`
                      display: flex;
                      flex-direction: ${buttonPlacement === 'stacked_right'
                        ? 'column'
                        : 'row'};
                      gap: 4px;
                      margin-top: ${buttonPlacement === 'bottom'
                        ? '4px'
                        : '0'};
                      justifyContent: ${buttonPlacement === 'bottom'
                        ? 'flex-end'
                        : 'center'};
                      align-items: center;
                      flex-shrink: 0;
                ...
                {editMode ? (
                  <div>
                    ...
                    {availableFilters.length > 0 ? (
                      ...
                      <span css={css`font-size: ${theme.fontSizeSM}px;`}>
