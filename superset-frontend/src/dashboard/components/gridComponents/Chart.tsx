...
const Chart = (props: $TSFixMe) => {
  ...
  // @ts-expect-error TS(2339): Property 'charts' does not exist on type 'DefaultR... Remove this comment to see the full error message
  const chart = useSelector(state => state.charts[props.id] || EMPTY_OBJECT);
  const slice = useSelector(
    // @ts-expect-error TS(2339): Property 'sliceEntities' does not exist on type 'D... Remove this comment to see the full error message
    state => state.sliceEntities.slices[props.id] || EMPTY_OBJECT,
  );
  // @ts-expect-error TS(2339): Property 'dashboardState' does not exist on type '... Remove this comment to see the full error message
  ...
  const supersetCanExplore = useSelector(
    // @ts-expect-error TS(2339): Property 'dashboardInfo' does not exist on type 'D... Remove this comment to see the full error message
  ...
  const datasource = useSelector(
    state =>
      (chart &&
        chart.form_data &&
        // @ts-expect-error TS(2339): Property 'datasources' does not exist on type 'Def... Remove this comment to see the full error message
  ...
  // @ts-expect-error TS(2339): Property 'nativeFilters' does not exist on type 'D... Remove this comment to see the full error message
  const nativeFilters = useSelector(state => state.nativeFilters?.filters);
  // @ts-expect-error TS(2339): Property 'dataMask' does not exist on type 'Defaul... Remove this comment to see the full error message
  ...
  const isCached =
    queriesResponse?.map(({ is_cached }: $TSFixMe) => is_cached) || [];
  const cachedDttm =
    // eslint-disable-next-line camelcase
    queriesResponse?.map(({ cached_dttm }: $TSFixMe) => cached_dttm) || [];
  ...
  return (
    ...
    >
      <SliceHeader
        ...
        // @ts-expect-error TS(2322): Type 'CachedFormDataWithExtraControls' is not assi... Remove this comment to see the full error message
        formData={formData}
        width={width}
      ...
      >
        ...
        <ChartContainer
          ...
          // @ts-expect-error TS(2739): Type 'CachedFormDataWithExtraControls' is missing ... Remove this comment to see the full error message
          formData={formData}
