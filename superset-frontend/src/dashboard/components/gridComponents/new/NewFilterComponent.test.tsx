...
import NewFilterComponent from 'src/dashboard/components/gridComponents/new/NewFilterComponent';
import { NEW_FILTER_ID } from 'src/dashboard/util/constants';
...
jest.mock(
  ...
  () =>
    ({ type, id }: { type: string; id: string }) => (
      <div
        data-test="mock-draggable-new-component"
        data-testid="mock-draggable-new-component"
      >{`${type}:${id}`}</div>
    ),
);

function setup() {
  return render(<NewFilterComponent />);
}
