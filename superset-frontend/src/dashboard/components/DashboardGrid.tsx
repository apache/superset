...
const renderDraggableContent = (dropProps: $TSFixMe) =>
...
const GridContent = styled.div`
  // @ts-expect-error TS(2339): Property 'editMode' does not exist on type '{ them... Remove this comment to see the full error message
  ${({ theme, editMode }) => css`
    display: flex;
...
class DashboardGrid extends PureComponent {
  ...
  getRowGuidePosition(resizeRef: $TSFixMe) {
  ...
  handleResizeStop(
    ...
    // @ts-expect-error TS(2339): Property 'resizeComponent' does not exist on type ... Remove this comment to see the full error message
  ...
  handleTopDropTargetDrop(dropResult: $TSFixMe) {
    if (dropResult) {
  ...
  handleChangeTab({ pathToTabIndex }: $TSFixMe) {
    // @ts-expect-error TS(2339): Property 'setDirectPathToChild' does not exist on ... Remove this comment to see the full error message
    this.props.setDirectPathToChild(pathToTabIndex);
  }

  render() {
    const {
      // @ts-expect-error TS(2339): Property 'gridComponent' does not exist on type 'R... Remove this comment to see the full error message
      gridComponent,
      // @ts-expect-error TS(2339): Property 'handleComponentDrop' does not exist on t... Remove this comment to see the full error message
      handleComponentDrop,
      ...
      width,
      // @ts-expect-error TS(2339): Property 'isComponentVisible' does not exist on ty... Remove this comment to see the full error message
      isComponentVisible,
      ...
      editMode,
      // @ts-expect-error TS(2339): Property 'canEdit' does not exist on type 'Readonl... Remove this comment to see the full error message
      canEdit,
      // @ts-expect-error TS(2339): Property 'setEditMode' does not exist on type 'Rea... Remove this comment to see the full error message
      setEditMode,
    ...
    // @ts-expect-error TS(2339): Property 'isResizing' does not exist on type 'Read... Remove this comment to see the full error message
      ...
      <>
        ...
        <div className="dashboard-grid" ref={this.setGridRef}>
          <GridContent
            ...
            // @ts-expect-error TS(2322): Type '{ children: any[]; className: string; "data-... Remove this comment to see the full error message
            editMode={editMode}
          >
            {/* make the area above components droppable */}
            {editMode && (
              <Droppable
                // @ts-expect-error TS(2322): Type '{ children: (dropProps: any) => any; compone... Remove this comment to see the full error message
            ...
            {gridComponent?.children?.map((id: $TSFixMe, index: $TSFixMe) => (
              <Fragment key={id}>
                <DashboardComponent
                  id={id}
                  ...
                  depth={depth + 1}
                  index={index}
                  // @ts-expect-error TS(2322): Type '{ id: any; parentId: any; depth: any; index:... Remove this comment to see the full error message
