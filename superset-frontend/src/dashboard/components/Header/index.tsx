...
const headerContainerStyle = (theme: $TSFixMe) => css`
  border-bottom: 1px solid ${theme.colorSplit};
`;

const editButtonStyle = (theme: $TSFixMe) => css`
  color: ${theme.colors.primary.dark2};
`;

const actionButtonsStyle = (theme: $TSFixMe) => css`
  display: flex;
  align-items: center;
...
const undoRedoStyle = (theme: $TSFixMe) => css`
...
const undoRedoEmphasized = (theme: $TSFixMe) => css`
  color: ${theme.colors.grayscale.base};
`;

const undoRedoDisabled = (theme: $TSFixMe) => css`
  color: ${theme.colors.grayscale.light2};
`;

const saveBtnStyle = (theme: $TSFixMe) => css`
...
const discardBtnStyle = (theme: $TSFixMe) => css`
...
const Header = () => {
  ...
  // @ts-expect-error TS(2339): Property 'dashboardLayout' does not exist on type ... Remove this comment to see the full error message
  ...
  // @ts-expect-error TS(2339): Property 'user' does not exist on type 'DefaultRoo... Remove this comment to see the full error message
  ...
  const startPeriodicRender = useCallback(
    interval => {
      let intervalMessage: $TSFixMe;

      if (interval) {
        const periodicRefreshOptions =
          dashboardInfo.common?.conf?.DASHBOARD_AUTO_REFRESH_INTERVALS;
        const predefinedValue = periodicRefreshOptions.find(
          (option: $TSFixMe) => Number(option[0]) === interval / 1000,
        );
      ...
      const fetchCharts = (charts: $TSFixMe, force = false) =>
  ...
  const handleOnPropertiesChange = useCallback(
    updates => {
      boundActionCreators.dashboardInfoChanged({
        // @ts-expect-error TS(2345): Argument of type '{ slug: any; metadata: any; cert... Remove this comment to see the full error message
        slug: updates.slug,
    ...
    () => [
      !editMode && (
        <PublishedStatus
          ...
          // @ts-expect-error TS(2322): Type '{ dashboardId: any; isPublished: boolean; sa... Remove this comment to see the full error message
          visible={!editMode}
        />
      ),
  ...
  const handleReportDelete = async (report: $TSFixMe) => {
  ...
  const [menu, isDropdownVisible, setIsDropdownVisible] = useHeaderActionsMenu({
    ...
    // @ts-expect-error TS(2322): Type '(text: string, options?: Partial<Omit<ToastM... Remove this comment to see the full error message
    ...
    colorNamespace,
    colorScheme,
    // @ts-expect-error TS(2322): Type '(data: any, id: any, saveType: any) => (disp... Remove this comment to see the full error message
    ...
    // @ts-expect-error TS(2322): Type '(eventName: string, eventData: Record<string... Remove this comment to see the full error message
  ...
  return (
    ...
    >
      <PageHeaderWithActions
        ...
        menuDropdownProps={{
          // @ts-expect-error TS(2322): Type 'boolean | Element | Dispatch<SetStateAction<... Remove this comment to see the full error message
