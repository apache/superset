...
const Header = () => {
  ...
  const overwriteDashboard = useCallback(() => {
    ...
    if (isFeatureEnabled(FeatureFlag.TaggingSystem)) {
      if (
        dashboardInfo.tags?.length > 0 &&
        typeof dashboardInfo.tags[0] === 'object'
      ) {
        // filter only on custom tags (type 1) and not implicit tags generated by Superset (type 2, 3 and 4)
