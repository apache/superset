...
export function useSaveCrossFiltersSetting() {
  ...
  return useMutation({
    // Serialize with the other json_metadata mutations: each snapshots metadata
    // at start and PUTs the whole blob, so concurrent runs would clobber each
    // other's fields (last-writer-wins). A shared scope runs them one at a time.
    scope: { id: 'dashboard-metadata' },
