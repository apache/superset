  ...
  >({
    mutationFn: async ({
      ...
      try {
        ...
        // Drop the deleted customizations' data masks only after the save
        // succeeds, so a failed PUT doesn't strand the user with lost values.
        ...
        useNativeFiltersStore
          .getState()
          .setFiltersConfigComplete(mergedResult as FilterEntry[], deletedIds);
