...
// Ported from the retired savePublished thunk tests: the toast prefers the
// server's own message and falls back to the permission text.
const permissionToast = 'You do not have permissions to edit this dashboard.';

test('shows the permission toast for a non-JSON 403', async () => {
  const id = 123;
  setCurrentDashboard(id);
  const put = jest.spyOn(SupersetClient, 'put').mockRejectedValue(
  ...
  await waitFor(() => expect(mockAddDangerToast).toHaveBeenCalledTimes(1));
  expect(mockAddDangerToast).toHaveBeenCalledWith(permissionToast);
  put.mockRestore();
});
...
test.each([
  ...
  [500, {}, permissionToast],
])('publish failing with %i %j shows "%s"', async (status, body, text) => {
  const id = 123;
  setCurrentDashboard(id);
  const put = jest
    .spyOn(SupersetClient, 'put')
    .mockRejectedValue(new Response(JSON.stringify(body), { status }));

  const { result } = renderHook(() => usePublishDashboard(id), { wrapper });
  result.current.mutate(true);

  await waitFor(() => expect(mockAddDangerToast).toHaveBeenCalledTimes(1));
  expect(mockAddDangerToast).toHaveBeenCalledWith(text);
  put.mockRestore();
});
