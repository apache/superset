...
import { rebaselinePublishedInSnapshot } from 'src/dashboard/util/rebaselineHydrationDashboardInfo';
import { getClientErrorFromUnknown } from '../getClientError';
...
export function usePublishDashboard(id: number) {
  ...
  return useMutation({
    ...
    onSuccess: isPublished => {
      ...
      // Patch only `isPublished` in the discard snapshot so the publish state
      // survives a later in-place discard without baking unsaved edits into it.
      rebaselinePublishedInSnapshot(id, isPublished);
    },
    onError: async (response: unknown) => {
      if (!isCurrentDashboard(id)) return;
      // Prefer the server's own message; fall back to the generic permission
      // text when it carries nothing useful.
      const { error } = await getClientErrorFromUnknown(response);
      addDangerToast(
