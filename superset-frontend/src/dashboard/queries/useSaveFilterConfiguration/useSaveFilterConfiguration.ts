...
export function useSaveFilterConfiguration() {
  ...
  return useMutation({
    ...
    onSuccess: result => {
      ...
      // chartsInScope/tabsInScope in the response are the persisted (possibly
      // stale) copies for every filter, including ones this save didn't touch.
      // Drop them so the stores keep the scopes calculated live this session.
      ...
      useNativeFiltersStore
        ...
        .setFiltersConfigComplete(
          savedFilters as FilterEntry[],
          filterChanges.deleted,
        );
      useDashboardInfoStore.getState().setNativeFiltersConfig(savedFilters);
