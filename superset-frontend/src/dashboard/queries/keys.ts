...
export const dashboardKeys = {
  all: DASHBOARD_ROOT_KEY,
  detail: (idOrSlug: string | number) =>
    [...dashboardKeys.all, 'detail', String(idOrSlug)] as const,
  // Shared prefix (derived from `all`) so gcTime can be set once instead of
  // per dashboard id, and the invariant holds by construction.
  hydrationPayloadAll: [...DASHBOARD_ROOT_KEY, 'hydration-payload'] as const,
...
export const sliceKeys = {
  all: ['slices'] as const,
  // Normalize the fetcher's `sortColumn` default so an omitted value and an
  // explicit 'changed_on' map to one cache entry rather than two.
  list: (params: SlicesListParams) =>
    [
      ...sliceKeys.all,
      'list',
      { ...params, sortColumn: params.sortColumn ?? 'changed_on' },
    ] as const,
} as const;
