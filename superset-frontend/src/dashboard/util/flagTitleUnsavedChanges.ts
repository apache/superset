...
export function flagTitleUnsavedChanges(
  ...
  // EditableTitle.handleBlur only commits a non-empty, actually-changed value,
  // so an empty or trim-unchanged title never persists; treat it as clean here
  // too, otherwise typing to empty and blurring would strand the dirty flag.
  const trimmed = value.trim();
  const willChange = trimmed !== '' && trimmed !== savedTitle;
  if (willChange) {
