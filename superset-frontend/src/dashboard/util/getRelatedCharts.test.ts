...
test('getRelatedCharts returns empty array when the filter is undefined', () => {
  // A native filter can transiently disappear from the map (e.g. right after
  // saving a chart customization) while it is still hovered/focused. Guard
  // against reading .scope on undefined so the dashboard doesn't crash.
