...
type MockArgs = {
  ...
  hasChildren?: boolean;
  orientation?: 'row' | 'column';
  clientOffset?: Partial<ClientOffset>;
...
}: MockArgs): [DropTargetMonitor, DropTargetComponent] {
  const completeClientOffset: ClientOffset = {
    x: clientOffset.x ?? 0,
    y: clientOffset.y ?? 0,
  };

  const completeBoundingClientRect: DOMRectLike = {
    top: boundingClientRect.top ?? 0,
    right: boundingClientRect.right ?? 0,
    bottom: boundingClientRect.bottom ?? 0,
    left: boundingClientRect.left ?? 0,
  };

  const monitorMock: DropTargetMonitor = {
    getItem: () => ({
      id: 'id',
      type: draggingType,
    }),
    getClientOffset: () => completeClientOffset,
  };

  const ComponentMock: DropTargetComponent = {
    props: {
      depth,
      parentComponent: {
        type: parentType,
    ...
    ref: {
      getBoundingClientRect: () => completeBoundingClientRect,
    },
  };
