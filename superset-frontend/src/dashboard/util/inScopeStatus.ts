...
export function setInScopeStatusOfFilters(
  ...
  const filtersWithScopes = filterScopes
    .map(scope => {
      // Skip a scope whose filter isn't in the store yet: spreading `undefined`
      // would produce an id-less entry that later keys the store by "undefined".
      const existing = filters[scope.filterId];
