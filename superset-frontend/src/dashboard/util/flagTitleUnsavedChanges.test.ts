...
test('does not flag unsaved for an empty title, which never commits', () => {
  const ref: TitleDirtyRef = { current: undefined };
  flagTitleUnsavedChanges('Sales', '', ref);
  expect(isDirty()).toBe(false);
});

test('does not flag unsaved for a trim-only change, which never commits', () => {
  const ref: TitleDirtyRef = { current: undefined };
  flagTitleUnsavedChanges('Sales', '  Sales  ', ref);
  expect(isDirty()).toBe(false);
});
