...
export default function buildFilterScopeTreeEntry({
  ...
  // @ts-expect-error TS(7006): Parameter 'filterField' implicitly has an 'any' ty... Remove this comment to see the full error message
  editingList.forEach(filterField => {
    (filterScopeMap[filterField].checked || []).forEach((chartId: $TSFixMe) => {
