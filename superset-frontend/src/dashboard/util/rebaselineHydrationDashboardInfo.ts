...
/**
 * Refresh only the persisted `isPublished` flag in the discard snapshot's state
 * seed. Publishing persists nothing else, so rebaselining the whole snapshot
 * here would bake any unsaved edits (made in edit mode while the PUT is in
 * flight) into the discard baseline.
 */
export function rebaselinePublishedInSnapshot(
  id: number,
