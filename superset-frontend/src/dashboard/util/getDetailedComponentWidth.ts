...
function getTotalChildWidth({ id, components }: $TSFixMe) {
  const component = components[id];
  if (!component) return 0;

  let width = 0;

  (component.children || []).forEach((childId: $TSFixMe) => {
...
export default function getDetailedComponentWidth({
  ...
  if (component.type === ROW_TYPE) {
    ...
    // @ts-expect-error TS(2322): Type '12' is not assignable to type 'undefined'.
    result.width =
    ...
    // @ts-expect-error TS(2322): Type '1' is not assignable to type 'undefined'.
    ...
    // @ts-expect-error TS(2322): Type '0' is not assignable to type 'undefined'.
