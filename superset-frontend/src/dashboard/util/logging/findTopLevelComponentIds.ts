...
function findTopLevelComponentIds(layout: $TSFixMe) {
  const topLevelNodes: $TSFixMe = [];
  ...
  function recurseFromNode({
    ...
    if (node.children && node.children.length) {
      node.children.forEach((childId: $TSFixMe, childIndex: $TSFixMe) => {
        recurseFromNode({
          node: layout[childId],
          index: childIndex,
...
let cachedTopLevelNodes: $TSFixMe;
export default function findTopLevelComponentIdsWithCache(layout: $TSFixMe) {
