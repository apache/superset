...
export function recursivelyDeleteChildren(
  componentId: $TSFixMe,
  componentParentId: $TSFixMe,
  nextComponents: $TSFixMe,
) {
  ...
  if (component) {
    ...
    children?.forEach?.((childId: $TSFixMe) => {
...
const actionHandlers = {
  [HYDRATE_DASHBOARD](state: $TSFixMe, action: $TSFixMe) {
    return {
      ...action.data.dashboardLayout.present,
    };
  },

  [UPDATE_COMPONENTS](state: $TSFixMe, action: $TSFixMe) {
    const {
  ...
  [DELETE_COMPONENT](state: $TSFixMe, action: $TSFixMe) {
    const {
      payload: { id, parentId },
    } = action;
  ...
  [CREATE_COMPONENT](state: $TSFixMe, action: $TSFixMe) {
    const {
      payload: { dropResult },
    } = action;
  ...
  [MOVE_COMPONENT](state: $TSFixMe, action: $TSFixMe) {
    const {
      payload: { dropResult },
    } = action;
  ...
  [CREATE_TOP_LEVEL_TABS](state: $TSFixMe, action: $TSFixMe) {
    const {
      payload: { dropResult },
    } = action;
  ...
  [DELETE_TOP_LEVEL_TABS](state: $TSFixMe) {
    const rootComponent = state[DASHBOARD_ROOT_ID];
    const topLevelId = rootComponent.children[0];
    ...
    let childrenToMove: $TSFixMe = [];
    const nextEntities = { ...state };

    topLevelTabs.children.forEach((tabId: $TSFixMe) => {
  ...
  [UPDATE_COMPONENTS_PARENTS_LIST](state: $TSFixMe) {
    const nextState = {
      ...state,
    };
  ...
  [DASHBOARD_TITLE_CHANGED](state: $TSFixMe, action: $TSFixMe) {
    return {
      ...state,
      [DASHBOARD_HEADER_ID]: {
...
export default function layoutReducer(state = {}, action: $TSFixMe) {
