...
export default function dashboardStateReducer(state = {}, action) {
  switch (action.type) {
    ...
    case SET_IN_SCOPE_STATUS_OF_CUSTOMIZATIONS: {
      const scopeMap = {};
      action.payload.forEach(
        ({ customizationId, chartsInScope, tabsInScope }) => {
          scopeMap[customizationId] = { chartsInScope, tabsInScope };
        },
      );
        ...
        metadata: {
          ...
          chart_customization_config:
            state.metadata?.chart_customization_config?.map(customization => ({
              ...customization,
              chartsInScope:
                scopeMap[customization.id]?.chartsInScope ??
                customization.chartsInScope,
              tabsInScope:
                scopeMap[customization.id]?.tabsInScope ??
                customization.tabsInScope,
            })) || [],
        },
      };
    }
