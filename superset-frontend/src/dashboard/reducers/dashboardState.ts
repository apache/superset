...
export default function dashboardStateReducer(state = {}, action: $TSFixMe) {
  const actionHandlers = {
    ...
    [ADD_SLICE]() {
      // @ts-expect-error TS(2339): Property 'sliceIds' does not exist on type '{}'.
    ...
    [TOGGLE_EXPAND_SLICE]() {
      // @ts-expect-error TS(2339): Property 'expandedSlices' does not exist on type '... Remove this comment to see the full error message
    ...
    [SET_ACTIVE_TAB]() {
      // @ts-expect-error TS(2339): Property 'activeTabs' does not exist on type '{}'.
      const newActiveTabs = new Set(state.activeTabs).difference(
        new Set(action.inactiveTabs.concat(action.prevTabId)),
      );
      // @ts-expect-error TS(2339): Property 'inactiveTabs' does not exist on type '{}... Remove this comment to see the full error message
      const newInactiveTabs = new Set(state.inactiveTabs)
        // @ts-expect-error TS(2339): Property 'difference' does not exist on type 'Set<... Remove this comment to see the full error message
    ...
    [UNSET_FOCUSED_FILTER_FIELD]() {
      ...
      if (
        // @ts-expect-error TS(2339): Property 'focusedFilterField' does not exist on ty... Remove this comment to see the full error message
