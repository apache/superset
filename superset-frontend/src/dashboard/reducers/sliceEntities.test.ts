...
describe('sliceEntities reducer', () => {
  ...
  it('should set loading when fetching slices', () => {
    expect(
      sliceEntitiesReducer(
        // @ts-expect-error TS(2345): Argument of type '{ isLoading: false; }' is not as... Remove this comment to see the full error message
        { isLoading: false },
  ...
  it('should set slices', () => {
    const result = sliceEntitiesReducer(
      // @ts-expect-error TS(2345): Argument of type '{ slices: { a: {}; }; }' is not ... Remove this comment to see the full error message
      { slices: { a: {} } },
