  ...
  return (dispatch: AppDispatch, getState: GetState) => {
    ...
    if (
      ...
      const updateDashboard = async (): Promise<JsonObject | void> => {
        try {
          ...
          // Report the real error, then rethrow so TanStack sees a rejection.
          await onError(error as Response);
          throw error;
        }
      };
      return new Promise<void>((resolve, reject) => {
        ...
        SupersetClient.get({
          ...
          .then((response: JsonObject) => {
            const dashboard = (response.json as JsonObject)
              .result as JsonObject;
          ...
          .catch(async (error: Response) => {
            // Surface the real precheck failure instead of hanging.
            await onError(error);
            reject(error);
          });
      })
    ...
    return SupersetClient.post({
      ...
      .catch(async response => {
        // Rethrow after side effects so the TanStack mutation sees a rejection
        // instead of resolving with the deferred-save `undefined` sentinel.
