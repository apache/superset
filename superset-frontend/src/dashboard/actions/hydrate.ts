...
// Re-exported for back-compat: the migration relocated this type to
// buildDashboardLayout, but consumers (e.g. versionHistory) import it from here.
export type { HydrateChartData } from '../util/buildDashboardLayout';
...
export const hydrateDashboard =
  ...
  (dispatch: AppDispatch, getState: GetState): AnyAction => {
    ...
    // Version preview passes editModeOverride=false to force view mode; otherwise
    // derive edit mode from permission + the `edit` URL param.
    const hydratedEditMode = editModeOverride ?? (canEdit && editMode);
    ...
    const hydrated = dispatch({
      type: HYDRATE_DASHBOARD,
      data: hydrateData,
    });

    // Seeding the layout store above fires SyncDashboardState's layout
    // subscription, which rebuilds this cache from Redux `dashboardFilters` —
    // still the PREVIOUS dashboard's, because only the dispatch above hydrates
    // them. Rebuild once more from the hydrated pair so an A→B navigation can't
    // leave A's filter scopes cached against B's layout.
    buildActiveFilters({
      dashboardFilters: dashboardFilters as Parameters<
        typeof buildActiveFilters
      >[0]['dashboardFilters'],
