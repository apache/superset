...
function DatabaseList({
  ...
  {databaseCurrentlyDeleting.dashboards.count >= 1 && (
    <>
      <h4>{t('Affected Dashboards')}</h4>
      <ul>
        {databaseCurrentlyDeleting.dashboards.result.slice(0, 10).map(
          (result: { id: number; title: string }, index: number) => (
            <li key={result.id}>
              <a
                href={`/superset/dashboard/${result.id}`}
                target="_atRiskItem"
        ...
        {databaseCurrentlyDeleting.dashboards.result.length > 10 && (
          <li>
            {t(
              '... and %s others',
              databaseCurrentlyDeleting.dashboards.result.length - 10
  ...
  {databaseCurrentlyDeleting.charts.count >= 1 && (
    <>
      ...
      <ul>
        {databaseCurrentlyDeleting.charts.result.slice(0, 10).map(
          (result: { id: number; slice_name: string }, index: number) => (
            <li key={result.id}>
              <a
                href={`/explore/?slice_id=${result.id}`}
                target="_atRiskItem"
  ...
  {DatabaseDeleteRelatedExtension && (
    <DatabaseDeleteRelatedExtension database={databaseCurrentlyDeleting} />
  )}
</>
