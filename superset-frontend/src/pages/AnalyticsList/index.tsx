...
/**
 * withToasts types the properties modals as ComponentType<any>, so the
 * renderExtraFields callback argument has to be annotated at the call site.
 */
type ExtraFieldsContext = {
  assetId: number;
...
const PASSWORDS_NEEDED_MESSAGE = t(
  'The passwords for the databases below are needed in order to ' +
    'import them together with the assets. Please note that the ' +
    '"Secure Extra" and "Certificate" sections of ' +
    'the database configuration are not present in export files, and ' +
    'should be added manually after the import if they are needed.',
);
const CONFIRM_OVERWRITE_MESSAGE = t(
  'You are importing one or more assets that already exist. ' +
...
function AnalyticsList({
  ...
  const [folderValidationError, setFolderValidationError] = useState<
    string | null
  >(null);
  const pendingFolderMove = useRef<{
    assetType: 'chart' | 'dashboard';
    assetId: number;
    folderUuid: string | null;
  } | null>(null);
  const executePendingFolderMove = useCallback(async () => {
    const move = pendingFolderMove.current;
    if (!move) return;
    pendingFolderMove.current = null;
    try {
      if (move.folderUuid) {
        await SupersetClient.post({
          endpoint: `/api/v1/folders/${move.folderUuid}/assets`,
          jsonPayload: {
            assets: [{ type: move.assetType, id: move.assetId }],
          },
        });
      } else {
        // Move to root — remove from current folder
        const { json } = await SupersetClient.get({
          endpoint: `/api/v1/folders/asset-location?type=${move.assetType}&id=${move.assetId}`,
        });
        const currentFolder = json?.result?.folder_uuid;
        if (currentFolder) {
      ...
      addDangerToast(t('Error moving item to folder'));
    }
  }, [addDangerToast]);
      ...
      filters.forEach(({ id, operator, value }) => {
        ...
        switch (id) {
          ...
          case 'type':
            ...
            break;
          case 'editors':
            filterExps.push({ col: 'editors', opr: 'rel_m_m', value: scalar });
            break;
          case 'viewers':
            filterExps.push({ col: 'viewers', opr: 'rel_m_m', value: scalar });
            break;
          case 'changed_on':
              ...
              items: [
                {
                  ...
                  label: t('Chart'),
                  onClick: () => setImportResource('chart'),
                },
                {
                  key: 'import-dashboard',
                  icon: <Icons.LayoutOutlined />,
                  label: t('Dashboard'),
                  onClick: () => setImportResource('dashboard'),
                },
              ],
            }}
          >
            <Button buttonStyle="secondary">
              <Icons.DownloadOutlined iconSize="m" /> {t('Import')}{' '}
    ...
    <>
      ...
      {editChart && (
        <ChartPropertiesModal
          ...
          onSave={async () => {
            await executePendingFolderMove();
            setEditChart(null);
            refreshData();
          }}
          renderExtraFields={({
            ...
            content: (
              <FolderSelector
                ...
                onChange={folderUuid => {
                  pendingFolderMove.current = {
                    assetType,
                    assetId,
                    folderUuid,
                  };
                }}
                onValidationError={setFolderValidationError}
              />
            ),
            saveDisabled: !!folderValidationError,
            saveTooltip: folderValidationError ?? undefined,
          })}
        />
      )}
      ...
      {importResource && (
        <ImportModelsModal
          resourceName={importResource}
          resourceLabel={
            importResource === 'chart' ? t('chart') : t('dashboard')
          }
          ...
          addSuccessToast={addSuccessToast}
          onModelImport={() => {
            setImportResource(null);
            refreshData();
            addSuccessToast(
              importResource === 'chart'
                ? t('Chart imported')
                : t('Dashboard imported'),
            );
          }}
          show
          onHide={() => setImportResource(null)}
