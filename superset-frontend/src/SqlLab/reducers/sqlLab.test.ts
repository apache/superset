...
describe('sqlLabReducer', () => {
  describe('Query editors actions', () => {
    let newState: $TSFixMe;
    let defaultQueryEditor: $TSFixMe;
    let qe: $TSFixMe;
    beforeEach(() => {
      ...
      qe = newState.queryEditors.find((e: $TSFixMe) => e.id === 'abcd');
    });
    ...
    it('should select the latest query editor when tabHistory is empty', () => {
      ...
      expect(
        newState.queryEditors.map((qe: $TSFixMe) => qe.id),
      ).not.toContainEqual(currentQE.id);
    ...
    it('should remove a query editor including unsaved changes', () => {
      ...
      // @ts-expect-error TS(2741): Property 'dbId' is missing in type '{ type: string... Remove this comment to see the full error message
      action = {
        type: actions.REMOVE_QUERY_EDITOR,
        queryEditor: qe,
      };
    ...
    it('should migrate query editor by new query editor id', () => {
      const index = newState.queryEditors.findIndex(
        ({ id }: $TSFixMe) => id === qe.id,
      );
