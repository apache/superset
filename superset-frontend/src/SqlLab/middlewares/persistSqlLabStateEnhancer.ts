...
const CLEAR_ENTITY_HELPERS_MAP = {
  ...
  unsavedQueryEditor: (qe: $TSFixMe) => clearQueryEditors([qe])[0],
};

const sqlLabPersistStateConfig = {
  paths: ['sqlLab'],
  config: {
    slicer: (paths: $TSFixMe) => (state: $TSFixMe) => {
      const subset = {};
      paths.forEach((path: $TSFixMe) => {
        if (isFeatureEnabled(FeatureFlag.SqllabBackendPersistence)) {
          ...
          if (
            ...
            // @ts-expect-error TS(2339): Property 'sqlLab' does not exist on type '{}'.
            subset.sqlLab = {
              queryEditors: unsavedQueryEditors,
              ...(!hasFinishedMigrationFromLocalStorage && {
                tabHistory,
                tables: tables.filter(
                  (table: $TSFixMe) => table.inLocalStorage,
                ),
                queries: pickBy(
                  queries,
    ...
    merge: (initialState: $TSFixMe, persistedState = {}) => {
      const result = {
        ...initialState,
        ...persistedState,
        sqlLab: {
...
/** @type {any} */
// @ts-expect-error TS(2742): The inferred type of 'persistSqlLabStateEnhancer' ... Remove this comment to see the full error message
