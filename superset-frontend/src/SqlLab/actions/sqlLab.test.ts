...
describe('fetchQueryResults', () => {
  ...
  test('calls queryFailed on fetch error', ({ expect }) => {
    expect.assertions(1);
    fetchMock.removeRoute(fetchQueryEndpoint);
    fetchMock.get(
      fetchQueryEndpoint,
...
describe('popSavedQuery', () => {
  const supersetClientGetSpy = vi.spyOn(SupersetClient, 'get');
  const store = mockStore({});
...
describe('backend sync', () => {
  ...
  describe('addQueryEditor', () => {
    test('creates the tab state in the local storage', () => {
      ...
      expect(store.getActions()).toEqual(expectedActions);
      expect(fetchMock.callHistory.calls(updateTabStateEndpoint)).toHaveLength(
        0,
      );
    });
  });
