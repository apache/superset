...
function Treemap(element, props) {
  ...
  function draw(data, eltWidth, eltHeight) {
    ...
    const text = function(selection) {
      selection.selectAll('tspan').attr('x', d => x(d.x) + 6);
      selection
        .attr('x', d => x(d.x) + 6)
        .attr('y', d => y(d.y) + 6)
    ...
    const display = function(d) {
      ...
      const transition = function(d) {
        ...
        chartContainer.selectAll('.depth').sort((a, b) => a.depth - b.depth);
      ...
      t.append('tspan').text(d => d.name);

      t.append('tspan')
        .attr('dy', '1.0em')
        .text(d => formatNumber(d.value));
      t.call(text);
      g.selectAll('rect').style('fill', d => colorFn(d.name));
