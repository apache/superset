...
test('chart type screenshot', async ({ page }) => {
  ...
  await page
    .locator('.viz-gallery')
    .screenshot({ path: docsPath + 'gallery.jpg', type: 'jpeg' });
});
...
test('dashboard content screenshot', async ({ page }) => {
  ...
  await page
    ...
    .screenshot({ path: docsPath + 'slack_dash.jpg', type: 'jpeg' });
});
...
test('chart editor screenshot', async ({ page }) => {
  ...
  await page
    .locator('[data-test="Life Expectancy VS Rural %-list-chart-title"]')
    .click();
...
test('sqllab screenshot', async ({ page }) => {
  ...
  await page.waitForLoadState('domcontentloaded');
  await page
    .getByRole('combobox', { name: 'Select schema or type to' })
    .fill('main');
  await page
    .getByRole('combobox', { name: 'Select schema or type to' })
    .press('Enter');
  ...
  await page
    .getByRole('combobox', { name: 'Select table or type to' })
    .fill('covid');
  await page
    .getByRole('combobox', { name: 'Select table or type to' })
    .press('Enter');
