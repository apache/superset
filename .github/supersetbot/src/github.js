...
class Github {
  ...
  async createBumpLibPullRequest({pythonPackage, verbose = false, dryRun = false, useCurrentRepo = false}) {
    ...
    await runShellCommand({ command: `pip-compile-multi -P ${pythonPackage}`, ...shellOptions });

    // Diffing
    const diffResults = await runShellCommand({ command: 'git diff --color=never --unified=0', ...shellOptions });
