...
def extract_feature_flags(config_path: Path) -> list[FeatureFlag]:
    ...
    if not match:
        print("ERROR: Could not find DEFAULT_FEATURE_FLAGS in config.py", file=sys.stderr)
        sys.exit(1)
    ...
    for i, line in enumerate(lines):
        ...
        if stripped.startswith("#"):
            ...
            if comment_text.startswith("These features") or comment_text.startswith("These flags"):
                current_comments = []
                continue
...
def parse_comment_lines(comment_lines: list[str], flag_name: str, default: bool) -> FeatureFlag | None:
