...
class VersionChangeRecordSchema(Schema):
    ...
    operation = fields.String(
        metadata={
            "description": (
                "The verb for this change: 'add', 'remove', 'move', 'edit', "
                "'update' (a collapsed-summary record for a field whose per-leaf "
                "changes exceeded the cap), or 'announce' (a synthetic headline "
                "record). Surfaced explicitly so consumers need not infer the "
                "verb from from_value/to_value null-tests — 'move' in particular "
