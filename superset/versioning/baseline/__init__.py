...
* :mod:`.dirty` — :func:`_force_parent_dirty_on_child_change` and
  :func:`_pin_audit_columns`: promote a parent into ``session.dirty``
...
* :mod:`.collection` — discovery: which parents need a baseline row?
  ...
  :func:`_collect_parents_to_baseline`, the
  :func:`_child_to_parent_registry` mapping, and the per-parent
...
* :mod:`.shadow` — low-level :func:`_insert_baseline_shadow_row`
...
from superset.versioning.baseline.dirty import _pin_audit_columns
