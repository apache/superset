...
def _apply_result_aggregation(
    ...
    def contributing(
        ...
        if level not in grouped_sources:
            dims = rows[:row_depth] + columns[:column_depth]
            if dims:
                keys = [
                    source[dim].fillna("SUPERSET_PANDAS_NAN").astype(str)
                    for dim in dims
                ]
                grouped_sources[level] = {
                    key if isinstance(key, tuple) else (key,): group
                    for key, group in source.groupby(keys, sort=False, dropna=False)
                }
            else:
                grouped_sources[level] = {(): source}
        key = _rollup_key(row, row_depth, metric_level, is_column=False) + _rollup_key(
            column,
            column_depth,
            metric_level,
    ...
    for ri, row in enumerate(frame.index):
        for ci, column in enumerate(frame.columns):
            ...
            selected_metrics = [metric] if metric in metrics else metrics
            ...
            if fraction in SHOW_VALUES_AS_PERCENT_MODES:
                ...
                denominator_metrics = (
                    metrics
                    if fraction == "percent_total"
                    or ((fraction == "percent_row") != metrics_on_rows)
                    else selected_metrics
                )
                denominator = _reduce_result_values(
                    contributing(row, column, dr, dc, denominator_metrics), aggregate
                )
                value = (
...
def pivot_df(  # pylint: disable=too-many-locals, too-many-arguments, too-many-statements, too-many-branches  # noqa: C901
    ...
    metric_rollups = not result_aggregation and (bool(percent_mode) or metric_totals)
    ...
    if metric_rollups and rollup_levels:
...
def pivot_table_v2(
    ...
    pivot_options: dict[str, Any] = {
        ...
        "metric_totals": not result_aggregation,
