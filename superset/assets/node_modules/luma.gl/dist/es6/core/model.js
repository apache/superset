...
const LOG_DRAW_PRIORITY = 2; // These old picking uniforms should be avoided and we should use picking module
// and set uniforms using Model class 'updateModuleSettings()'

const DEPRECATED_PICKING_UNIFORMS = ['renderPickingBuffer', 'pickingEnabled']; // Model abstract O3D Class
...
export default class Model extends Object3D {
  ...
  render(uniforms = {}, attributes = {}, samplers = {}, transformFeedback = null, parameters = {}, vertexArray = null) {
    addModel(this);
    ...
    log.group(LOG_DRAW_PRIORITY, `>>> RENDERING MODEL ${this.id}`, {
  ...
  addViewUniforms(uniforms) {
    ...
    const viewUniforms = viewMatrix ? this.getCoordinateUniforms(viewMatrix, modelMatrix) : {};
  ...
  _initializeProgram({
    ...
    if (!program) {
      ...
      const getUniforms = assembleResult.getUniforms;
  ...
  _timerQueryEnd() {
    if (this.timerQueryEnabled === true) {
      ...
      if (this.timeElapsedQuery.isResultAvailable()) {
        this.lastQueryReturned = true;
        const elapsedTime = this.timeElapsedQuery.getResult(); // Update stats (e.g. for seer)
...
count: ${this.stats.profileFrameCount}`)();
  ...
  _logAttributesAndUniforms(priority, uniforms = {}) {
    if (log.priority >= priority) {
      ...
      const _getUniformsTable = getUniformsTable({
      ...
      const _getUniformsTable2 = getUniformsTable({
        header: `${this.id} uniforms`,
        program: this.program,
        uniforms: Object.assign({}, this.uniforms, uniforms),
        undefinedOnly: true
      }),
            table = _getUniformsTable2.table,
  ...
  _createAttributeEntry(attribute, location, header) {
    ...
    return {
      ...
      'Memory Size and Layout': `${instanced ? 'I ' : 'P '} ${verts} (x${size}=${bytes}bytes ${type}) loc=${location}`
    };
  } // DEPRECATED / REMOVED
