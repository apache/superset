...
export default class PlaneGeometry extends Geometry {
  ...
  constructor(opts = {}) {
    ...
    const coords = type.split(','); // width, height

    let c1len = opts[`${coords[0]}len`];
    const c2len = opts[`${coords[1]}len`]; // subdivisionsWidth, subdivisionsDepth
    ...
    for (let z = 0; z < subdivisions2; z++) {
      for (let x = 0; x < subdivisions1; x++) {
        const index = (z * subdivisions1 + x) * 6; // Make triangle 1 of quad.

        indices[index + 0] = (z + 0) * numVertsAcross + x;
        indices[index + 1] = (z + 1) * numVertsAcross + x;
        indices[index + 2] = (z + 0) * numVertsAcross + x + 1; // Make triangle 2 of quad.
