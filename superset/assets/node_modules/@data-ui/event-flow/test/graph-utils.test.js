...
describe('buildNodesFromEntityEvents', () => {
  ...
  test('it should create one node for each unique event sequence', () => {
    const nodes = {};
    buildNodesFromEntityEvents(user1, 0, nodes);
    expect(Object.keys(nodes).length).toBe(4);
  ...
  test('it should add proper depth attributes', () => {
    ...
    const expectedDepths = Array(user1.length).fill().map((_, i) => i - startIndex);
  ...
  test('it should add event referenecs to nodes', () => {
    ...
    expect(Object.keys(nodes).length).toBe(user1.length);

    // for each node
    Object.keys(nodes).forEach((nId) => {
      expect(Object.keys(nodes[nId].events).length).toBe(1);
...
describe('getRoot', () => {
  ...
  test('it should only add nodes with depth === 0 to the root', () => {
    const nodes = {
      1: { depth: -1 },
      2: { depth: 0 },
      3: { depth: 1 },
...
describe('ancestorsFromNode', () => {
  ...
  test('it should return events in root -> child order', () => {
  ...
  test('it should work for negative-depth nodes', () => {
    const negativeKey = ordered[0];
    const negativeNode = nodes[negativeKey];
    const expectedEvents = ordered.slice(0, startIndex + 1).map(k => nodes[k]).reverse();

    expect(ancestorsFromNode(negativeNode)).toEqual(expectedEvents);
  });

  test('it should return just the node if it is a root node', () => {
