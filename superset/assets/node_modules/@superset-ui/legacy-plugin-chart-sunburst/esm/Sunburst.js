 ...
 * under the License.
 */ /* eslint-disable no-param-reassign, sort-keys, no-magic-numbers */ /* eslint-disable complexity, no-plusplus, no-continue, babel/no-invalid-this */import d3 from"d3";import PropTypes from"prop-types";import{CategoricalColorNamespace}from"@superset-ui/color";import{getNumberFormatter,NumberFormats}from"@superset-ui/number-format";import wrapSvgText from"./utils/wrapSvgText";import"./Sunburst.css";var propTypes={// Each row is an array of [hierarchy-lvl1, hierarchy-lvl2, metric1, metric2]
// hierarchy-lvls are string. metrics are number
data:PropTypes.arrayOf(PropTypes.array),width:PropTypes.number,height:PropTypes.number,colorScheme:PropTypes.string,metrics:PropTypes.arrayOf(PropTypes.oneOfType([PropTypes.string,PropTypes.object]))};function metricLabel(a){return"string"==typeof a||a instanceof String?a:a.label}// Given a node in a partition layout, return an array of all of its ancestor
...
var b=d3.hsl(E?F(a.name):n(a.m2/a.m1));return .5>b.l?"white":"black"}).attr("class","step-label").text(function(a){return a.name.replace(/_/g," ")}).call(wrapSvgText,l.width,l.height/2),c.attr("transform",function(a,b){return"translate("+b*(l.width+l.spacing)+", 0)"}),c.exit().remove(),o.select(".end-label").attr("x",(a.length+.5)*(l.width+l.spacing)).attr("y",l.height/2).attr("dy","0.35em").text(b),o.style("visibility",null)}// Fade all but the current sequence, and show it in the breadcrumb trail.
...
w[0]!==w[1]&&w[1]&&(E=!1,c=d3.extent(d,function(a){return a.m2/a.m1}),n=d3.scale.linear().domain([c[0],c[0]+(c[1]-c[0])/2,c[1]]).range(["#00D1C1","white","#FFB400"])),q.selectAll("path").data(d).enter().append("svg:path").attr("display",function(a){return a.depth?null:"none"}).attr("d",H).attr("fill-rule","evenodd").style("fill",function(a){return E?F(a.name):n(a.m2/a.m1)}).style("opacity",1).on("mouseenter",f),m=b.value}var j=d3.select(a);j.classed("superset-legacy-chart-sunburst",!0);var k,l,m,n,o,p,q,r,s=b.data,t=b.width,u=b.height,v=b.colorScheme,w=b.metrics,x={top:10,right:5,bottom:10,left:5},y=t,z=u,A=.085*z,B=y-x.left-x.right,C=z-x.top-x.bottom-A,D=Math.min(B,C)/2,E=!0,F=CategoricalColorNamespace.getScale(v),G=d3.layout.partition().size([2*Math.PI,D*D]).value(function(a){return a.m1}),H=d3.svg.arc().startAngle(function(a){return a.x}).endAngle(function(a){return a.x+a.dx}).innerRadius(function(a){return Math.sqrt(a.y)}).outerRadius(function(a){return Math.sqrt(a.y+a.dy)}),I=getNumberFormatter(NumberFormats.SI_3_DIGIT),J=getNumberFormatter(NumberFormats.PERCENT_3_POINT);// vars with shared scope within this function
j.select("svg").remove();var K=j.append("svg:svg").attr("width",y).attr("height",z);c(s[0]),i(s)}Sunburst.displayName="Sunburst",Sunburst.propTypes=propTypes;export default Sunburst;