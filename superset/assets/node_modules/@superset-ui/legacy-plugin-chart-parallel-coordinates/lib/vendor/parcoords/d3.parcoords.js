...
var a={date:function d(a){var b=d3.extent(A.data,function(b){return b[a]?b[a].getTime():null});// special case if single value
return b[0]===b[1]?d3.scale.ordinal().domain([b[0]]).rangePoints(c()):d3.time.scale().domain(b).range(c())},number:function d(a){var b=d3.extent(A.data,function(b){return+b[a]});// special case if single value
return b[0]===b[1]?d3.scale.ordinal().domain([b[0]]).rangePoints(c()):d3.scale.linear().domain(b).range(c())},string:function f(a){var d={},e=[];// Let's get the count for each value so that we can sort the domain based
