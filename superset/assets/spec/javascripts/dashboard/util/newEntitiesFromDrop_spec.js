...
describe('newEntitiesFromDrop', () => {
  test(
    'should return a new Entity of appropriate type, and add it to the drop target children',
    () => {
  ...
  test(
    'should create Tab AND Tabs components if the drag entity is Tabs',
    () => {
      const result = newEntitiesFromDrop({
        dropResult: {
          destination: { id: 'a', index: 0 },
  ...
  test(
    'should create a Row if the drag entity should be wrapped in a row',
    () => {
      const result = newEntitiesFromDrop({
        dropResult: {
          destination: { id: 'a', index: 0 },
