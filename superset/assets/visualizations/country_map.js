...
function countryMapChart(slice, payload) {
    ...
    d3.json('/static/assets/visualizations/countries/' + payload.form_data.select_country.toLowerCase() + '.geojson', function (error, mapData) {
        ...
        var scale = 150;
        var offset = [slice.width() / 2, slice.height() / 2];
        ...
        var hscale = scale * slice.width() / (bounds[1][0] - bounds[0][0]);
        var vscale = scale * slice.height() / (bounds[1][1] - bounds[0][1]);
        var scale = (hscale < vscale) ? hscale : vscale;
        var offset = [slice.width() - (bounds[0][0] + bounds[1][0]) / 2, slice.height() - (bounds[0][1] + bounds[1][1]) / 2];
    ...
    const clicked = function clicked(d) {
        var x, y, k, bigTextX, bigTextY, bigTextSize, resultTextX, resultTextY;
