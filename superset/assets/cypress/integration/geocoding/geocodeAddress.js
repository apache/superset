...
export default () => {
  describe('geocode Data from a Table', () => {
    beforeEach(() => {
      ...
      cy.route('/superset/geocoding').as('finish_geocoding');
    });

    // get data from examples database, this should be a valid address and then geocode that
    // next step check if data was added to the table -> use hardcoded values or regex?
    // use country_name column from wb_health_population
    it('test geocoding of simple data', () => {

      cy.get('#datasource').then((elem) => {
        elem.val('1');
      });
      ocy.get('#countryColumn').then((elem) => {
        elem.val('country_name');
      });

       cy.get('button').contains('Geocode').click();
      cy.url({ timeout: 30000 }).should('include', '200');
