...
import { requiresQuery } from './AnnotationTypes';
...
const px = function (state) {
  ...
  function runAnnotationQuery(annotation, timeout = 60, formData = null) {
    ...
    const sliceFormData = Object.keys(annotation.overrides)
      .reduce((d, k) => ({
        ...d,
        [k]: annotation.overrides[k] || formData[k],
      }), {});
    ...
    }).then(queryResponse => ({ [name]: queryResponse.data }))
      .catch(() => {});
  }
