...
class AsyncTaskPruneCommand(BaseCommand):
    """
    Command to prune the async_tasks table by deleting rows older than the specified
    retention period.

    This command deletes records from the `AsyncTask` table that have been completed
    ...
    def __init__(self, retention_period_days: int, max_rows_per_run: int | None = None):
        """
        :param retention_period_days: Number of days to keep in the async_tasks table
    ...
    def run(self) -> None:
        ...
        if self.max_rows_per_run is not None and self.max_rows_per_run > 0:
            select_stmt = select_stmt.order_by(
                AsyncTask.ended_at.asc(), AsyncTask.id.asc()
        ...
        for i in range(0, total_rows, batch_size):
            ...
            result = db.session.execute(
                sa.delete(AsyncTask).where(AsyncTask.id.in_(batch_ids))
            )
            ...
            if percentage_complete >= next_logging_threshold:
                logger.info(
                    "Deleted %s rows from the async_tasks table older than %s days (%d%% complete)",  # noqa: E501
                    f"{total_deleted:,}",
