...
class AnalyzeDatabaseSchemaCommand(BaseCommand):
    ...
    def _extract_table_info(
        ...
        # Get primary keys
        pk_constraint = inspector.get_pk_constraint(table_name, schema=self.report.schema_name)
        primary_keys = pk_constraint["constrained_columns"] if pk_constraint else []
        
        # Get foreign keys
        foreign_keys = inspector.get_foreign_keys(table_name, schema=self.report.schema_name)
        fk_columns = set()
        ...
        try:
            result = engine.execute(
                text(
                    f"SELECT obj_description('{self.report.schema_name}.{table_name}'::regclass, 'pg_class')"
                )
            )
            row = result.fetchone()
        ...
        if table_type == TableType.TABLE:
            try:
                query = text(
                    f'SELECT * FROM "{self.report.schema_name}"."{table_name}" '
                ...
                try:
                    query = text(
                        f'SELECT * FROM "{self.report.schema_name}"."{table_name}" LIMIT 3'
                    )
                    result = engine.execute(query)
                    for row in result:
                        sample_rows.append(dict(row))
        ...
        try:
            result = engine.execute(
                text(
                    ...
                    f"WHERE oid = '{self.report.schema_name}.{table_name}'::regclass"
                )
            )
            row = result.fetchone()
