...
#
# Supported tokens in any string value:
#   {title}  — replaced with the dashboard title at render time
#   {date}   — replaced with the Chromium-injected print date
#
# Constraints:
#   • Each slot is capped at 200 px wide with overflow:hidden + text-overflow:ellipsis
#     so long strings are truncated gracefully rather than overflowing the band.
#   • HTML is not supported — values are HTML-escaped before insertion.
#   • The footer right slot ("Page N of M") is fixed and cannot be overridden.
#
# Set BROWSER_PRINT_PDF_HEADER_FOOTER to False (in app config) to disable the
# entire header/footer band.
BROWSER_PRINT_PDF_HEADER_CONTENT: dict[str, str] = {
    # Left slot: bold dashboard title
    "left": "{title}",
    # Center slot: empty by default
    "center": "",
    # Right slot: app name + print date
    "right": "Apache Superset | {date}",
}
BROWSER_PRINT_PDF_FOOTER_CONTENT: dict[str, str] = {
    # Left slot
    "left": "Confidential",
    # Center slot
    "center": "Generated by Apache Superset",
    # Right slot is always "Page N of M" — not overridable via this config.
}
