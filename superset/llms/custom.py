...
class CustomLlm(BaseLlm):
    ...
    def _load_provider_config(self) -> None:
        ...
        if not provider_name.startswith("custom_"):
            logger.error("Invalid custom provider name: %s", provider_name)
            return
        ...
        try:
            ...
            logger.error("Invalid custom provider ID in: %s", provider_name)
            return
        ...
        custom_provider = (
            superset_db.session.query(CustomLlmProvider)
            .filter_by(id=provider_id, enabled=True)
            .first()
        )

        if not custom_provider:
            logger.error(
                f"Custom provider with ID {provider_id} not found or disabled."
            )
            return

        self._provider_config = custom_provider
        logger.info("Loaded custom provider config for '%s'", custom_provider.name)
    ...
    def _extract_response_content(
        self, response_data: Dict[str, Any], path: str
    ) -> str:
        ...
        try:
            ...
            logger.error("Failed to extract content using path '%s': %s", path, e)
            return ""
    ...
    def _substitute_template_variables(
        ...
        if isinstance(template, str):
            ...
            return {
                k: self._substitute_template_variables(v, variables)
    ...
    def generate_sql(self, pk: int, prompt: str, context: str, schemas: List[str] | None) -> str:
        ...
        if not self._provider_config:
            logger.error(
                f"Custom provider configuration not found for database {self.pk}."
            )
        ...
        system_prompt = (
            ...
            if user_instructions
                ...
                self._provider_config.system_instructions
                or self.get_system_instructions(self.dialect)
            )
        )
        ...
        try:
            ...
            logger.error(
                "Invalid request_template JSON in custom provider configuration."
            )
        ...
        request_data = self._substitute_template_variables(
            request_template, template_variables
        )
        ...
        if self._provider_config.headers:
            try:
                ...
                headers.update(
                    self._substitute_template_variables(
                        custom_headers, template_variables
                    )
                )
            except json.JSONDecodeError:
        ...
        try:
            ...
            logger.error("Custom LLM API request failed: %s", e)
            return f"-- Failed to generate SQL: {str(e)}"
        except json.JSONDecodeError as e:
            logger.error("Invalid JSON response from custom LLM: %s", e)
            return "-- Failed to generate SQL: Invalid JSON response"

        # Extract SQL from response
        raw_content = self._extract_response_content(
            response_data, self._provider_config.response_path
        )
    ...
    def get_context_size(self) -> int:
        ...
        logger.info("Estimated context size: %s", estimated_tokens)
        return self.cached_context_size
