...
def _get_last_successful_task_for_database(
    pk: int,
) -> Tuple[ContextBuilderTask, AsyncResult]:
...
def _get_or_create_llm_provider(pk: int, dialect: str, provider_type: str) -> BaseLlm:
    (context_builder_task, context_builder_worker) = (
        _get_last_successful_task_for_database(pk)
    )
...
def get_state(pk: int) -> dict:
    ...
    if successful_task:
        provider = _get_or_create_llm_provider(
            pk, db.backend, db.llm_connection.provider
        )
        result["context"] = {
    ...
    error_task = next((task for task in last_two_tasks if task.status == "ERROR"), None)
