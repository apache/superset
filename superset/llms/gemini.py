...
class GeminiLlm(BaseLlm):
    ...
    def _create_schema_cache(
        self, gemini_client, model, user_instructions
    ) -> types.CachedContent:
    ...
    def _get_response_error(self, response: types.GenerateContentResponse) -> str:
        ...
        match response.candidates[0].finish_reason:
            ...
            case types.FinishReason.SPII:
                return (
                    error
                    + "Gemini detected personally identifiable information in the output"
                )
    ...
    def generate_sql(self, prompt: str, history: str, schemas: List[str] | None) -> str:
        ...
        if schemas:
            ...
            context = json.dumps(
                [schema for schema in self.context if schema["schema_name"] in schemas]
            )
            instructions = (
                user_instructions
                if user_instructions
            ...
            contents = (
                [context, instructions, history, prompt]
                if history
                else [context, instructions, prompt]
            )
