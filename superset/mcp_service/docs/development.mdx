...
> 🚀 **New to MCP?** Start with the [Overview](./overview.mdx) to understand what the service does before diving into development.
>
> 📚 **Need API examples?** Check the [API Reference](./api-reference.mdx) to see how existing tools work.
>
> 🔐 **Planning production use?** Review [Authentication](./authentication.mdx) for security considerations.
...
Test your implementations with examples from the [API Reference](./api-reference.mdx).

### 🔐 **Securing Your Extensions?**
Add authentication to your tools using the [Authentication Guide](./authentication.mdx).

### 🏗️ **Understanding the Big Picture?**
See the complete system design in the [Architecture Overview](./architecture.mdx).

### 🏢 **Building Enterprise Features?**
Explore advanced patterns in the [Preset Integration Guide](./preset-integration.mdx).
