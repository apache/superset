...
#
# Generic engine errors are deliberately excluded: drivers use that bucket for
# syntax and GROUP BY failures too, and calling those connectivity problems sends
# an agent toward credentials instead of fixing its chart configuration.
...
def _classify_as_database_error(exc: BaseException, dataset_id: int) -> bool:
    ...
    """
    # Only SQLAlchemy errors that explicitly report a disconnected/invalidated
    # connection are connectivity failures. OperationalError also represents
    # ordinary query errors and cannot be blanket-mapped.
