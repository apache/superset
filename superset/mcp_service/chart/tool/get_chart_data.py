...
async def get_chart_data(  # noqa: C901
    ...
    try:
        ...
        # Validator ensures identifier or form_data_key is set; the unsaved-only
        # branch above returned, so identifier must be non-None here.
        ...
        with event_logger.log_context(action="mcp.get_chart_data.chart_lookup"):
            await ctx.debug(
                "Performing chart lookup: identifier=%s" % (request.identifier,)
            )
