...
def _create_auth_provider(flask_app: Any) -> Any | None:
    ...
    if auth_factory := flask_app.config.get("MCP_AUTH_FACTORY"):
        try:
            ...
            # Auth was explicitly configured, so a provider that cannot be
            # built must fail closed instead of starting unauthenticated.
            logger.error("Failed to create auth provider from MCP_AUTH_FACTORY")
            raise MCPAuthConfigError(
                "MCP_AUTH_FACTORY raised while building the auth provider; "
                "refusing to start an unauthenticated MCP server. Fix the "
                "factory or unset MCP_AUTH_FACTORY."
            ) from None
