...
## 🚀 Using GitHub Codespaces with Claude Desktop

GitHub Codespaces provides a complete cloud development environment that works seamlessly with the MCP service and Claude Desktop. This is the fastest way to get started without any local setup.

### Prerequisites

1. **GitHub Account**: With access to create Codespaces
2. **Claude Desktop**: Installed on your local machine
3. **Branch with MCP**: Use a branch that includes the MCP service (e.g., `mcp_service_amin_dev`)

### Step-by-Step Setup

#### Step 1: Create a Codespace with MCP

1. **Navigate to the branch** containing the MCP service on GitHub
2. **Click the green "Code" button** → "Codespaces" tab
3. **Click the "..." menu** next to "Create codespace on branch"
4. **Select "New with options"**
5. **Choose Dev container configuration**: Select **"Apache Superset Development with MCP"**
6. **Create the Codespace**

> 💡 **Note**: The initial build takes 10-20 minutes. Subsequent starts are much faster due to caching.

#### Step 2: Wait for Services to Start

Once your Codespace opens:

1. **Check the terminal** - You'll see Superset starting automatically
2. **Monitor the PORTS tab** - Wait for ports to appear:
   - Port **9001**: Superset Frontend
   - Port **5008**: MCP Service
3. **Wait for "Running" status** on both ports (usually 5-10 minutes)

#### Step 3: Make MCP Port Public
...
5. **Copy the public URL** (hover over the address to see the copy button)

> ⚠️ **Security Note**: Making ports public is safe for development with sample data. Never expose production data this way.

#### Step 4: Configure Claude Desktop

1. **Get your MCP URL**:
   - Take the copied URL (e.g., `https://ubiquitous-giggle-7wpxgqj5w7hxwv4-5008.app.github.dev`)
   - **Add `/mcp` to the end**: `https://ubiquitous-giggle-7wpxgqj5w7hxwv4-5008.app.github.dev/mcp`
     ...
     "mcpServers": {
       "Superset Codespace": {
         ...
         "args": [
           ...
           "https://your-codespace-url-5008.app.github.dev/mcp"
         ]
       }
     }
   }
...
4. **Restart Claude Desktop**:
   - Quit Claude Desktop completely
   - Start it again
   - You should see "Superset Codespace" in the MCP servers list
...
"Show me the datasets available"
"Get superset instance info"
"Help me get started with Superset"
```

### Important Notes About Codespaces

#### URL Behavior
- **Superset links** generated by MCP tools (like explore URLs) will point to the internal Codespace URLs
- **To access these links**: Copy the path portion and append it to your Codespace's port 9001 URL
- **Example**: If MCP returns `http://localhost:9001/explore/`, use `https://your-codespace-9001.app.github.dev/explore/`

#### Performance Tips
- **First startup is slow** (10-20 minutes) - be patient
- **Keep Codespace running** to avoid rebuild times
- **Use the "Stop Codespace" option** instead of deleting to preserve state

#### Troubleshooting

**MCP Connection Issues**:
- Ensure port 5008 is public
- Verify the URL ends with `/mcp` (not `/mcp/`)
- Check that Superset has fully started (port 9001 is accessible)

**No Tools Available**:
- Restart Claude Desktop after configuration changes
- Verify the curl command syntax in your config
- Check the Codespace logs for MCP service errors

**Authentication Errors**:
- The Codespace MCP service has authentication disabled by default
- No tokens or credentials needed for development

### Alternative: FastMCP Proxy (Advanced)

If you prefer using the proxy script instead of curl:

1. **Install the proxy locally**:
   ```bash
   pip install fastmcp
   ```

2. **Create a proxy script** that points to your Codespace:
   ```bash
   #!/bin/bash
   fastmcp proxy https://your-codespace-5008.app.github.dev/mcp
   ```

3. **Use this script** in your Claude Desktop configuration instead of curl
