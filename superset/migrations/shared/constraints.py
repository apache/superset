...
def redefine_exact(
    ...
    """
    Redefine the foreign key constraint that exactly matches param foreign_key 
    to include the ON DELETE and ON UPDATE constructs for cascading purposes.
    ...
    with op.batch_alter_table(foreign_key.table, naming_convention=conv) as batch_op:
        # Find the exact FK constraint that matches both the local columns and the
        # referenced columns. The generic helper used in redefine() only matches on
        # the referenced columns/table which can return the first FK found and lead
        # to dropping the wrong constraint when a table has multiple FKs referencing
        # the same remote column (e.g. created_by_fk and changed_by_fk both
        # referencing ab_user.id).
        constraint = None
        for fk in insp.get_foreign_keys(foreign_key.table):
            if (
                fk["referred_table"] == foreign_key.referent_table
                and set(fk["referred_columns"]) == set(foreign_key.remote_cols)
                and set(fk["constrained_columns"]) == set(foreign_key.local_cols)
            ):
                constraint = fk["name"]
                break
