...
def upgrade():
    ...
    for dashboard in paginated_update(
        session.query(Dashboard)  # pylint: disable=too-many-nested-blocks
    ):
        try:
...
def downgrade():
    ...
    for dashboard in paginated_update(
        ...
        try:
            ...
            for slc in session.query(Slice).filter(Slice.id.in_(slice_ids)).all():
