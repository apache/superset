...
confirmed to have no successor). This one handles the other half of
...
view (``superset/views/core.py``) into per-resource ``ModelRestApi``s.

``PVM_MAP`` started as an 11-entry list. Two follow-up investigation passes
found: (1) 19 permissions originally believed to be pure deletions (in the
delete migration's earlier draft) actually have a verified live successor and
belong here instead; (2) the original ``can_get_or_create_table`` key was
itself a phantom -- no method named ``get_or_create_table`` was ever
generated by FAB, so it never matched any real historical permission and
would have silently no-op'd, leaving its intended target
(``can_sqllab_table_viz``, the real permission for ``def sqllab_table_viz`` /
route ``/get_or_create_table/``) to be deleted with no migration; and (3) two
more genuinely-real permissions (``can_annotation_json``,
``can_fave_dashboards_by_username``) were missing from both lists entirely.
All three are fixed below.

Two entries rest on strong-but-not-decorator-level evidence rather than a
formal ``@deprecated(new_target=...)`` marker: ``can_my_queries`` -> both
``SavedQuery.can_list`` (the old endpoint's redirect target, gating the page
shell) and ``SavedQuery.can_read`` (gating the ``/api/v1/saved_query/`` list
call the page actually renders -- without it, a migrated role gets a page
with no visible data) -- and
``can_import_dashboards`` -> ``Dashboard.can_write`` (a same-commit
functional replacement) -- flagged here for reviewer awareness, not hidden.
...
NEW_PVMS = {
    "Chart": ("can_read", "can_write"),
    "Query": ("can_read",),
    "Database": ("can_read", "can_write"),
    "Datasource": ("can_get_column_values",),
    "Log": ("can_recent_activity",),
    "Dataset": ("can_get_or_create_dataset", "can_read", "can_write"),
...
PVM_MAP = {
    ...
    Pvm(OLD_VIEW_MENU, "can_testconn"): (Pvm("Database", "can_write"),),
    ...
    Pvm(OLD_VIEW_MENU, "can_add_slices"): (Pvm("Chart", "can_write"),),
    ...
    Pvm(OLD_VIEW_MENU, "can_sqllab_viz"): (Pvm("Dataset", "can_write"),),
    Pvm(OLD_VIEW_MENU, "can_import_dashboards"): (Pvm("Dashboard", "can_write"),),
