...
def generate_focused_table(
    ...
    """
    Generate a focused table as a 2D list with databases as rows.
    ...
    Returns:
        Tuple of (2D list representing the table, list of excluded database names)
    """
    ...
    return [headers] + rows, excluded_dbs
...
def generate_feature_tables() -> list[list[list[Any]]]:
    """
    Generate multiple focused tables organized by feature categories.

    Returns a list of 2D lists, where each 2D list is a table.
    """
    info = {}
    ...
    for db_name, db_info in sorted_info.items():
        ...
        rows.append(row)
    output_tables.append([headers] + rows)
    ...
    table, _ = generate_focused_table(
        info,
        feature_keys=common_grains_keys,
        column_labels=common_grains_keys,
        value_extractor=extract_time_grain,
    )
    output_tables.append(table)

    # Table 5: Time Grains – Extended
    extended_grains_keys = [
        "FIVE_SECONDS",
        "THIRTY_SECONDS",
        "FIVE_MINUTES",
        "TEN_MINUTES",
    ...
    table, _ = generate_focused_table(
        info,
        feature_keys=extended_grains_keys,
        column_labels=extended_grains_keys,
        value_extractor=extract_time_grain,
    )
    output_tables.append(table)
...
def format_row(row: list[Any]) -> str:
    """
    Format a row for printing and quote everything to keep it docusaurus compatible.
    """
    formatted_items = []
    for item in row:
...
if __name__ == "__main__":
    ...
    with app.app_context():
        tables = generate_feature_tables()
