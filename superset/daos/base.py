...
class BaseDAO(Generic[T]):
    ...
    def list(
        ...
        datamodel = SQLAInterface(cls.model_cls, db.session)

        # Start with base query
        query = datamodel.session.query(cls.model_cls)

        # Apply base filter if defined
        if cls.base_filter:
            query = cls.base_filter(  # pylint: disable=not-callable
                cls.id_column_name, datamodel
            ).apply(query, None)
        ...
        if custom_filters:
            for filter_name, filter_class in custom_filters.items():
                if hasattr(cls.model_cls, filter_name):
                    custom_filter = filter_class(filter_name, datamodel)
