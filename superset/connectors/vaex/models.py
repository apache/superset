...
import pandas
# from pyvaex.client import PyVaex
# from pyvaex.utils.aggregators import count
# from pyvaex.utils.dimensions import MapLookupExtraction, RegexExtraction
# from pyvaex.utils.filters import Dimension, Filter
# from pyvaex.utils.having import Aggregation
# from pyvaex.utils.postaggregator import (
...
class VaexColumn(Model, BaseColumn):
    ...
    def refresh_metrics(self):
        ...
        dbmetrics = (
            db.session.query(VaexMetric)
            .filter(VaexMetric.datasource_id == self.datasource_id)
            .filter(VaexMetric.metric_name.in_(metrics.keys()))
        )
...
class VaexDatasource(Model, BaseDatasource):
    ...
    def import_obj(cls, i_datasource, import_time=None):
        ...
        def lookup_datasource(d):
            return db.session.query(VaexDatasource).filter(
                VaexDatasource.datasource_name == d.datasource_name,
                VaexCluster.cluster_name == d.cluster_name,
            ).first()

        def lookup_cluster(d):
            return db.session.query(VaexCluster).filter_by(
    ...
    def latest_metadata(self):
        ...
        # client = self.cluster.get_pyvaex_client()
        # try:
        ...
        # realtime segments, which triggered a bug (fixed in vaex 0.8.2).
        # https://groups.google.com/forum/#!topic/vaex-user/gVCqqspHqOQ
        lbound = (max_time - timedelta(days=7)).isoformat()
        # if LooseVersion(self.cluster.vaex_version) < LooseVersion('0.8.2'):
    ...
    def sync_to_db_from_config(
            ...
            cluster,
            refresh=True):
        """Merges the ds config from vaex_config into one stored in the db."""
        session = db.session
        datasource = (
            session.query(cls)
            .filter_by(datasource_name=vaex_config['name'])
            .first()
        )
        # Create a new datasource.
        if not datasource:
            datasource = cls(
                datasource_name=vaex_config['name'],
                cluster=cluster,
                owner=user,
        ...
        dimensions = vaex_config['dimensions']
        col_objs = (
            session.query(VaexColumn)
            .filter(VaexColumn.datasource_id == datasource.id)
            .filter(VaexColumn.column_name.in_(dimensions))
        )
        ...
        # Import Vaex metrics
        metric_objs = (
            session.query(VaexMetric)
            .filter(VaexMetric.datasource_id == datasource.id)
            .filter(VaexMetric.metric_name.in_(
                spec['name'] for spec in vaex_config['metrics_spec']
            ))
        )
        metric_objs = {metric.metric_name: metric for metric in metric_objs}
        for metric_spec in vaex_config['metrics_spec']:
    ...
    # http://vaex.io/docs/0.8.0/querying/granularities.html
    ...
    def get_post_agg(mconf):
        ...
        kind of post aggregation for pyvaex.
        """
    ...
    def recursive_get_fields(_conf):
        ...
        if _field:
            field_names += VaexDatasource.recursive_get_fields(_field)
        if _fields:
            for _f in _fields:
                field_names += VaexDatasource.recursive_get_fields(_f)
    ...
    def resolve_postagg(postagg, post_aggs, agg_names, visited_postaggs, metrics_dict):
        ...
        required_fields = set(
            VaexDatasource.recursive_get_fields(mconf) +
        ...
        if len(required_fields) > 0:
            missing_postaggs = VaexDatasource.find_postaggs_for(
            ...
            for missing_postagg in missing_postaggs:
                VaexDatasource.resolve_postagg(
                    missing_postagg, post_aggs, agg_names, visited_postaggs, metrics_dict)
        post_aggs[postagg.metric_name] = VaexDatasource.get_post_agg(postagg.json_obj)

    @staticmethod
    def metrics_and_post_aggs(metrics, metrics_dict, vaex_version=None):
        ...
        aggs = VaexDatasource.get_aggregations(
            metrics_dict,
            saved_agg_names,
            adhoc_agg_configs,
        )
    ...
    def values_for_column(self,
        ...
        # TODO: Use Lexicographic TopNMetricSpec once supported by PyVaex
        ...
        if df is not None and not df.empty:
            ...
            for unused, row in df.iterrows():
                ...
                for dim in dimensions:
                    ...
                    # If so, create the appropriate pyvaex extraction object
                    if isinstance(dim, dict) and 'extractionFn' in dim:
                        (col, extraction_fn) = VaexDatasource._create_extraction_fn(dim)
    ...
    @staticmethod
    def vaex_type_from_adhoc_metric(adhoc_metric):
    ...
    def get_aggregations(metrics_dict, saved_metrics, adhoc_metrics=[]):
        ...
        for adhoc_metric in adhoc_metrics:
            aggregations[adhoc_metric['label']] = {
                ...
                'type': VaexDatasource.vaex_type_from_adhoc_metric(adhoc_metric),
    ...
    def run_query(  # noqa / vaex
        ...
        """Runs a query against Vaex and returns a dataframe.
        """
        # TODO refactor into using a TBD Query object
        client = client or self.cluster.get_pyvaex_client()
        ...
        if (
            self.cluster and
            LooseVersion(self.cluster.get_vaex_version()) < LooseVersion('0.11.0')
        ):
            for metric in metrics:
                self.sanitize_metric_object(metric)
            self.sanitize_metric_object(timeseries_limit_metric)

        aggregations, post_aggs = VaexDatasource.metrics_and_post_aggs(
            metrics,
            metrics_dict)
        ...
        qry = dict(
            ...
            granularity=VaexDatasource.granularity(
                granularity,
                timezone=timezone,
                origin=extras.get('vaex_time_origin'),
            ),
            post_aggregations=post_aggs,
            intervals=self.intervals_from_dttms(from_dttm, to_dttm),
        )

        filters = VaexDatasource.get_filters(filter, self.num_cols, columns_dict)
        if filters:
            qry['filter'] = filters

        having_filters = self.get_having_filters(extras.get('having_vaex'))
        if having_filters:
        ...
        if columns:
            ...
            if timeseries_limit_metric:
                ...
                aggs_dict, post_aggs_dict = VaexDatasource.metrics_and_post_aggs(
    ...
    def homogenize_types(df, groupby_cols):
        ...
        When grouping by a numeric (say FLOAT) column, pyvaex returns
    ...
    def query(self, query_obj):
        ...
        # time_offset = VaexDatasource.time_offset(query_obj['granularity'])
    ...
    def get_filters(cls, raw_filters, num_cols, columns_dict):  # noqa
        """Given Superset filter data structure, returns pyvaex Filter(s)"""
        filters = None
        for flt in raw_filters:
            ...
            extraction_fn = None
            if dim_spec and 'extractionFn' in dim_spec:
                (col, extraction_fn) = VaexDatasource._create_extraction_fn(dim_spec)
            ...
            # For the ops below, could have used pyvaex's Bound,
            # but it doesn't support extraction functions
            elif op == '>=':
                cond = Filter(
                    type='bound',
