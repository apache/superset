...
from superset.common.utils import df_metrics_to_num, left_join_df, QueryCacheManager
...
class QueryContextProcessor:
    ...
    def get_df_payload(
        ...
        cache_key = self.query_cache_key(query_context.datasource, query_obj)
        cache = QueryCacheManager.get(
            cache_key, CacheRegion.DATA, query_context.force, force_cached,
        )
        ...
        if query_obj and cache_key and not cache.is_loaded:
            try:
                invalid_columns = [
                    ...
                    if (
                        col not in query_context.datasource.column_names
                        and col != DTTM_ALIAS
                    )
                ]
                if invalid_columns:
                ...
                query_result = self.get_query_result(query_context, query_obj)
                annotation_data = self.get_annotation_data(query_context, query_obj)
                cache.set_query_result(
                    key=cache_key,
                    query_result=query_result,
                    annotation_data=annotation_data,
                    force_query=query_context.force,
                    timeout=self.get_cache_timeout(query_context),
        ...
        return {
            ...
            "cache_timeout": self.get_cache_timeout(query_context),
            "df": cache.df,
    ...
    def query_cache_key(
        self, datasource: BaseDatasource, query_obj: QueryObject, **kwargs: Any
    ) -> Optional[str]:
        """
    ...
    def get_query_result(
        self, query_context: QueryContext, query_object: QueryObject
    ) -> QueryResult:
        ...
        # parsing logic
        if not df.empty:
            df = self.normalize_df(query_context.datasource, df, query_object)
    ...
    def normalize_df(
        self, datasource: BaseDatasource, df: pd.DataFrame, query_object: QueryObject
    ) -> pd.DataFrame:
    ...
    def processing_time_offsets(  # pylint: disable=too-many-locals
        self, query_context: QueryContext, df: pd.DataFrame, query_object: QueryObject,
    ) -> CachedTimeOffset:
        ...
        for offset in time_offsets:
            ...
            cache_key = self.query_cache_key(
                query_context.datasource, query_object_clone, time_offset=offset
            )
            cache = QueryCacheManager.get(
            ...
            result = query_context.datasource.query(query_object_clone_dct)
            ...
            if offset_metrics_df.empty:
                ...
                offset_metrics_df = self.normalize_df(
                    query_context.datasource, offset_metrics_df, query_object_clone
                )
    ...
    @staticmethod
    def get_data(
        result_format: ChartDataResultFormat, df: pd.DataFrame
    ...
    def get_payload(
        ...
        if cache_query_context:
            cache_key = self.cache_key(query_context)
            set_and_log_cache(
                cache_manager.cache,
                cache_key,
                {"data": query_context.cache_values},
                self.get_cache_timeout(query_context),
            )
            return_value["cache_key"] = cache_key  # type: ignore

        return return_value

    def get_cache_timeout(self, query_context: QueryContext) -> int:
        cache_timeout_rv = query_context.get_cache_timeout()
        if cache_timeout_rv:
            return cache_timeout_rv
        return config["CACHE_DEFAULT_TIMEOUT"]

    def cache_key(self, query_context: QueryContext, **extra: Any) -> str:
        """
        ...
        """
        key_prefix = "qc-"
        cache_dict = query_context.cache_values.copy()
        cache_dict.update(extra)

        return generate_cache_key(cache_dict, key_prefix)

    def get_annotation_data(
        self, query_context: QueryContext, query_obj: QueryObject
    ) -> Dict[str, Any]:
        """
        :param query_context:
        :param query_obj:
        ...
        for annotation_layer in [
            ...
            annotation_data[name] = self.get_viz_annotation_data(
                annotation_layer, query_context.force
            )
    ...
    def raise_for_access(self, query_context: QueryContext) -> None:
        """
