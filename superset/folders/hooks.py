...
def folder_access_charts(user_id: int) -> Any:
    """Return select() of chart IDs accessible via folder membership."""
    from superset.folders.models import FolderObject

    return select(FolderObject.chart_id).where(
...
def folder_access_dashboards(user_id: int) -> Any:
    """Return select() of dashboard IDs accessible via folder membership."""
    from superset.folders.models import FolderObject

    return select(FolderObject.dashboard_id).where(
...
def folder_extra_owners(resource: Any) -> list[Any]:
    ...
    from superset.folders.models import folder_editors, FolderObject
    ...
    users = db.session.query(User).filter(User.id.in_(editor_ids)).all()
    return [
        {
            "id": u.id,
