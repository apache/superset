...
def _serialize_subjects(subjects: Any) -> list[dict[str, Any]]:
    return [
...
def serialize_rows_with_paths(
    rows: list[ResolvedAsset],
    current_folder_id: int | None,
) -> list[dict[str, Any]]:
    """Serialize rows and attach folder_path for items from child folders."""
    # Batch-resolve folder paths for assets
    asset_ids: dict[str, list[int]] = {}
    for kind, obj in rows:
        if kind != "folder":
            asset_ids.setdefault(kind, []).append(obj.id)
    asset_paths = FolderDAO.get_asset_folder_paths(asset_ids) if asset_ids else {}

    # Resolve current folder UUID once, not per row
    current_uuid = None
    if current_folder_id is not None:
        current_folder = db.session.get(Folder, current_folder_id)
        current_uuid = str(current_folder.uuid) if current_folder else None

    result = []
    for kind, obj in rows:
        path = None
        if kind == "folder" and obj.parent_id != current_folder_id:
            # Folder from a deeper level — show path to its parent (not itself)
            path = FolderDAO.get_folder_path(obj.parent_id) if obj.parent_id else None
        elif kind != "folder":
            full_path = asset_paths.get((kind, obj.id))
            if full_path:
                asset_folder_uuid = full_path[-1]["uuid"] if full_path else None
                if asset_folder_uuid != current_uuid:
                    path = full_path
        result.append(serialize_row(kind, obj, path))
    return result
...
def serialize_asset(asset_type: str, asset: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "editors": _serialize_subjects(getattr(asset, "editors", [])),
        "viewers": _serialize_subjects(getattr(asset, "viewers", [])),
...
def _parse_rison_args(rison_args: dict[str, Any]) -> dict[str, Any]:
    ...
    Expected rison format (matches ``useListViewResource``)::

        {
          ...
          "filters": [
            ...
            {"col": "editors", "opr": "rel_m_m", "value": 1},
    ...
    for f in rison_args.get("filters", []):
        ...
        elif col == "editors" and opr == "rel_m_m":
            editors = result["editors"] or []
            editors.append(int(val))
            result["editors"] = editors
        elif col == "viewers" and opr == "rel_m_m":
            viewers = result["viewers"] or []
            viewers.append(int(val))
...
class FolderRestApi(BaseSupersetApi):
    ...
    @statsd_metrics
    @event_logger.log_this_with_context(
        action=lambda self, *args, **kwargs: f"{self.__class__.__name__}.delete_assets",
        log_to_statsd=False,
    )
    @transaction()
