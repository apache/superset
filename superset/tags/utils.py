...
from superset.tags.models import ObjectTypes, TagTypes, TaggedObject, Tag, get_tag
...
def delete_custom_object_tags(
    ...
    tags_to_delete = db.session.query(Tag).filter(
        Tag.name.in_(tags if tags else []),
    ...
    for tag in tags_to_delete:
        tag_objs = db.session.query(TaggedObject).filter(
