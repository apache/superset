...
from superset.datamanage.commands.exceptions import DatamanageInvalidError
...
class ImportDatamanageCommand(BaseCommand):
    """
    ...
    def run(self) -> None:
        ...
        for file_name, config in self._configs.items():
            logger.info("Importing datamanage from file %s", file_name)
            if isinstance(config, dict):
                import_from_dict(db.session, config, sync=self.sync)
            else:  # list
                for datamanage in config:
                    # UI exports don't have the database metadata, so we assume
                    # the DB exists and has the same name
                    params = json.loads(datamanage["params"])
                    database = (
                        db.session.query(Database)
                        .filter_by(database_name=params["database_name"])
                        .one()
                    )
                    datamanage["database_id"] = database.id
                    SqlaTable.import_from_dict(db.session, datamanage, sync=self.sync)
