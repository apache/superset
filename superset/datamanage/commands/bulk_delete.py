...
class BulkDeleteDatamanageCommand(BaseCommand):
    ...
    def run(self) -> None:
        ...
        if not self._models:
            return None
        try:
            DatamanageDAO.bulk_delete(self._models)
            for model in self._models:
                ...
                if view_menu:
                    ...
                    if not view_menu:
                        logger.error(
                            "Could not find the data access permission for the Datamanage",
                            exc_info=True,
                        )
            db.session.commit()
            ...
            raise DatamanageBulkDeleteFailedError() from ex

    def validate(self) -> None:
        # Validate/populate model exists
        self._models = DatamanageDAO.find_by_ids(self._model_ids)
