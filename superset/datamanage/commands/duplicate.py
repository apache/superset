...
from superset.datamanage.commands.exceptions import (
    DatamanageDuplicateFailedError,
...
class DuplicateDatamanageCommand(CreateMixin, BaseCommand):
    ...
    def run(self) -> Model:
        ...
        try:
            ...
            db.session.rollback()
            raise DatamanageDuplicateFailedError() from ex
        return table
    ...
    def validate(self) -> None:
        ...
        base_model = DatamanageDAO.find_by_id(base_model_id)
        if not base_model:
            exceptions.append(DatamanageNotFoundError())
        else:
        ...
        if DatamanageDAO.find_one_or_none(table_name=duplicate_name):
            exceptions.append(DatamanageExistsValidationError(table_name=duplicate_name))
