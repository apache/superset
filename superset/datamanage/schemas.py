...
from superset.datamanage.models import Datamanage
...
class DatamanageColumnsPutSchema(Schema):
    id = fields.Integer()
...
class DatamanageMetricsPutSchema(Schema):
    id = fields.Integer()
...
class DatamanagePostSchema(Schema):
...
class DatamanagePutSchema(Schema):
    ...
    owners = fields.List(fields.Integer())
    columns = fields.List(fields.Nested(DatamanageColumnsPutSchema))
    metrics = fields.List(fields.Nested(DatamanageMetricsPutSchema))
    extra = fields.String(allow_none=True)
    is_managed_externally = fields.Boolean(allow_none=True, default=False)
    external_url = fields.String(allow_none=True)


class DatamanageDuplicateSchema(Schema):
    base_model_id = fields.Integer(required=True)
    table_name = fields.String(required=True, allow_none=False, validate=Length(1, 250))


class DatamanageRelatedChart(Schema):
    id = fields.Integer()
    slice_name = fields.String()
    viz_type = fields.String()


class DatamanageRelatedDashboard(Schema):
    id = fields.Integer()
...
class DatamanageRelatedCharts(Schema):
    count = fields.Integer(description="Chart count")
    result = fields.List(
        fields.Nested(DatamanageRelatedChart), description="A list of dashboards"
    )


class DatamanageRelatedDashboards(Schema):
    count = fields.Integer(description="Dashboard count")
    result = fields.List(
        fields.Nested(DatamanageRelatedDashboard), description="A list of dashboards"
    )


class DatamanageRelatedObjectsResponse(Schema):
    charts = fields.Nested(DatamanageRelatedCharts)
    dashboards = fields.Nested(DatamanageRelatedDashboards)
...
class ImportV1DatamanageSchema(Schema):
    # pylint: disable=no-self-use, unused-argument
    @pre_load
    def fix_extra(self, data: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        """
...
class DatamanageSchema(SQLAlchemyAutoSchema):
    """
    Schema for the ``Datamanage`` model.
    """
