...
# Test MCP Proxy Endpoints - Mirror of test_mcp_all.sh but through Superset proxy

echo "====================================="
echo "  SUPERSET MCP PROXY TEST SUITE"
echo "====================================="
...
echo -e "\n2️⃣ LISTING AVAILABLE TOOLS via PROXY..."
echo "--------------------------------"
curl -s -X POST ${MCP_PROXY_URL} \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
echo -e "\n3️⃣ CALLING list_datasets TOOL via PROXY..."
echo "--------------------------------"
...
echo -e "\n4️⃣ GETTING INSTANCE INFO via PROXY..."
echo "--------------------------------"
RESPONSE=$(curl -s -X POST ${MCP_PROXY_URL} \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
