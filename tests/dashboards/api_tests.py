...
class TestDashboardApi(SupersetTestCase, ApiOwnersTestCaseMixin):
    ...
    def test_get_dashboard(self):
        ...
        dashboard = dashboard_utils.insert_dashboard(
            expected_title, expected_slug, [admin.id], admin
        )
    ...
    def test_update_dashboard_chart_owners(self):
        ...
        slices.append(
            db.session.query(Slice).filter_by(slice_name="Trends").first()
        )
        slices.append(
            db.session.query(Slice).filter_by(slice_name="Boys").first()
        )
