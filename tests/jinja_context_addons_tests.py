...
class Jinja2ContextAddonsTest(SupersetTestCase):
    ...
    def test_render_ipv4_column_template(self) -> None:
        test_filter = [
            {"col": "src_ip_num", "op": "==", "val": "1.1.1.1"},
            {"col": "src_ip_num", "op": "IN", "val": ["3.3.3.3", "2.2.2.2"]},
        ]
        rendered = render_ipv4_number_column(test_filter, "src_num_ip")
        self.assertEqual(
            " AND (src_num_ip = 16843009) AND ((src_num_ip = 50529027) OR (src_num_ip = 33686018))",
            rendered,
        )

    def test_render_ipv4_either_number_columns_template(self) -> None:
        test_filter = [
            {"col": "src_ip_num", "op": "==", "val": ["3.0.0.0/8", "2.2.2.2"]}
        ]
        rendered = render_ipv4_either_number_columns(
            test_filter, "src_num_ip", "dst_num_ip"
        )
        self.assertEqual(
            " AND ((src_num_ip >= 50331648 AND src_num_ip <= 67108863) OR (src_num_ip = 33686018) OR (dst_num_ip >= 50331648 AND dst_num_ip <= 67108863) OR (dst_num_ip = 33686018))",
            rendered,
        )

    def test_render_ipv4_between_number_colums_template(self) -> None:
        test_filter = [
            {"col": "src_ip_num", "op": "LIKE", "val": ["12.0.0.0/8", "2.0.0.0/16"]}
        ]
    ...
    def test_render_in_conditions_template(self) -> None:
        ...
        self.assertEqual(
            [
                "(src_num_ip = 16843009)",
                "(src_num_ip >= 4026531840 AND src_num_ip <= 4294967295)",
            ],
            rendered,
        )
    ...
    def test_dashboard_link_template(self) -> None:
        ...
        rendered = dashboard_link(
            test_link_label, test_dashboard_id, test_src_column, test_target_column
        )
        self.assertEqual(
            " concat('<a href=\"http://10.162.232.22:8088/superset/dashboard/2301/?preselect_filters={%22160%22:{%22target_col%22:[%22', target_col, '%22]}}\">LABEL</a>' ) ",
            rendered,
        )
    ...
    def test_render_ipv4_column_template_exception(self) -> None:
        ...
        test_filter = [
            {"col": "src_ip_num", "op": "==", "val": "1.1.1.1"},
            {"col": "src_ip_num", "op": "IN", "val": ["3.3.3.3/8"]},
        ]
        self.assertRaises(
            ValueError, render_ipv4_number_column, test_filter, "src_ip_num"
        )

    def test_render_ipv4_either_number_columns_template_invalid_cidr(self) -> None:
        # Invalid error cidr comes from 2.2.2.200/34
        test_filter = [
            {"col": "src_ip_num", "op": "==", "val": ["3.0.0.0/8", "2.2.2.200/34"]}
        ]
        self.assertRaises(
            NetmaskValueError,
            render_ipv4_either_number_columns,
            test_filter,
            "src_num_ip",
            "dst_num_ip",
        )
    ...
    def test_render_ipv4_between_number_colums_template_invalid_arguments(self) -> None:
        ...
        self.assertRaises(
            ValueError,
            render_ipv4_between_number_colums,
            test_filter,
            "1.1.1.1",
            "2.2.2.2",
        )
    ...
    def test_render_in_conditions_template_invalid_cidr(self) -> None:
        ...
        self.assertRaises(
            AddressValueError, render_in_conditions, test_ip_array, "src_num_ip"
        )
