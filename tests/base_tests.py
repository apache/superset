...
class SupersetTestCase(TestCase):
    ...
    def get_query_context_payload(
        self,
        table_name: str = "birth_names",
        query_objects: Optional[List[Dict[str, Any]]] = None,
    ) -> Dict[str, Any]:
        """
        Create a request payload for constructing a QueryContext object.
        By default returns a payload corresponding to one generated by the
        "Boy Name Cloud" chart in the examples.

        :param table_name: name of datasource
        :param query_objects: query objects
        :return: Request payload
        """
        self.login(username="admin")
        table = self.get_table_by_name(table_name)
        payload = get_query_context(
            datasource_id=table.id, datasource_type=table.type, queries=query_objects,
        )
        return payload

    def get_query_context_payload_with_post_processing(self) -> Dict[str, Any]:
        """
        Create a request payload with post transformation operations.

        :return: Request payload
        """
        query = get_query_object()
        query["post_processing"] = get_postprocessing_operation()
        return self.get_query_context_payload(query_objects=[query])
