...
class SupersetTestCase(TestCase):
    ...
    # ============================================================================
    # These methods help create idempotent tests that can run multiple times
    # without conflicts. Choose between:
    #
    ...
    #           # chart_data = {"slice_name": "name1", ...}  # May conflict!
    #
    ...
    def upsert_dashboard_by_slug(self, slug: str, **kwargs):
        """Upsert (update or insert) dashboard by slug to avoid test conflicts

        If a dashboard with the slug exists, it will be UPDATED with new kwargs.
        If not, a new dashboard will be created.

        Args:
            slug: Dashboard slug (unique identifier)
            **kwargs: Dashboard attributes to set/update

        Returns:
            Dashboard: The dashboard object (existing updated or newly created)

        Example:
            # First call creates dashboard
            dash1 = self.upsert_dashboard_by_slug(
                "test-dash", dashboard_title="Title 1"
            )

            # Second call updates existing dashboard
            dash2 = self.upsert_dashboard_by_slug(
                "test-dash", dashboard_title="Title 2"
            )
            assert dash1.id == dash2.id  # Same object
            assert dash2.dashboard_title == "Title 2"  # Updated
        """
        return self.get_or_create(Dashboard, {"slug": slug}, **kwargs)

    def upsert_chart_by_name(self, slice_name: str, **kwargs):
        """Upsert (update or insert) chart by name to avoid test conflicts

        If a chart with the name exists, it will be UPDATED with new kwargs.
        If not, a new chart will be created.

        Args:
            slice_name: Chart name (unique identifier)
            **kwargs: Chart attributes to set/update

        Returns:
            Slice: The chart object (existing updated or newly created)
        """
        return self.get_or_create(Slice, {"slice_name": slice_name}, **kwargs)

    def upsert_database_by_name(self, database_name: str, **kwargs):
        """Upsert (update or insert) database by name to avoid test conflicts

        If a database with the name exists, it will be UPDATED with new kwargs.
        If not, a new database will be created.

        Args:
            database_name: Database name (unique identifier)
            **kwargs: Database attributes to set/update

        Returns:
            Database: The database object (existing updated or newly created)
        """
        return self.get_or_create(
            models.Database, {"database_name": database_name}, **kwargs
        )

    def get_or_create_dashboard_by_slug(self, slug: str, **kwargs):
        """Get existing dashboard by slug or create new one (no updates to existing)

        If a dashboard with the slug exists, it will be returned UNCHANGED.
        If not, a new dashboard will be created.

        Args:
            slug: Dashboard slug (unique identifier)
            **kwargs: Dashboard attributes (only used for creation)

        Returns:
            Dashboard: The dashboard object (existing or newly created)

        Example:
            # First call creates dashboard
            dash1 = self.get_or_create_dashboard_by_slug(
                "test-dash", dashboard_title="Title 1"
            )

            # Second call returns existing unchanged
            dash2 = self.get_or_create_dashboard_by_slug(
                "test-dash", dashboard_title="Title 2"
            )
            assert dash1.id == dash2.id  # Same object
            assert dash2.dashboard_title == "Title 1"  # Unchanged
        """
    ...
    def get_or_create_chart_by_name(self, slice_name: str, **kwargs):
        """Get existing chart by name or create new one (no updates to existing)

        If a chart with the name exists, it will be returned UNCHANGED.
        If not, a new chart will be created.

        Args:
            slice_name: Chart name (unique identifier)
            **kwargs: Chart attributes (only used for creation)

        Returns:
            Slice: The chart object (existing or newly created)
        """
    ...
    def get_or_create_database_by_name(self, database_name: str, **kwargs):
        """Get existing database by name or create new one (no updates to existing)

        If a database with the name exists, it will be returned UNCHANGED.
        If not, a new database will be created.

        Args:
            database_name: Database name (unique identifier)
            **kwargs: Database attributes (only used for creation)

        Returns:
            Database: The database object (existing or newly created)
        """
        existing = (
