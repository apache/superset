...
# under the License.
"""Integration coverage for the time-based soft-delete purge (sc-111185).
...
class TestSoftDeletePurge(DeletionRetentionTestBase):
    def test_aged_out_purged_in_window_and_active_preserved(self) -> None:
        """SC-PURGE-001: rows past the window are purged; in-window
    ...
    def test_window_zero_disables(self) -> None:
        """SC-PURGE-003: window 0 disables the time-based purge."""
        aged = self.make_chart("aged")
        aged_id = aged.id
        self.soft_delete(aged, days_ago=90)
    ...
    def test_dry_run_removes_nothing(self) -> None:
        """SC-PURGE-009: dry-run reports would_purge but deletes nothing."""
        aged = self.make_chart("aged")
        aged_id = aged.id
        self.soft_delete(aged, days_ago=90)
    ...
    def test_purging_chart_unlinks_live_dashboard_but_keeps_it(self) -> None:
        """SC-PURGE-005 / C14: purging a chart removes its dashboard_slices
    ...
    def test_purging_dashboard_preserves_its_charts(self) -> None:
        """SC-PURGE-005: purging a dashboard does not remove the
    ...
    def test_purging_dataset_removes_children_and_permission(self) -> None:
        """C18 / C20: a dataset's owned columns + metrics and its datasource
    ...
    def test_purging_dataset_leaves_referencing_chart_dangling(self) -> None:
        """C21 / SC-PURGE-008: no dependent guard — a soft-deleted dataset is
        purged even with a live chart referencing it; the chart is left
    ...
    def test_tags_removed_on_purge(self) -> None:
        """C14 / C20: the entity's tagged_object rows are removed (the
    ...
    def test_cascade_with_fk_enforcement_off(self) -> None:
        """C10 / review B1: the explicit sa.delete cascade — not DB FK cascade
    ...
    def test_version_history_removed_and_shared_tx_preserved(self) -> None:
        """Stage 2 / C12 / ⚠️ B2: a purged entity's version shadows + scoped
    ...
    def test_version_tables_absent_noop(self) -> None:
        """T024 / Stage 1 merge gate: when the version tables are absent the
