...
async def main():
    ...
    async with Client("http://localhost:5008/mcp") as client:
        ...
        await test_tool(
            client,
            "list_datasets",
            {"filters": [{"col": "table_name", "opr": "sw", "value": "ab"}]},
            "(table_name sw 'ab')",
            issues,
        )
        # Test list_datasets with pagination
        ...
        await test_tool(
            client, "get_dataset_info", {"dataset_id": 999999}, "(invalid id)", issues
        )
    ...
    if issues:
        ...
        logger.info(
            "All list_datasets and list_datasets calls returned successfully with no "
            "errors or warnings."
        )
