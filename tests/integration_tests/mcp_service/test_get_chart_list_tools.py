...
async def main():
    ...
    async with Client("http://localhost:5008/mcp") as client:
        ...
        await test_tool(client, "list_charts", {"filters": [{"col": "slice_name", "opr": "sw", "value": "ab"}]}, "(slice_name sw 'ab')", issues)
        # Test list_charts with pagination
        await test_tool(client, "list_charts", {"page": 1, "page_size": 2}, "(page=1, page_size=2)", issues)

        # Test get_chart_info with a likely invalid ID (should return error)
        await test_tool(client, "get_chart_info", {"chart_id": 999999}, "(invalid id)", issues)

        # Test get_chart_available_filters
        await test_tool(client, "get_chart_available_filters", {}, "(no params)", issues)
