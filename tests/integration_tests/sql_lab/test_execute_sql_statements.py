...
from superset import db, results_backend
...
def test_results_backend_write_failure(
    ...
    backend = results_backend._get_current_object()
    with (
        patch.object(backend, "set", return_value=False) as mock_set,
        pytest.raises(SupersetErrorException) as exc_info,
    ):
...
def test_results_backend_write_success(
    ...
    backend = results_backend._get_current_object()
    with patch.object(backend, "set", return_value=True) as mock_set:
...
def test_results_backend_write_failure_sync_mode(
    ...
    backend = results_backend._get_current_object()
    with patch.object(backend, "set", return_value=False) as mock_set:
