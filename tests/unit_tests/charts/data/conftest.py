...
from superset.charts.data.query_context_validator import (
    QueryContextValidator,
    QueryContextValidatorImpl,
)
...
@fixture
def internal_table_name() -> str:
    return "ab_user"


@fixture
def metric_for_internal_table_query_context(
    datasource: MockDatasource, internal_table_name: str, processor: Mock
) -> QueryContext:
    generator = QueryContextGenerator()
    query_obj = generator.generate_query_object(datasource.name, False, False)
    query_obj["metrics"].append(
        generator.generate_sql_expression_metric("name", internal_table_name)
    )
    return create_query_context(
        datasource, [QueryObject(datasource=None, **query_obj)], processor,
    )
...
def validator(
    ...
    return QueryContextValidatorImpl(dataset_dao_mock, security_manager_mock)
