...
from tests.unit_tests.charts.data.conftest import (
...
class TestQueryContextValidatorImpl:
    class TestWithSqlDatabase:
        class Base:
            validator: QueryContextValidatorImpl
            ...
            def set_validator(self, validator: QueryContextValidatorImpl):
            ...
            def set_dataset_dao_mock(self, dataset_dao_mock: Mock):
                ...
                dataset_dao_mock.get_by_sql_database_components.side_effect = (
                    side_effect
                )
                self.dataset_dao_mock = dataset_dao_mock

            @fixture
            def arrange_database_permission(
                self, database
        ...
        class TestWithoutMetric(Base):
            ...
            def test_when_actor_has_all_database_permission__validation_success(self):
                # act
                self.validator.validate(self.query_context)
        ...
        class TestWithSqLMetric(Base):
            ...
            def test_when_metric_is_queried_from_internal_table__exception_is_raised(
                self,
                metric_for_internal_table_query_context,
                internal_table_name,
                database,
            ):

                with pytest.raises(Exception) as ex:
                    self.validator.validate(metric_for_internal_table_query_context)
                    assert ex.__name__ == "SupersetSecurityException"
                ...
                self.security_manager_mock.raise_for_access.assert_not_called()
                self.dataset_dao_mock.get_by_sql_database_components.assert_called_once_with(
                    database, internal_table_name, None
                )
