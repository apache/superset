...
def test_validate_predicate_rejects_set_operations():
    from superset.models.helpers import validate_adhoc_subquery

    mock_db = MagicMock()
    with pytest.raises(SupersetSecurityException) as ex:
        validate_adhoc_subquery(
            "status = 'active' UNION SELECT id FROM other_table",
            mock_db,
            None,
            "public",
            "postgresql",
...
def test_validate_adhoc_subquery_preserves_jinja_on_non_predicate():
    """Non-predicate callers receive the original SQL unchanged when RLS is applied."""
...
def test_validate_predicate_rejects_multiple_statements():
    from superset.models.helpers import validate_adhoc_subquery
    ...
    assert "multi-statement" in str(ex.value)
