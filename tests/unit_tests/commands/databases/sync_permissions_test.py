...
def test_sync_permissions_command_validate(mocker: MockerFixture):
    """
...
@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_validate_passing_all_values(mocker: MockerFixture):
    """
...
@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_validate_raise(mocker: MockerFixture):
    """
...
@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_run(mocker: MockerFixture):
    """
    ...
    cmmd = SyncPermissionsCommand(
        1,
        old_db_connection_name="same_name",
        db_connection=mock_db,
        ssh_tunnel=None,
    )
    mocker.patch.object(
...
@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_run_new_db_name(mocker: MockerFixture):
    """
    ...
    mock_rename_db_perm.assert_called_once_with("catalog", ["schema"])


@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_run_no_catalog(mocker: MockerFixture):
    """
...
@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_run_no_catalog_new_db_name(mocker: MockerFixture):
    """
    ...
    mock_refresh_schemas.assert_called_once_with(None, ["schema"])
    mock_rename_db.assert_called_once_with(None, ["schema"])


@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_get_catalog_names(mocker: MockerFixture):
    """
    ...
    cmmd = SyncPermissionsCommand(
        1,
        old_db_connection_name="DB Connection Name",
        db_connection=mock_db,
        ssh_tunnel=None,
    )
    result = cmmd._get_catalog_names()
...
@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_get_catalog_names_oauth2_exception(
    mocker: MockerFixture,
):
    """
...
@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_get_catalog_names_generic_db_exception(
    mocker: MockerFixture,
):
    """
    Test the ``_get_catalog_names`` method when an OAuth2 exception
...
@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_get_schema_names(mocker: MockerFixture):
    """
    ...
    result = cmmd._get_schema_names("my_catalog")
...
@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_get_schema_names_oauth2_exception(
    mocker: MockerFixture,
):
    """
    ...
    with pytest.raises(OAuth2RedirectError):
        cmmd._get_schema_names("my_catalog")


@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_get_schema_names_generic_db_exception(
    mocker: MockerFixture,
):
    """
    Test the ``_get_schema_names`` method when an OAuth2 exception
    ...
    with pytest.raises(DatabaseConnectionFailedError):
        cmmd._get_schema_names(None)


@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_refresh_schemas(mocker: MockerFixture):
    """
    ...
    get_schem_perm_mock = mocker.patch(
        "superset.commands.database.sync_permissions.security_manager.get_schema_perm"
    )
    ...
    cmmd._refresh_schemas("catalog", ["schema1", "schema2"])
...
@with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
def test_sync_permissions_command_rename_database_in_permissions(
    mocker: MockerFixture,
):
    """
    ...
    cmmd = SyncPermissionsCommand(
        1,
        old_db_connection_name="old_name",
        db_connection=mock_db,
        ssh_tunnel=None,
    )
    cmmd._rename_database_in_permissions("catalog", ["schema1", "schema2"])
...
def test_sync_permissions_command_async_mode_validate_new_db_name(
    ...
    command = SyncPermissionsCommand(
        1,
        username="username",
        old_db_connection_name="Old Connection Name",
    )
    command.validate()
