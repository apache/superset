...
def test_strip_params_removes_field() -> None:
    ...
    assert changed
    result = stdlib_json.loads(slc.params)
    assert _FIELD not in result["extra_form_data"]
    assert result["extra_form_data"]["filters"] == [{"col": "col1", "op": "==", "val": "a"}]


def test_strip_params_noop_when_field_absent() -> None:
    params = stdlib_json.dumps({"extra_form_data": {"filters": []}})
    slc = Slice(params=params)
    changed = _strip_params(slc)
    assert not changed
    assert stdlib_json.loads(slc.params) == {"extra_form_data": {"filters": []}}
...
def test_upgrade_strips_both_fields(engine) -> None:
    ...
    # connection and commit that connection after upgrade() — this matches how
    # Alembic runs migrations in production.  The session.flush() inside upgrade()
    # must emit the SQL UPDATEs before the connection is committed; without it the
    # outer commit has nothing to write and the data is left unchanged.
    ...
    with Session(engine) as verify:
        ...
        params1 = stdlib_json.loads(slc1.params)
        ...
        qc1 = stdlib_json.loads(slc1.query_context)
        ...
        params2 = stdlib_json.loads(slc2.params)
