...
def test_diff_layout_node_edit_emits_leaf_record_not_whole_node() -> None:
    ...
    assert len(records) == 1
    # Path is pure navigation: [node_id, ...leaf]. No verb prefix.
    assert records[0].path == ["HEADER-id-1", "text"]
...
def test_diff_layout_node_edit_emits_one_record_per_changed_leaf() -> None:
    ...
    assert paths == {
        ("HEADER-id-1", "text"),
        ("HEADER-id-1", "headerSize"),
    }
    # Every record's operation is "edit" — both leaves changed from one
    # populated value to another. kind stays "header" for both.
...
def test_diff_layout_node_add_remove_move_unchanged_shape() -> None:
    """Layout add/remove/move emit a single record carrying the minimal
    node payload. Path is pure navigation (``[node_id]``); the verb lives
    in ``operation``, the element type in ``kind``."""
    chart_node = {
        "id": "CHART-x",
        "type": "CHART",
    ...
    assert len(added) == 1
    assert added[0].path == ["CHART-x"]
    assert added[0].kind == "chart"
    assert added[0].operation == "add"
    ...
    assert removed[0].path == ["CHART-x"]
    assert removed[0].kind == "chart"
    ...
    assert len(moved) == 1
    assert moved[0].path == ["CHART-x"]
    assert moved[0].kind == "chart"
    assert moved[0].operation == "move"
...
def test_diff_layout_node_edit_respects_depth_cap() -> None:
    ...
    # Path is [node_id, <recurse...>]. _LAYOUT_META_DIFF_DEPTH=3 bounds
    # the recursion starting from the meta dict, so the deepest path
    # captures at most 3 levels below ``X``. The leaf is emitted as an
    # opaque sub-tree at the cap.
    assert len(records) == 1
    assert len(records[0].path) <= 1 + _LAYOUT_META_DIFF_DEPTH
