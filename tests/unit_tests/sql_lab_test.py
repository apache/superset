...
def test_rls_blocks_mutation_on_protected_table(mocker: MockerFixture, app) -> None:
    """Mutation on a table with active RLS rules is blocked when RLS_IN_SQLLAB=True."""
    ...
    with pytest.raises(SupersetSecurityException):
        execute_sql_statements(
            query_id=1,
            rendered_query="UPDATE secret_table SET customer_name='x' WHERE owner_id=1",
            return_results=True,
            store_results=False,
            start_time=None,
            expand_data=False,
