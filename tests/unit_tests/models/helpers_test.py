...
def test_values_for_column_passes_catalog_and_schema(
    ...
    """
    Test that `values_for_column` passes the dataset's catalog and schema to
    `get_sqla_engine`, so schema-in-URL databases query the correct schema.
    """
    import pandas as pd

    from superset.connectors.sqla.models import SqlaTable, TableColumn

    get_sqla_engine_calls: list[dict[str, str | None]] = []

    # Wrap the existing mock to record the catalog/schema arguments
    original_mock = database.get_sqla_engine

    @contextmanager
    def recording_get_sqla_engine(catalog=None, schema=None, **kwargs):
        get_sqla_engine_calls.append({"catalog": catalog, "schema": schema})
        with original_mock(catalog=catalog, schema=schema, **kwargs) as engine:
            yield engine

    mocker.patch.object(
        database,
        "get_sqla_engine",
        new=recording_get_sqla_engine,
    )

    # Test with an explicit catalog and schema
    table = SqlaTable(
        database=database,
        catalog="my_catalog",
        schema="my_schema",
    ...
    assert len(get_sqla_engine_calls) == 1
    assert get_sqla_engine_calls[0]["catalog"] == "my_catalog"
    assert get_sqla_engine_calls[0]["schema"] == "my_schema"

    # Test with None catalog and schema (default / no-schema case)
    get_sqla_engine_calls.clear()
    table_no_schema = SqlaTable(
        database=database,
        catalog=None,
        schema=None,
        table_name="t",
    ...
    with patch(
        ...
        table_no_schema.values_for_column("a")

    assert len(get_sqla_engine_calls) == 1
    assert get_sqla_engine_calls[0]["catalog"] is None
    assert get_sqla_engine_calls[0]["schema"] is None
