...
@patch("superset.commands.explore.form_data.create.CreateFormDataCommand.run")
@pytest.mark.asyncio
async def test_create_chart_explore_link_success(mock_form_data_run, mcp_server):
    mock_form_data_run.return_value = "test_form_data_key_123"

    # Create a simple table chart request with save_chart=False
    config = TableChartConfig(
        chart_type="table",
        columns=[
            ColumnRef(name="region", label="Region"),
    ...
    async with Client(mcp_server) as client:
        resp = await client.call_tool("create_chart", {"request": request.model_dump()})
        assert resp.data["chart"] is None
        assert resp.data["explore_url"] is not None
        assert "form_data_key=test_form_data_key_123" in resp.data["explore_url"]
        assert "datasource_id=1" in resp.data["explore_url"]
        assert resp.data["error"] is None
        mock_form_data_run.assert_called_once()


@patch("superset.commands.explore.form_data.create.CreateFormDataCommand.run")
@pytest.mark.asyncio
async def test_create_chart_explore_link_fallback(mock_form_data_run, mcp_server):
    # Simulate form data caching failure
    mock_form_data_run.side_effect = Exception("Cache error")

    # Create a simple line chart request with save_chart=False
    config = XYChartConfig(
        chart_type="xy",
        x=ColumnRef(name="date", label="Date"),
        y=[ColumnRef(name="sales", label="Sales")],
    ...
    async with Client(mcp_server) as client:
        ...
        assert resp.data["chart"] is None
        assert resp.data["explore_url"] is not None
        assert "form_data=" in resp.data["explore_url"]
        assert "datasource_id=1" in resp.data["explore_url"]
        assert resp.data["error"] is None
