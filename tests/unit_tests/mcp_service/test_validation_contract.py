...
class ReasonChoice(Enum):
    """Allowed values for enum validation coverage."""

    ALLOWED = "allowed"


class ReasonRequest(SensitiveRequest):
    """Real constraints that produce representative Pydantic error codes."""

    model_config = ConfigDict(extra="forbid")

    above: int = Field(default=2, gt=1)
    below: int = Field(default=2, lt=3)
    at_least: int = Field(default=2, ge=1)
    at_most: int = Field(default=2, le=3)
    choice: ReasonChoice = ReasonChoice.ALLOWED
    literal: Literal["allowed"] = "allowed"
    text: str = Field(default="ok", min_length=2, max_length=3)


@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("arguments", "error_type", "reason"),
    [
        ({"extra": 1}, "extra_forbidden", "Unexpected field"),
        ({"above": 1}, "greater_than", "Must be greater than the allowed minimum"),
        ({"below": 3}, "less_than", "Must be less than the allowed maximum"),
        ({"at_least": 0}, "greater_than_equal", "Must be at least the allowed minimum"),
        ({"at_most": 4}, "less_than_equal", "Must be at most the allowed maximum"),
        ({"choice": "other"}, "enum", "Expected one of the allowed values"),
        ({"literal": "other"}, "literal_error", "Expected one of the allowed values"),
        (
            {"text": "a"},
            "string_too_short",
            "String is shorter than the allowed minimum",
        ),
        (
            {"text": "long"},
            "string_too_long",
            "String exceeds the allowed maximum length",
        ),
        ({"secret": "private"}, "value_error", "Invalid value; check the field schema"),
    ],
)
async def test_validation_reason_strings(
    arguments: dict[str, Any], error_type: str, reason: str
) -> None:
    """Pin static reason strings using errors generated by real model validation."""
    with pytest.raises(ValidationError) as exc_info:
        ReasonRequest.model_validate(arguments)

    assert [error["type"] for error in exc_info.value.errors()] == [error_type]
    context = MiddlewareContext(message=CallToolRequestParams(name="reasons"))
    message = await validation_message(exc_info.value, context)

    assert message == f"Request validation failed: [field]: {reason}"
