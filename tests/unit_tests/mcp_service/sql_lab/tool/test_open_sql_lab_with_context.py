...
def _force_passthrough_decorators() -> dict[str, types.ModuleType]:
    ...
    # 6.0 adaptation: tool uses @mcp.tool + @mcp_auth_hook from superset.mcp_service
    # instead of master's @tool from superset_core. Stub both as passthroughs so
    # direct function invocation in tests skips auth/permission gates.
    mock_app = MagicMock()
    mock_app.mcp = mock_mcp
    sys.modules["superset.mcp_service.app"] = mock_app

    mock_auth = MagicMock()
    mock_auth.mcp_auth_hook = _passthrough_tool
    sys.modules["superset.mcp_service.auth"] = mock_auth
