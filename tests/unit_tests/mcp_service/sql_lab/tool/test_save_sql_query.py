...
def _force_passthrough_decorators():
    """Force mcp.tool and mcp_auth_hook decorators to be passthroughs.

    On 6.0-release, tools use @mcp.tool and @mcp_auth_hook from the local
    codebase. For unit tests we want to bypass auth and test tool logic
    directly, so we replace decorators with passthroughs.
    ...
    def _passthrough_auth_hook(*args, **kwargs):
    ...
    # Mock the app module so @mcp.tool is a passthrough
    app_mod_name = "superset.mcp_service.app"
    if app_mod_name in sys.modules:
        saved_modules[app_mod_name] = sys.modules[app_mod_name]
    mock_app = types.ModuleType(app_mod_name)
    mock_mcp = MagicMock()
    mock_mcp.tool = _passthrough_tool
    mock_app.mcp = mock_mcp
    sys.modules[app_mod_name] = mock_app

    # Mock the auth module so @mcp_auth_hook is a passthrough
    auth_mod_name = "superset.mcp_service.auth"
    if auth_mod_name in sys.modules:
        saved_modules[auth_mod_name] = sys.modules[auth_mod_name]
    mock_auth = types.ModuleType(auth_mod_name)
    mock_auth.mcp_auth_hook = _passthrough_auth_hook
    sys.modules[auth_mod_name] = mock_auth
