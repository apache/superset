...
def test_mixed_timeseries_with_wrong_kind_fields_returns_actionable_error() -> None:
    ...
    # ``ChartConfig`` (PR #39732 backport), so this Pydantic ValueError is
    # caught by ``SchemaValidator._enhance_validation_error`` before it can
    # bubble to ``ValidationPipeline``'s generic ``except Exception`` branch.
    # The early-catch path emits ``VALIDATION_ERROR`` instead of upstream's
    # ``VALIDATION_PIPELINE_ERROR``; both are actionable and include the
    # cleaned-up tagged-union body, which is what this regression guards.
    assert dumped["error_code"] in {"VALIDATION_PIPELINE_ERROR", "VALIDATION_ERROR"}
...
def test_non_value_error_pydantic_body_is_surfaced() -> None:
    ...
    assert result.error is not None
    dumped = result.error.model_dump()
    # See note in test_mixed_timeseries_with_wrong_kind_fields_returns_actionable_error
    # — 6.0's typed ``ChartConfig`` catches this in SchemaValidator first.
    assert dumped["error_code"] in {"VALIDATION_PIPELINE_ERROR", "VALIDATION_ERROR"}
    assert "tagged-union" not in dumped["message"]
    assert "tagged-union" not in dumped["details"]
