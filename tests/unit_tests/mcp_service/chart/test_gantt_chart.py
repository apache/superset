...
def test_gantt_update_preview_matches_would_be_persisted_state(
    ...
    assert preview["slice_name"] == "Gantt"
    assert isinstance(validate_gantt_form_data(preview), GanttChartConfig)
    persisted = __import__("json").loads(payload["params"])
    preview_state = {
        key: value
...
@pytest.mark.asyncio
async def test_default_gantt_update_validates_named_preview_before_caching() -> None:
    """Default updates validate the complete preview envelope, not stripped params."""
    chart = SimpleNamespace(
        id=1495,
        datasource_id=1,
        datasource=SimpleNamespace(id=1),
        slice_name="Saved schedule",
        params=__import__("json").dumps(_native_example()),
        uuid="chart-uuid",
        viz_type="gantt_chart",
    )
    request = UpdateChartRequest(identifier=1495, config=_config())
    assert request.generate_preview is True

    with (
        patch.object(
            update_chart_module.event_logger,
            "log_context",
            side_effect=lambda **kwargs: nullcontext(),
        ),
        patch(
            "superset.mcp_service.auth.get_user_from_request", return_value=Mock(id=1)
        ),
        patch.object(
            update_chart_module, "find_chart_by_identifier", return_value=chart
        ),
        ...
        patch.object(
            DatasetValidator, "_get_dataset_context", return_value=_dataset_context()
        ),
        patch.object(
            update_chart_module,
        ...
        patch.object(
            ...
            "_create_preview_url",
            return_value=(
                "http://superset/explore/?form_data_key=preview",
                "preview",
                [],
            ),
        ) as create_preview,
    ...
    assert result.success is True
    validated_config, preview, dataset = compile_chart.call_args.args
    assert isinstance(validated_config, GanttChartConfig)
    assert preview["slice_name"] == "Saved schedule"
    assert preview["slice_id"] == chart.id
    assert preview["datasource"] == "1__table"
    assert dataset is chart.datasource
    assert create_preview.call_args.args[1] == preview
    command.assert_not_called()


def test_gantt_transport_name_does_not_allow_unknown_controls() -> None:
    """Allow chart naming metadata without weakening visualization typo checks."""
    form_data = {**map_gantt_config(_config()), "slice_name": "Saved schedule"}
    with pytest.raises(
        GanttSemanticNormalizationError, match="Unknown field 'zoomabl'"
    ):
        validate_gantt_form_data({**form_data, "zoomabl": True})


def _gantt_partial_update_states(
    existing: dict[str, object], config: GanttChartConfig, dataset_rebind: bool = False
) -> list[dict[str, Any]]:
    """Build saved-preview, persisted, and cached-preview shared merge states."""
    chart = SimpleNamespace(
        id=1495,
        datasource_id=1,
        slice_name="Saved schedule",
    ...
    request = UpdateChartRequest(
        identifier=1495, config=config, dataset_id=2 if dataset_rebind else None
    )
    with patch(
...
def test_gantt_partial_update_preserves_omitted_query_controls(explicit: bool) -> None:
    """Omitted query controls survive; explicit empty/default values replace them."""
    existing = map_gantt_config(
        _config(
            tooltip_columns=[{"name": "project"}],
            tooltip_metrics=[{"name": "priority", "aggregate": "SUM"}],
            order_by=[{"column": "priority", "ascending": False}],
            row_limit=42,
        )
    )
    config = (
        ...
        if explicit
        else _config(zoomable=True)
    )
    for state in _gantt_partial_update_states(existing, config):
        assert state["zoomable"] is True
        for key in ("tooltip_columns", "tooltip_metrics", "order_by_cols", "row_limit"):
            assert state[key] == (
                map_gantt_config(config)[key] if explicit else existing[key]
            )
        assert isinstance(validate_gantt_form_data(state), GanttChartConfig)
    assert existing["row_limit"] == 42


def test_gantt_dataset_rebind_drops_saved_query_references() -> None:
    """Changing datasets must not carry old series, filters, or tooltip/sort roles."""
    existing = map_gantt_config(
        _config(
            series={"name": "old_series"},
            subcategories=True,
            filters=[{"column": "old_filter", "op": "=", "value": "old"}],
            temporal_column="old_time",
            time_range="Last month",
            tooltip_columns=[{"name": "old_tooltip"}],
            tooltip_metrics=[{"name": "old_metric", "saved_metric": True}],
            order_by=[{"column": "old_sort", "ascending": True}],
            row_limit=42,
        )
    )
    config = _config()
    for state in _gantt_partial_update_states(existing, config, dataset_rebind=True):
        assert not state.get("series")
        assert not state.get("subcategories")
        assert state["tooltip_columns"] == []
        assert state["tooltip_metrics"] == []
        assert state["order_by_cols"] == []
        assert state["row_limit"] == 10000
        assert state["adhoc_filters"] == map_gantt_config(config)["adhoc_filters"]
        assert "old_" not in __import__("json").dumps(state)
        assert isinstance(validate_gantt_form_data(state), GanttChartConfig)


@pytest.mark.parametrize(
    ("overrides", "subject", "comparator"),
    [
        ({}, "end_time", "Last month"),
        ({"time_range": "Last 7 days"}, "start_time", "Last 7 days"),
        ({"temporal_column": "start_time"}, "start_time", "No filter"),
...
def test_gantt_partial_update_reconciles_native_temporal_filter(
    ...
    for state in _gantt_partial_update_states(
        ...
        adapted = validate_gantt_form_data(state)
        assert isinstance(adapted, GanttChartConfig)
        temporal = [
            item
            for item in state["adhoc_filters"]
...
@pytest.mark.parametrize(
    ("bounds", "expected"),
    [
        (["", "19:00:00"], (None, "19:00:00")),
        (["08:00:00", ""], ("08:00:00", None)),
        (["", ""], (None, None)),
        ([None, ""], (None, None)),
    ],
)
def test_gantt_partial_update_accepts_empty_native_time_bounds(
    bounds: list[str | None], expected: tuple[str | None, str | None]
) -> None:
    """Explore's empty bound strings round-trip as unset values during updates."""
    existing = {**map_gantt_config(_config()), "x_axis_time_bounds": bounds}
    for state in _gantt_partial_update_states(existing, _config(zoomable=True)):
        adapted = validate_gantt_form_data(state)
        assert isinstance(adapted, GanttChartConfig)
        assert adapted.x_axis_time_bounds == expected
        assert map_gantt_config(adapted)["x_axis_time_bounds"] == list(expected)


@pytest.mark.parametrize("overrides", [{}, {"time_range": "Last 7 days"}])
def test_gantt_partial_update_rejects_multiple_native_temporal_predicates(
    overrides: dict[str, object],
) -> None:
    """Reconciliation must not silently discard multiple user time predicates."""
    existing = map_gantt_config(
        _config(temporal_column="end_time", time_range="Last month")
    )
    existing.pop("_mcp_dashboard_time_filter_subject")
    existing["adhoc_filters"].append(
        {
            "expressionType": "SIMPLE",
            "clause": "WHERE",
            "operator": "TEMPORAL_RANGE",
    ...
    states = _gantt_partial_update_states(existing, _config(**overrides))
    for state in states:
        with pytest.raises(
            GanttSemanticNormalizationError, match="at most one TEMPORAL_RANGE"
        ):
            validate_gantt_form_data(state)


@pytest.mark.parametrize("bound", [" ", "8am", "08:00", "25:00:00"])
def test_gantt_partial_update_still_rejects_invalid_nonempty_time_bounds(
    bound: str,
) -> None:
    """Only empty bound strings are unset; malformed times remain errors."""
    existing = {**map_gantt_config(_config()), "x_axis_time_bounds": [bound, ""]}
    with pytest.raises(GanttSemanticNormalizationError, match="HH:MM:SS"):
        _gantt_partial_update_states(existing, _config())
