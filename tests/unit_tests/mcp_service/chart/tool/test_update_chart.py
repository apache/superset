...
# The __init__.py re-exports the update_chart and generate_chart *functions*,
# so a plain `from ... import update_chart` gives the function, not the module.
...
class TestUpdateChartSaveWithConfig:
    ...
    async def test_save_chart_with_config_success(
        ...
        with patch.object(generate_chart_module, "_compile_chart") as mock_compile:
            from superset.mcp_service.chart.tool.generate_chart import CompileResult
...
class TestUpdateChartQueryContextReset:
    """Tests that config updates always reset query_context (Issues 1 & 2)."""

    def test_config_payload_includes_query_context_none(self):
        """_build_update_payload sets query_context=None for config updates."""
        config = TableChartConfig(
            chart_type="table",
    ...
    def test_name_only_payload_does_not_include_query_context(self):
        """_build_update_payload does not touch query_context for rename-only."""
        request = UpdateChartRequest(identifier=1, chart_name="New Name")
        chart = Mock()
    ...
    @patch("superset.db.session")
    @pytest.mark.asyncio
    async def test_config_update_passes_query_context_none_to_command(
        self,
        mock_db_session,
        mock_find_by_id,
        mock_update_cmd_cls,
        ...
        """UpdateChartCommand is called with query_context=None for config updates."""
        mock_chart = Mock()
        mock_chart.id = 5
        mock_chart.datasource_id = 10
        ...
        # Patch the compile check so it passes without a real DB
        with patch.object(generate_chart_module, "_compile_chart") as mock_compile:
...
class TestUpdateChartConfigPersistence:
    """Tests that filters and row_limit are persisted in params (Issues 1 & 2)."""
    ...
    @patch("superset.db.session")
    @pytest.mark.asyncio
    async def test_filters_persisted_in_params(
        self,
        mock_db_session,
        mock_find_by_id,
        mock_update_cmd_cls,
        ...
        """Filters in config are saved inside params['adhoc_filters']."""
        ...
        updated_chart = Mock()
        updated_chart.id = 1
        updated_chart.slice_name = "Chart"
        updated_chart.viz_type = "table"
        updated_chart.uuid = "uuid-1"
        mock_update_cmd_cls.return_value.run.return_value = updated_chart
        ...
        request = {
            ...
            "config": {
                "chart_type": "table",
                "columns": [{"name": "country"}, {"name": "sales"}],
        ...
        call_args = mock_update_cmd_cls.call_args
        payload = call_args[0][1]
        params = utils_json.loads(payload["params"])
        assert "adhoc_filters" in params
        assert len(params["adhoc_filters"]) >= 1
        subjects = [f.get("subject") for f in params["adhoc_filters"]]
        assert "country" in subjects
    ...
    @patch("superset.db.session")
    @pytest.mark.asyncio
    async def test_row_limit_persisted_in_params(
        self,
        mock_db_session,
        mock_find_by_id,
        mock_update_cmd_cls,
        ...
        """row_limit in config is saved inside params['row_limit']."""
        from superset.utils import json as utils_json
        ...
        payload = call_args[0][1]
        params = utils_json.loads(payload["params"])
        assert params["row_limit"] == 75


class TestUpdateChartCompileCheck:
    """Tests for the compile check that guards against invalid column names."""
    ...
    @patch("superset.db.session")
    @pytest.mark.asyncio
    async def test_compile_failure_prevents_db_write(
        self,
        mock_db_session,
        mock_find_by_id,
        mock_update_cmd_cls,
        ...
        """When compile check fails, UpdateChartCommand is never called."""
        mock_chart = Mock()
        mock_chart.id = 3
        mock_chart.datasource_id = 10
        mock_chart.slice_name = "Chart"
        mock_chart.viz_type = "table"
        mock_chart.uuid = "uuid-3"
        mock_find_by_id.return_value = mock_chart
        ...
        with patch.object(generate_chart_module, "_compile_chart") as mock_compile:
            ...
            mock_compile.return_value = CompileResult(
                success=False, error="Column 'nonexistent_column' not found in dataset"
            )
        ...
        assert "not modified" in error["message"]
        assert "nonexistent_column" in error["details"]
        mock_update_cmd_cls.assert_not_called()
    ...
    @patch("superset.db.session")
    @pytest.mark.asyncio
    async def test_compile_success_allows_db_write(
        self,
        mock_db_session,
        mock_find_by_id,
        mock_update_cmd_cls,
        ...
        """When compile check passes, UpdateChartCommand is called exactly once."""
        mock_chart = Mock()
        mock_chart.id = 4
        mock_chart.datasource_id = 10
        mock_chart.slice_name = "Chart"
        mock_chart.viz_type = "table"
        mock_chart.uuid = "uuid-4"
        mock_find_by_id.return_value = mock_chart
        ...
        with patch.object(generate_chart_module, "_compile_chart") as mock_compile:
            ...
            mock_compile.return_value = CompileResult(success=True, row_count=10)
    ...
    @patch("superset.db.session")
    @pytest.mark.asyncio
    async def test_compile_check_skipped_when_no_datasource_id(
        self,
        mock_db_session,
        mock_find_by_id,
        mock_update_cmd_cls,
        ...
        """Config update proceeds without compile check when chart has no dataset."""
        mock_chart = Mock()
        mock_chart.id = 6
        mock_chart.datasource_id = None  # no linked dataset
        mock_chart.slice_name = "Chart"
        mock_chart.viz_type = "table"
        mock_chart.uuid = "uuid-6"
        mock_find_by_id.return_value = mock_chart
        ...
        # DB write must still happen even without a compile check
        assert result.structured_content["success"] is True
        mock_update_cmd_cls.assert_called_once()
        # compile check must be skipped entirely when there is no dataset
        mock_compile.assert_not_called()


class TestUpdateChartFormDataResponse:
    """Tests that the success response includes form_data (Issue 5)."""
    ...
    @patch("superset.db.session")
    @pytest.mark.asyncio
    async def test_config_update_response_contains_form_data(
        self,
        mock_db_session,
        mock_find_by_id,
        mock_update_cmd_cls,
        ...
        """Success response for a config update contains non-empty form_data."""
        mock_chart = Mock()
        mock_chart.id = 10
        mock_chart.datasource_id = 20
        mock_chart.slice_name = "Chart"
        mock_chart.viz_type = "table"
        mock_chart.uuid = "uuid-10"
        mock_find_by_id.return_value = mock_chart
        ...
        assert form_data["row_limit"] == 200
    ...
    @patch("superset.db.session")
    @pytest.mark.asyncio
    async def test_rename_only_response_form_data_is_empty(
        self,
        mock_db_session,
        mock_find_by_id,
        mock_update_cmd_cls,
        ...
        """Rename-only response has form_data: {} (visualization unchanged)."""
        mock_chart = Mock()
        mock_chart.id = 11
        mock_chart.datasource_id = 20
        mock_chart.slice_name = "Old Name"
        mock_chart.viz_type = "table"
        mock_chart.uuid = "uuid-11"
        mock_find_by_id.return_value = mock_chart
        ...
        updated_chart = Mock()
        updated_chart.id = 11
        updated_chart.slice_name = "New Name"
        updated_chart.viz_type = "table"
        updated_chart.uuid = "uuid-11"
        mock_update_cmd_cls.return_value.run.return_value = updated_chart
