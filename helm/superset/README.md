...
| externalSecrets | object | `{"enabled":false,"existingSecret":"","keys":{"adminPassword":"superset/admin-password","databasePassword":"superset/database-password","jwtSecret":"superset/jwt-secret","redisPassword":"superset/redis-password","secretKey":"superset/secret-key"},"secretStoreRef":{"kind":"ClusterSecretStore","name":""}}` | External Secrets Operator integration Use this to integrate with External Secrets Operator (ESO) for secret management Supports both ExternalSecret CRD and direct secret references |
| externalSecrets.enabled | bool | `false` | Enable External Secrets Operator integration |
| externalSecrets.existingSecret | string | `""` | Alternative: Use an existing secret name directly If set, the chart will reference this secret instead of creating one This is useful when External Secrets Operator has already synced a secret |
| externalSecrets.keys | object | `{"adminPassword":"superset/admin-password","databasePassword":"superset/database-password","jwtSecret":"superset/jwt-secret","redisPassword":"superset/redis-password","secretKey":"superset/secret-key"}` | Key mapping for External Secrets Maps chart secret keys to external secret keys |
| externalSecrets.keys.adminPassword | string | `"superset/admin-password"` | External secret key for admin user password |
| externalSecrets.keys.databasePassword | string | `"superset/database-password"` | External secret key for database password |
| externalSecrets.keys.jwtSecret | string | `"superset/jwt-secret"` | External secret key for JWT secret (websockets) |
| externalSecrets.keys.redisPassword | string | `"superset/redis-password"` | External secret key for Redis password |
| externalSecrets.keys.secretKey | string | `"superset/secret-key"` | External secret key for Superset secret key |
| externalSecrets.secretStoreRef | object | `{"kind":"ClusterSecretStore","name":""}` | External Secrets Operator configuration Use this when you want the chart to create an ExternalSecret CRD |
| externalSecrets.secretStoreRef.kind | string | `"ClusterSecretStore"` | Kind of the secret store (SecretStore or ClusterSecretStore) |
| externalSecrets.secretStoreRef.name | string | `""` | Name of the SecretStore or ClusterSecretStore |
