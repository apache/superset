...
To use credentials managed outside the chart, create a Secret in the release's namespace
and reference its keys:
...
The Secret must contain the selected keys. Omit `secretKeys.user` to keep using
`database.user` (or the legacy username). The password key defaults to `password`.
The chart uses `secretKeyRef` in the application containers and initialization job;
it does not read or copy the credentials into its generated Secret. The built-in
wait containers only check the database host and port.

Secret references override literal and legacy credentials. A full `database.uri`
takes precedence and disables these references. Avoid also setting the selected
`DB_USER` or `DB_PASS` through environment overrides. Keep `secretEnv.create` enabled
unless you already provide the other required environment settings through an external
Secret. Restart the workloads after rotating credentials in the existing Secret.
...
| database.existingSecret | string | `""` | Existing Secret containing database credentials (supports templating). Ignored when database.uri is set. |
...
| database.secretKeys.password | string | `"password"` | Password key in database.existingSecret. Overrides database.password and legacy db_pass. |
| database.secretKeys.user | string | `""` | Optional username key in database.existingSecret. When empty, database.user and legacy db_user are used. |
