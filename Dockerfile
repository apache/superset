...
RUN wget https://github.com/mozilla/geckodriver/releases/download/${GECKODRIVER_VERSION}/geckodriver-${GECKODRIVER_VERSION}-linux64.tar.gz -O - | tar xzf - -C /usr/local/bin \
    # Install Firefox
    ...
    && pip install --no-cache -r /app/requirements/docker.txt \
    && pip install --no-cache -r /app/requirements/requirements-local.txt || true
USER superset
...
COPY --chown=superset --chmod=755 ./docker/docker-*.sh /app/docker/
