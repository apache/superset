...
- [43916](https://github.com/apache/superset/pull/43916): The `docker-compose` dev loop now skips re-running `superset load_examples` on every `docker compose up` once example data has loaded successfully into the `superset_home` volume (subsequent runs only refresh metadata; set `SUPERSET_FORCE_LOAD_EXAMPLES=yes` to force a full reload), and the `superset-node` service now defaults `DISABLE_TS_CHECKER=true` like `docker-compose-light.yml` already did, skipping webpack's TypeScript type-checking pass in dev by default.
