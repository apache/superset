...
Everything consuming the gate inherits the tightening — including **alert and report execution**, not only schedule creation/validation. An already-scheduled report against an *unpublished, no-viewers* dashboard whose execution principal (`ALERT_REPORTS_EXECUTE_AS`, a service or selenium account) is a datasource-entitled non-editor will fail on its next run after upgrade. Before upgrading, audit report schedules targeting unpublished dashboards and either publish the dashboard, add the executing principal (or one of its roles) to the dashboard's viewers, or re-own the schedule to a dashboard editor.
