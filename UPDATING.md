...
- [44499](https://github.com/apache/superset/pull/44499): New opt-in feature flag `REQUIRE_DOWNLOAD_REASON` (default `False`). When enabled, users are asked why they are downloading data before CSV/XLSX exports (chart data, SQL Lab, dashboard Excel export) and the answer is sent as the new `download_reason` query parameter, which the event logger records in `logs.json` (Security → Action Log). With the flag on, `GET/POST /api/v1/chart/data`, `GET /api/v1/sqllab/export/<client_id>/`, `POST /api/v1/sqllab/export_streaming/` and `POST /api/v1/dashboard/<pk>/export_xlsx/` return `400` when the parameter is missing; API clients must pass it. Scheduled reports supply their own reason automatically. Nothing changes while the flag is off.
